...
__device__ void filter_and_histogram(const T *in_buf, const IdxT *in_idx_buf,
...
__device__ void scan(volatile IdxT *histogram) {
  ...
  if constexpr (num_buckets >= BlockSize) {
    ...
    typedef cub::BlockLoad<IdxT, BlockSize, items_per_thread,
                           cub::BLOCK_LOAD_TRANSPOSE>
        BlockLoad;
    typedef cub::BlockStore<IdxT, BlockSize, items_per_thread,
...
__device__ void scan_warp_version(cg::thread_block_tile<WARP_SIZE> const &warp,
                                  volatile IdxT *histogram,
                                  Counter<T, IdxT> *counter, const IdxT k,
                                  const int pass) {
  constexpr int num_buckets = calc_num_buckets<BitsPerPass>();

  __shared__ IdxT warp_histogram[num_buckets / WARP_SIZE];
  for (int i = threadIdx.x; i < num_buckets; i += BlockSize) {
    IdxT data = histogram[i];
    IdxT warp_sum = cg::reduce(warp, data, cg::plus<IdxT>());
    if (i % WARP_SIZE == 0) {
      warp_histogram[i / WARP_SIZE] = warp_sum;
    }
  }
  __syncthreads();
  ...
  if (threadIdx.x < WARP_SIZE) {
    IdxT value =
        warp_histogram[threadIdx.x * 2] + warp_histogram[threadIdx.x * 2 + 1];
    ...
    IdxT prefix_high =
        __shfl_sync(FULL_WARP_MASK, value, threadIdx.x - 1, WARP_SIZE);
    if (threadIdx.x == 0)
      prefix_high = 0;
    ...
    for (int i = threadIdx.x; i < 64; i += WARP_SIZE) {
      ...
      if (__popc(mask) > 0) {
        target_warp = __ffs(mask) + i / WARP_SIZE;
        break;
      }
    }

    // Find the target bucket
    value = histogram[target_warp * WARP_SIZE + threadIdx.x];
    for (int offset = 1; offset < WARP_SIZE; offset <<= 1) {
      IdxT n = __shfl_up_sync(FULL_WARP_MASK, value, offset, WARP_SIZE);
      if (threadIdx.x >= offset)
        value += n;
    }
    value += warp_histogram[target_warp * WARP_SIZE];
    for (int i = threadIdx.x; i < WARP_SIZE; i += WARP_SIZE) {
      IdxT prev =
          (i == 0) ? 0 : __shfl_up_sync(FULL_WARP_MASK, value, 1, WARP_SIZE);
      IdxT cur = value;
      if (prev < k && cur >= k) {
        counter->k = k - prev;     // how many values still are there to find
        counter->len = cur - prev; // number of values in next pass
        typename cub::Traits<T>::UnsignedBits bucket =
            target_warp * WARP_SIZE + i;
        int start_bit = calc_start_bit<T, BitsPerPass>(pass);
        counter->kth_value_bits |= bucket << start_bit;
        printf("target_warp: %d, i: %d, bucket: %d, start_bit: %d prev=%d, "
               "cur=%d, k=%d\n",
               target_warp, i, bucket, start_bit, prev, cur, counter->k);
      }
    }
  }
}
 ...
 */
template <typename T, typename IdxT, int BitsPerPass>
__device__ void choose_bucket(Counter<T, IdxT> *counter, const IdxT *histogram,
                              const IdxT k, const int pass) {
  constexpr int num_buckets = calc_num_buckets<BitsPerPass>();
  for (int i = threadIdx.x; i < num_buckets; i += blockDim.x) {
...
__device__ void last_filter(const T *in_buf, const IdxT *in_idx_buf, T *out,
                            IdxT *out_idx, IdxT current_len, IdxT k,
                            Counter<T, IdxT> *counter, const bool select_min,
                            const int pass) {
  const auto kth_value_bits = counter->kth_value_bits;
  const int start_bit = calc_start_bit<T, BitsPerPass>(pass);
...
template <typename T, typename IdxT, int BitsPerPass,
          bool prioritize_smaller_indice = false>
__global__ void
last_filter_kernel(const T *in, const IdxT *in_idx, const T *in_buf,
                   const IdxT *in_idx_buf, T *out, IdxT *out_idx, IdxT len,
                   IdxT k, Counter<T, IdxT> *counters, const bool select_min) {
  ...
  auto f = [k, select_min, kth_value_bits, num_of_kth_needed, p_out_cnt,
            p_out_back_cnt, in_idx_buf, out, out_idx, p_equal,
 ...
 */
template <typename T, typename IdxT, int BitsPerPass, int BlockSize,
          bool fused_last_filter, bool prioritize_smaller_indice = false>
__global__ void radix_kernel(const T *in, const IdxT *in_idx, const T *in_buf,
                             ...
                             Counter<T, IdxT> *counters, IdxT *histograms,
                             const IdxT len, const IdxT k,
    ...
    if (pass == num_passes - 1) {
      ...
      if constexpr (fused_last_filter) {
        last_filter<T, IdxT, BitsPerPass, prioritize_smaller_indice>(
            ...
            out, out_idx, out_buf ? current_len : len, k, counter, select_min,
            pass);
      }
    }
  }
...
unsigned calc_grid_dim(int batch_size, IdxT len, int sm_cnt) {
  ...
  cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks,
      radix_kernel<T, IdxT, BitsPerPass, BlockSize, false, true>, BlockSize, 0);
...
set_buf_pointers(const T *in, const IdxT *in_idx, T *buf1, IdxT *idx_buf1,
                 T *buf2, IdxT *idx_buf2, int pass, const T *&in_buf,
                 const IdxT *&in_idx_buf, T *&out_buf, IdxT *&out_idx_buf) {
  if (pass == 0) {
    in_buf = in;
    in_idx_buf = nullptr;
    out_buf = nullptr;
...
template <typename T, typename IdxT>
__device__ void set_buf_pointers(const T *in, const IdxT *in_idx, char *bufs,
                                 IdxT buf_len, int pass, const T *&in_buf,
                                 const IdxT *&in_idx_buf, T *&out_buf,
                                 IdxT *&out_idx_buf) {
...
__device__ void filter_and_histogram_for_one_block(
    const T *in_buf, const IdxT *in_idx_buf, T *out_buf, IdxT *out_idx_buf,
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize,
          bool prioritize_smaller_indice = false>
__global__ void radix_topk_one_block_kernel(const T *in, const IdxT *in_idx,
                                            const IdxT len, const IdxT k,
                                            T *out, IdxT *out_idx,
  ...
  for (int pass = 0; pass < num_passes; ++pass) {
    ...
    choose_bucket<T, IdxT, BitsPerPass>(&counter, histogram, current_k,
    pass);
    if (threadIdx.x == 0) {
      counter.previous_len = current_len;
    }
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize>
void standalone_stable_radix_topk_(void *buf, size_t &buf_size, const T *in,
                                   const IdxT *in_idx, int batch_size, IdxT len,
                                   IdxT k, T *out, IdxT *out_idx,
                                   bool select_min, bool fused_last_filter,
                                   unsigned grid_dim, cudaStream_t stream) {
  ...
  auto kernel = air_topk_stable::radix_kernel<T, IdxT, BitsPerPass, BlockSize,
                                              false, true>;

  for (int pass = 0; pass < num_passes; ++pass) {
    air_topk_stable::set_buf_pointers(in, in_idx, buf1, idx_buf1, buf2,
                                      idx_buf2, pass, in_buf, in_idx_buf,
                                      out_buf, out_idx_buf);

    if (fused_last_filter && pass == num_passes - 1) {
      kernel = air_topk_stable::radix_kernel<T, IdxT, BitsPerPass, BlockSize,
                                             true, true>;
    }

    kernel<<<blocks, BlockSize, 0, stream>>>(
        in, in_idx, in_buf, in_idx_buf, out_buf, out_idx_buf, out, out_idx,
        counters, histograms, len, k, select_min, pass);
  }
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize>
void standalone_stable_radix_topk_one_block_(void *buf, size_t &buf_size,
  ...
  air_topk_stable::radix_topk_one_block_kernel<T, IdxT, BitsPerPass, BlockSize,
                                               true>
      <<<batch_size, BlockSize, 0, stream>>>(in, in_idx, len, k, out, out_idx,
                                             select_min, bufs);
}

template <typename T, typename idxT>
void standalone_stable_radix_11bits(void *buf, size_t &buf_size, const T *in,
  ...
  if (len <= block_dim * items_per_thread) {
    standalone_stable_radix_topk_one_block_<T, idxT, 11, block_dim>(
        buf, buf_size, in, static_cast<idxT *>(nullptr), batch_size, len, k,
        out, out_idx, !greater, stream);
  } else {
    int sm_cnt;
    {
      int dev;
    ...
    if (grid_dim == 1) {
      ...
      standalone_stable_radix_topk_<T, idxT, 11, block_dim>(
          buf, buf_size, in, static_cast<idxT *>(nullptr), batch_size, len, k,
          out, out_idx, !greater, fused_last_filter, grid_dim, stream);
    }
  }
}
} // namespace nv
