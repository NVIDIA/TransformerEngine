...
__device__ void scan_warp_version(cg::thread_block_tile<WARP_SIZE> const &warp,
  ...
  if (threadIdx.x < WARP_SIZE) {
    ...
    IdxT prefix_high =
        __shfl_sync(FULL_WARP_MASK, prefix, threadIdx.x - 1, WARP_SIZE);
    if (threadIdx.x == 0)
      prefix_high = 0;
...
last_filter_kernel(const T *in, const IdxT *in_idx, const T *in_buf,
  ...
  auto f = [k, select_min, kth_value_bits, needed_num_of_kth, p_out_cnt,
            p_out_back_cnt, in_idx_buf, out, out_idx](T value, IdxT i) {
    const auto bits = (twiddle_in(value, select_min) >> start_bit) << start_bit;
    if (bits < kth_value_bits) {
      IdxT pos = atomicAdd(p_out_cnt, static_cast<IdxT>(1));
      if constexpr (store_out) {
 ...
 */
template <typename T, typename IdxT, int BitsPerPass, int BlockSize,
          bool fused_last_filter, bool store_out>
__device__ void radix_kernel_func(const T *in, const IdxT *in_idx,
                                  const T *in_buf, const IdxT *in_idx_buf,
                                  T *out_buf, IdxT *out_idx_buf, T *out,
                                  IdxT *out_idx, Counter<T, IdxT> *counter,
                                  IdxT *histogram, const IdxT len, const IdxT k,
  ...
  filter_and_histogram<T, IdxT, BitsPerPass, store_out>(
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize,
          bool fused_last_filter, bool store_out>
__global__ void
radix_kernel(const T *in, const IdxT *in_idx, const T *in_buf,
  ...
  radix_kernel_func<T, IdxT, BitsPerPass, BlockSize, fused_last_filter,
                    store_out>(in, in_idx, in_buf, in_idx_buf, out_buf,
                               out_idx_buf, out, out_idx, counter, histogram,
...
unsigned calc_grid_dim(int batch_size, IdxT len, int sm_cnt) {
  ...
  cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks,
      radix_kernel<T, IdxT, BitsPerPass, BlockSize, false, false>, BlockSize,
      0);
...
__device__ void
filter_and_histogram_for_one_block(const T *in_buf, const IdxT *in_idx_buf,
                                   T *out_buf, IdxT *out_idx_buf, T *out,
                                   IdxT *out_idx, Counter<T, IdxT> *counter,
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize,
          bool store_out, bool is_vectorized>
__device__ void radix_topk_one_block_func(const T *in, const IdxT *in_idx,
                                          const IdxT len, const IdxT k, T *out,
                                          IdxT *out_idx, const bool select_min,
                                          T *buf1, IdxT *idx_buf1, T *buf2,
                                          IdxT *idx_buf2) {
  if (len <= k) {
    for (int index = threadIdx.x; index < len; index += BlockSize) {
      if constexpr (store_out) {
  ...
  for (int pass = 0; pass < num_passes; ++pass) {
    ...
    if (counter.len == counter.k || pass == num_passes - 1) {
      last_filter<T, IdxT, BitsPerPass, store_out>(
          ...
          out_idx, current_len, k, &counter, select_min, pass);
      break;
    }
  } // end for pass
} // end kernel
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize,
          bool store_out, bool is_vectorized>
__global__ void
radix_topk_one_block_kernel(const T *in, const IdxT *in_idx, const IdxT len,
                            const IdxT k, T *out, IdxT *out_idx,
                            const bool select_min, T *buf1, IdxT *idx_buf1,
                            T *buf2, IdxT *idx_buf2, IdxT *lengths) {
  ...
  radix_topk_one_block_func<T, IdxT, BitsPerPass, BlockSize, store_out, is_vectorized>(
      in, in_idx, actual_len, k, out, out_idx, select_min, buf1, idx_buf1, buf2,
      idx_buf2);
} // end kernel
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize>
void standalone_radix_topk_(void *buf, size_t &buf_size, const T *in,
                            const IdxT *in_idx, int batch_size, IdxT len,
                            IdxT k, T *out, IdxT *out_idx, bool select_min,
                            bool fused_last_filter, unsigned grid_dim,
  ...
  auto kernel =
      air_topk::radix_kernel<T, IdxT, BitsPerPass, BlockSize, false, true>;

  for (int pass = 0; pass < num_passes; ++pass) {
    air_topk::set_buf_pointers(in, in_idx, buf1, idx_buf1, buf2, idx_buf2, pass,
                               in_buf, in_idx_buf, out_buf, out_idx_buf);

    if (fused_last_filter && pass == num_passes - 1 && out != nullptr) {
      kernel =
          air_topk::radix_kernel<T, IdxT, BitsPerPass, BlockSize, true, true>;
    } else if (fused_last_filter && pass == num_passes - 1 && out == nullptr) {
      kernel =
          air_topk::radix_kernel<T, IdxT, BitsPerPass, BlockSize, true, false>;
    } else if (out == nullptr) {
      kernel =
          air_topk::radix_kernel<T, IdxT, BitsPerPass, BlockSize, false, false>;
    }

    kernel<<<blocks, BlockSize, 0, stream>>>(
        in, in_idx, in_buf, in_idx_buf, out_buf, out_idx_buf, out, out_idx,
        counters, histograms, len, k, select_min, pass, lengths);
  }

  if (!fused_last_filter) {
    if (out != nullptr) {
      air_topk::last_filter_kernel<T, IdxT, BitsPerPass, true>
          <<<blocks, BlockSize, 0, stream>>>(in, in_idx, out_buf, out_idx_buf,
                                             out, out_idx, len, k, counters,
                                             select_min);
    } else {
      air_topk::last_filter_kernel<T, IdxT, BitsPerPass, false>
          <<<blocks, BlockSize, 0, stream>>>(in, in_idx, out_buf, out_idx_buf,
                                             out, out_idx, len, k, counters,
                                             select_min);
    }
...
template <typename T, typename IdxT, int BitsPerPass, int BlockSize>
void standalone_radix_topk_one_block_(void *buf, size_t &buf_size, const T *in,
                                      const IdxT *in_idx, int batch_size,
  ...
  if (out != nullptr) {
      ...
      <<<batch_size, BlockSize, 0, stream>>>(in, in_idx, len, k, out, out_idx,
                                             select_min, buf1, idx_buf1, buf2,
                                             idx_buf2, lengths);
  } else {
      air_topk::radix_topk_one_block_kernel<T, IdxT, BitsPerPass, BlockSize,
                                          false, true>
        <<<batch_size, BlockSize, 0, stream>>>(in, in_idx, len, k, out, out_idx,
                                               select_min, buf1, idx_buf1, buf2,
                                               idx_buf2, lengths);
...
template <typename T, typename idxT>
void standalone_air_topk(void *buf, size_t &buf_size, const T *in,
  ...
  if (len <= single_block_dim * items_per_thread || is_prefill) {
    standalone_radix_topk_one_block_<T, idxT, 11, single_block_dim>(
        ...
        out, out_idx, !greater, stream, lengths);
  } else {
    int sm_cnt;
    {
      int dev;
    ...
    unsigned grid_dim = air_topk::calc_grid_dim<T, idxT, 11, multi_block_dim>(
    ...
    if (grid_dim == 1) {
      ...
      standalone_radix_topk_<T, idxT, 11, multi_block_dim>(
          buf, buf_size, in, static_cast<idxT *>(nullptr), batch_size, len, k,
          out, out_idx, !greater, fused_last_filter, grid_dim, stream, lengths);
    }
  }
}
} // namespace nv
