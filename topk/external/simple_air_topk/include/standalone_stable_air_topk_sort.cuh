...
namespace air_topk_stable_with_sort {
using WideT = float4;
constexpr int VECTORIZED_READ_SIZE = 16;
constexpr int WARP_SIZE = 32;
constexpr unsigned FULL_WARP_MASK = 0xffffffff;
constexpr int MAX_K_FOR_SINGLE_BLOCK = 2048;

template <typename IdxT> struct ComputeOffset {
  __host__ __device__ explicit ComputeOffset(const IdxT &cols) : cols_(cols) {}

  __host__ __device__ IdxT operator()(const IdxT &x) const { return cols_ * x; }
  IdxT cols_;
};
...
__global__ void radix_topk_one_block_kernel(const T *in, const IdxT *in_idx,
  ...
  int constexpr ITEMS_PER_THREAD_SORT = MAX_K_FOR_SINGLE_BLOCK / BlockSize;
  using BlockRadixSort =
      cub::BlockRadixSort<T, BlockSize, ITEMS_PER_THREAD_SORT, IdxT>;
  ...
  // The topK logits.
  T reg_out_values[ITEMS_PER_THREAD_SORT];
  // The topK indices.
  IdxT reg_out_indices[ITEMS_PER_THREAD_SORT];

// Load from shared memory.
#pragma unroll
  for (int ii = 0; ii < ITEMS_PER_THREAD_SORT; ++ii) {
    reg_out_values[ii] = out[ii * BlockSize + threadIdx.x];
    reg_out_indices[ii] = out_idx[ii * BlockSize + threadIdx.x];
  }
  BlockRadixSort(temp_storage)
      .SortDescendingBlockedToStriped(reg_out_values, reg_out_indices);
  for (int ii = 0; ii < ITEMS_PER_THREAD_SORT; ++ii) {
    out[ii * BlockSize + threadIdx.x] = reg_out_values[ii];
    out_idx[ii * BlockSize + threadIdx.x] = reg_out_indices[ii];
  }
}
} // namespace air_topk_stable_with_sort
...
void standalone_stable_radix_topk_(void *buf, size_t &buf_size, const T *in,
                                   ...
                                   bool sorted = false) {
  static_assert(air_topk_stable_with_sort::calc_num_passes<T, BitsPerPass>() >
                1);
  constexpr int num_buckets =
      air_topk_stable_with_sort::calc_num_buckets<BitsPerPass>();

  air_topk_stable_with_sort::Counter<T, IdxT> *counters = nullptr;
  ...
  air_topk_stable_with_sort::ComputeOffset<IdxT> computeoffset(k);
  thrust::counting_iterator<IdxT> counting_iter(0);
  thrust::transform_iterator<air_topk_stable_with_sort::ComputeOffset<IdxT>,
                             thrust::counting_iterator<IdxT>>
      transform_iter(counting_iter, computeoffset);
  cub::DeviceSegmentedSort::SortPairs(
      NULL, temp_storage_bytes, out_idx, out_idx, out, out, k * batch_size,
      batch_size, transform_iter, transform_iter + 1, stream);

  if (sorted) {
    if (select_min) {
      cub::DeviceSegmentedSort::StableSortPairs(
          NULL, temp_storage_bytes_sort, out, out, out_idx, out_idx,
          k * batch_size, batch_size, transform_iter, transform_iter + 1,
          stream);
    } else {
  ...
  {
    IdxT len_candidates = air_topk_stable_with_sort::calc_buf_len<T>(len);
  ...
  constexpr int num_passes =
      air_topk_stable_with_sort::calc_num_passes<T, BitsPerPass>();

  auto kernel = air_topk_stable_with_sort::radix_kernel<T, IdxT, BitsPerPass,
                                                        BlockSize, false, true>;

  for (int pass = 0; pass < num_passes; ++pass) {
    air_topk_stable_with_sort::set_buf_pointers(
        in, in_idx, buf1, idx_buf1, buf2, idx_buf2, pass, in_buf, in_idx_buf,
        out_buf, out_idx_buf);

    if (fused_last_filter && pass == num_passes - 1) {
      kernel = air_topk_stable_with_sort::radix_kernel<T, IdxT, BitsPerPass,
                                                       BlockSize, true, true>;
    }

    kernel<<<blocks, BlockSize, 0, stream>>>(
        in, in_idx, in_buf, in_idx_buf, out_buf, out_idx_buf, topk_out,
        topk_out_idx, counters, histograms, len, k, select_min, pass);
  }

  if (!fused_last_filter) {
    air_topk_stable_with_sort::last_filter_kernel<T, IdxT, BitsPerPass, true>
        <<<blocks, BlockSize, 0, stream>>>(in, in_idx, out_buf, out_idx_buf,
                                           topk_out, topk_out_idx, len, k,
                                           counters, select_min);
  }
  ...
  cub::DeviceSegmentedSort::SortPairs(
      sort_temp_storage, temp_storage_bytes, topk_out_idx, idx_sort_out_idx,
      topk_out, idx_sort_out, k * batch_size, batch_size, transform_iter,
      transform_iter + 1, stream);
  if (sorted) {
    if (select_min) {
      cub::DeviceSegmentedSort::StableSortPairs(
          sort_temp_storage, temp_storage_bytes, sort_in, out, sort_in_idx,
          out_idx, k * batch_size, batch_size, transform_iter,
          transform_iter + 1, stream);
    } else {
      cub::DeviceSegmentedSort::StableSortPairsDescending(
          sort_temp_storage, temp_storage_bytes, sort_in, out, sort_in_idx,
...
void standalone_stable_radix_topk_one_block_(
    void *buf, size_t &buf_size, const T *in, const IdxT *in_idx,
    int batch_size, IdxT len, IdxT k, T *out, IdxT *out_idx, bool select_min,
  ...
  char *bufs = nullptr;
  const IdxT buf_len =
      air_topk_stable_with_sort::calc_buf_len<T, IdxT, unsigned>(len);
  ...
  air_topk_stable_with_sort::radix_topk_one_block_kernel<T, IdxT, BitsPerPass,
                                                         BlockSize, true>
...
void standalone_stable_radix_11bits_sort(void *buf, size_t &buf_size,
                                         const T *in, int batch_size, idxT len,
                                         idxT k, T *out, idxT *out_idx,
                                         bool greater, cudaStream_t stream = 0,
  ...
  if (len <= block_dim * items_per_thread) {
    standalone_stable_radix_topk_one_block_<T, idxT, 11, block_dim>(
        ...
        out, out_idx, !greater, stream, sorted);
  } else {
    int sm_cnt;
    {
      int dev;
    ...
    unsigned grid_dim =
        air_topk_stable_with_sort::calc_grid_dim<T, idxT, 11, block_dim>(
    ...
    if (grid_dim == 1) {
      ...
      standalone_stable_radix_topk_<T, idxT, 11, block_dim>(
          buf, buf_size, in, static_cast<idxT *>(nullptr), batch_size, len, k,
          out, out_idx, !greater, fused_last_filter, grid_dim, stream, sorted);
    }
  }
}
} // namespace nv
