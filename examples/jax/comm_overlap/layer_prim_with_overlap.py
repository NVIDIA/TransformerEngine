...
if args.layer_type is layernorm_mlp:
    ...
    kernel_2 = jax.random.normal(kernel_2_rng, (args.activation_size, args.hidden_size),
                                 dtype=jnp.bfloat16)
...
with te.fp8_autocast(enabled=fp8_recipe is not None, fp8_recipe=fp8_recipe):
    fwd_bwd_serial = jax.jit(jax.value_and_grad(partial(_eval_layer_serial, args.layer_type),
                                                argnums=range(7)))
...
with mesh, global_shard_guard(mesh_resource), te.fp8_autocast(
    ...
    comm_overlaps = [
        CommOverlapHelperSet(fprop=fprop_1_overlap)
    ]
    if args.layer_type is layernorm_mlp:
...
if args.check_result:
    ...
    if myrank == 0:
        print(f"[{myrank}|{numranks}] Output: serial = {output_serial} | sharded = {output_sharded}")
