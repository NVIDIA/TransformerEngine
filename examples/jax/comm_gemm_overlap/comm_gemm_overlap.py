...
overlap_config = initialize_comm_overlap(
    lhs_shape if comm_type == tex.CommOverlapType.AG else output_shape,
    dtype,
    mesh,
    "tp",
    comm_type,
...
with te.sharding.global_shard_guard(mesh_resource):
    # NOTE: This call is already JIT decorated
    outputs = collective_gemm_impl(
        lhs,
        rhs_no_fsdp,
        comm_overlap_config=overlap_config,
    )
    # NOTE: Reduce-scattered output is in the auxiliary output buffer
    output = outputs[0] if comm_type == tex.CommOverlapType.AG else outputs[-1]
