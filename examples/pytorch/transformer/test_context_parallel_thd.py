...
def reconstruct_cp_logits(
    cp2_rank0_logits, cp2_rank1_logits, cp1_baseline_logits, cu_seqlens_padded, cp_size=2
):
    """
    ...
    logger.info(
        f"  Actual tokens - rank0: {cp2_rank0_logits.shape[0]}, rank1: {cp2_rank1_logits.shape[0]}"
    )

    if len(rank0_indices) != cp2_rank0_logits.shape[0]:
        logger.error(
            f"Rank 0 size mismatch: expected {len(rank0_indices)}, got {cp2_rank0_logits.shape[0]}"
        )
        return None

    if len(rank1_indices) != cp2_rank1_logits.shape[0]:
        logger.error(
            f"Rank 1 size mismatch: expected {len(rank1_indices)}, got {cp2_rank1_logits.shape[0]}"
        )
        return None
...
def test_data_loading(load_test_data):
    ...
    assert (
        cp2_rank0_logits.dim() == 2
    ), f"CP=2 rank 0 logits should be 2D, got {cp2_rank0_logits.dim()}D"
    assert (
        cp2_rank1_logits.dim() == 2
    ), f"CP=2 rank 1 logits should be 2D, got {cp2_rank1_logits.dim()}D"

    vocab_size = cp1_logits.shape[1]
    assert (
        cp2_rank0_logits.shape[1] == vocab_size
    ), f"Vocab size mismatch: CP=1 has {vocab_size}, CP=2 rank 0 has {cp2_rank0_logits.shape[1]}"
    assert (
        cp2_rank1_logits.shape[1] == vocab_size
    ), f"Vocab size mismatch: CP=1 has {vocab_size}, CP=2 rank 1 has {cp2_rank1_logits.shape[1]}"
...
def test_logits_reconstruction(load_test_data):
    ...
    reconstructed_cp2_logits = reconstruct_cp_logits(
        cp2_rank0_logits, cp2_rank1_logits, cp1_logits, cu_seqlens_padded
    )
...
def test_cp2_vs_cp1_logits_accuracy(load_test_data):
    ...
    logger.info(
        f"Logits accuracy test passed: {actual_accuracy:.1f}% within"
