...
class Batch(object):
    ...
    def remove_finished(self):
        self.finished = torch.where(self.gen_lens - self.expected_gen_lens < 0, False, True).to(
...
def test_paged_attn(dtype, model, qkv_format, is_paged, backend, is_cuda_graph):
    ...
    context_lens = torch.randint(
        1, max_context_len, [total_requests], dtype=torch.int32, device="cpu"
    )
    # generation lengths in Exponential distribution
    gen_dist = Exponential(1 / max_gen_len)
    gen_lens = gen_dist.sample((total_requests,))
    gen_lens = torch.where(gen_lens > max_gen_len, max_gen_len, gen_lens).to(
    ...
    while True:
        ...
        inference_params.step_dict = OrderedDict(
            zip(cur.seq_ids.tolist(), cur.step_lens_q.tolist())
        )
