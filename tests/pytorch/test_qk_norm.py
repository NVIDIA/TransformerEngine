...
def test_qk_norm_functionality(use_qk_norm, attention_type, qk_norm_eps) -> None:
    ...
    if attention_type == "cross":
        encoder_output = torch.randn(seq_len, batch_size, hidden_size, device="cuda", dtype=torch.float32)
    else:
        encoder_output = None
    ...
    assert output.shape == (seq_len, batch_size, hidden_size), f"Output shape mismatch: {output.shape}"
    ...
    if attention_type == "self":
        ...
        assert output_with_rope.shape == (seq_len, batch_size, hidden_size), "Output shape with RoPE mismatch"
...
def test_qk_norm_transformer_layer_output_difference() -> None:
    ...
    assert output_with_norm.shape == (seq_len, batch_size, hidden_size), \
