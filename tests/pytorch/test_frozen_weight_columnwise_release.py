...
def _make_grouped_tensor_module(fp8_recipe):
    """Frozen GroupedLinear pinned to the native grouped-tensor path."""
    with torch.no_grad(), quantized_model_init(enabled=True, recipe=fp8_recipe):
        module = GroupedLinear(
            NUM_GEMMS,
            IN_FEATURES,
            OUT_FEATURES,
...
def _run_grouped_tensor_fwd_bwd(module, fp8_recipe):
    """One fwd+bwd step on the grouped-tensor path, which requires CUDA m_splits."""
    inp = torch.randn(
        NUM_GEMMS * TOKENS_PER_GEMM,
        IN_FEATURES,
        device="cuda",
    ...
    m_splits = torch.tensor([TOKENS_PER_GEMM] * NUM_GEMMS, dtype=torch.int64, device="cuda")
    with autocast(enabled=True, recipe=fp8_recipe):
...
def test_grouped_tensor_path_release(monkeypatch):
    ...
    monkeypatch.setenv("NVTE_RELEASE_FROZEN_WEIGHT_COLUMNWISE", "1")
    module = _make_grouped_tensor_module(fp8_recipe)
    weights = _quantized_weights(module)
    assert weights, "expected quantized weights"

    for _ in range(3):  # release -> rebuild -> release must be stable
        _run_grouped_tensor_fwd_bwd(module, fp8_recipe)
        for weight in weights:
            assert weight._columnwise_data is None, "columnwise copy should be released"


@pytest.mark.skipif(not fp8_block_scaling_available, reason=reason_for_no_fp8_block_scaling)
def test_grouped_tensor_path_release_preserves_numerics(monkeypatch):
    """dgrad on the grouped-tensor path is identical with and without the flag."""
    fp8_recipe = _make_block_scaling_recipe()
    if not is_module_grouped_tensor_path_supported(fp8_recipe, torch.bfloat16):
        pytest.skip("Native GroupedTensor GEMM is unavailable on this system.")
    ...
    monkeypatch.setenv("NVTE_RELEASE_FROZEN_WEIGHT_COLUMNWISE", "0")
    reset_rng_states()
    module_ref = _make_grouped_tensor_module(fp8_recipe)
    dgrad_ref = [_run_grouped_tensor_fwd_bwd(module_ref, fp8_recipe) for _ in range(2)]

    monkeypatch.setenv("NVTE_RELEASE_FROZEN_WEIGHT_COLUMNWISE", "1")
    reset_rng_states()
    module_rel = _make_grouped_tensor_module(fp8_recipe)
    dgrad_rel = [_run_grouped_tensor_fwd_bwd(module_rel, fp8_recipe) for _ in range(2)]
...
def test_grouped_tensor_path_call_site_bf16(monkeypatch):
    ...
    m_splits = torch.tensor([TOKENS_PER_GEMM] * NUM_GEMMS, dtype=torch.int64, device="cuda")
    module(inp, m_splits).float().pow(2).mean().backward()
    assert torch.isfinite(inp.grad).all(), "dgrad should be finite"
