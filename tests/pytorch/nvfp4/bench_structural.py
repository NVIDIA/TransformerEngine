...
for M in [16384, 65536, 131072]:                                                                                                                                                                           
    ...
    q1 = make_quantizer()                                                                                                                                                                                  
    bench(lambda: tex.group_quantize(x, q1, num_experts, equal_tensor),                                                                                                                                    
          f"[M={M}] graph_safe_equal_O1")                                                                                                                                                                  
    ...
    q2 = make_quantizer()
    bench(lambda: tex.group_quantize(x, q2, num_experts, unequal_tensor),                                                                                                                                  
          f"[M={M}] graph_safe_unequal_binary_search")
                                                                                                                                                                                                           
    # 3. non-graph-safe + linear scan (GetGroupIdx)                                                                                                                                                        
    q_list = [NVFP4Quantizer(rowwise=True, columnwise=True, with_rht=True, with_post_rht_amax=True)
              for _ in range(num_experts)]                                                                                                                                                                 
    bench(lambda: tex.split_quantize(x, equal_splits, q_list),
          f"[M={M}] non_graph_safe_linear_scan")                                                                                                                                                           
