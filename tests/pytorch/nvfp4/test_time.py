...
def main():
    ...
    print(f"# device={device} x_dtype={x_dtype} out_dtype={out_dtype} "
    ...
    print("setting,bz,seq,in,out,dtype,SR,RHT,Use2D,"
    ...
    for s in settings:
        ...
        if s["rht"] and x_dtype != torch.bfloat16:
            # RHT 仅支持 bf16：跳过
            print(f"{s['name']},{B},{S},{K},{N},{args.dtype},{s['sr']},{s['rht']},{s['use2d']},"
        ...
        x_quant_ms = cuda_timing_ms(_quant_x_once, warmup=args.warmup, iters=args.iters, label=f"{s['name']}_quantX")

        # -------- W 量化（冷态 vs 热态）---------
        # 冷态：等价第一步或者权重更新后首次量化
        def _quant_w_cold_once():
            _ = quantize_tensor(qw, W)
        w_quant_cold_ms = cuda_timing_ms(_quant_w_cold_once, warmup=args.warmup, iters=args.iters, label=f"{s['name']}_quantW_cold")
        ...
        w_quant_hot_ms = cuda_timing_ms(_quant_w_hot_once, warmup=args.warmup, iters=args.iters, label=f"{s['name']}_quantW_hot")
        ...
        qgemm_ms = cuda_timing_ms(_qgemm_once, warmup=args.warmup, iters=args.iters, label=f"{s['name']}_QGEMM")
