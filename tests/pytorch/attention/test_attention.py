...
def test_dpa_packed_input_validation():
    dpa = _make_dpa("bshd")
    qkv = torch.randn(
        _PACKED_B, _PACKED_S, 3, _PACKED_H, _PACKED_D, dtype=_PACKED_DTYPE, device="cuda"
    )
    kv = torch.randn(
        _PACKED_B, _PACKED_S, 2, _PACKED_H, _PACKED_D, dtype=_PACKED_DTYPE, device="cuda"
    )
    k = torch.randn(_PACKED_B, _PACKED_S, _PACKED_H, _PACKED_D, dtype=_PACKED_DTYPE, device="cuda")
...
def test_dpa_thd_qkv_layer_declared_no_detection(monkeypatch):
    ...
    torch.manual_seed(0)
    t = _PACKED_B * _PACKED_S
    qkv = torch.randn(
        t, 3, _PACKED_H, _PACKED_D, dtype=_PACKED_DTYPE, device="cuda", requires_grad=True
    )
    cu = _cu_seqlens()
    dpa = DotProductAttention(
        _PACKED_H, _PACKED_D, attention_dropout=0.0, qkv_format="thd", attn_mask_type="padding"
    )
    _attention_backends["backend_selection_requires_update"] = True
    ran_full = True
    try:
        out = dpa(
            ...
            cu_seqlens_q=cu,
            cu_seqlens_kv=cu,
            max_seqlen_q=_PACKED_S,
            max_seqlen_kv=_PACKED_S,
        )
        out.backward(torch.ones_like(out))
        assert qkv.grad is not None and qkv.grad.shape == qkv.shape
    except ValueError:
...
def test_mha_self_attention_packed_pass_through(monkeypatch, interleaved):
    ...
    mha = MultiheadAttention(
        ...
        params_dtype=_PACKED_DTYPE,
        device="cuda",
    )
    torch.manual_seed(1)
    x = torch.randn(
        _PACKED_S, _PACKED_B, hidden, dtype=_PACKED_DTYPE, device="cuda", requires_grad=True
    )
...
def test_mha_cross_attention_packed_kv_pass_through(monkeypatch, interleaved):
    ...
    enc = torch.randn(_PACKED_S, _PACKED_B, hidden, dtype=_PACKED_DTYPE, device="cuda")
...
def test_mha_gqa_falls_back_to_views(monkeypatch):
    ...
    assert out.shape == (_PACKED_S, _PACKED_B, hidden)
...
def test_mha_rope_falls_back_to_views(monkeypatch):
    ...
    rope = RotaryPositionEmbedding(_PACKED_D)(max_seq_len=_PACKED_S).to("cuda")
    torch.manual_seed(1)
    x = torch.randn(
        _PACKED_S, _PACKED_B, hidden, dtype=_PACKED_DTYPE, device="cuda", requires_grad=True
    )
