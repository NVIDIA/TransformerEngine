...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([0, -1, 4], "must be nondecreasing"),
        ([0, 3, 2, 4], "must be nondecreasing"),
        ([0, 2, 5], "must end at the flattened token count 4"),
    ],
)
def test_gdn_rejects_malformed_thd_cu_seqlens(offsets, message):
    """Malformed THD offsets fail deterministically before entering the GDN kernel."""
    q, k, v, g, beta = (tensor.squeeze(0) for tensor in _inputs(1, 4, 1, 1))
...
def test_gdn_rejects_mismatched_thd_cu_seqlens():
    """Q and KV offsets must describe the same recurrent-state partitioning."""
    q, k, v, g, beta = (tensor.squeeze(0) for tensor in _inputs(1, 4, 1, 1))
    cu_seqlens_q = torch.tensor([0, 2, 4], device="cuda", dtype=torch.int32)
    cu_seqlens_kv = torch.tensor([0, 1, 4], device="cuda", dtype=torch.int32)
    attention = DotProductAttention(
        num_attention_heads=1,
        kv_channels=64,
        qkv_format="thd",
    ...
    with pytest.raises(ValueError, match="contain identical offsets"):
        attention(
            q,
            k,
            v,
...
def test_gdn_revalidates_mutated_thd_cu_seqlens(monkeypatch):
    """A cached validation result is invalidated by an in-place offsets update."""
    q, k, v, g, beta = (tensor.squeeze(0) for tensor in _inputs(1, 4, 1, 1))
    ...
    def fake_gated_delta_net(q_thd, key, value, gate_g, gate_beta, offsets, **kwargs):
        del q_thd, key, gate_g, gate_beta, offsets, kwargs
        return value, None

    gdn_module = importlib.import_module(
        "transformer_engine.pytorch.attention.dot_product_attention.gdn"
    )
    monkeypatch.setattr(gdn_module, "_import_gated_delta_net", lambda: fake_gated_delta_net)
