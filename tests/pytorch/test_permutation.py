...
def dtype_tols(te_dtype: constants.DType) -> Dict[str, float]:
    """Estimated tolerances for a datatype

    Based on tolerances for torch.testing.assert_close.

    """
    if te_dtype == constants.DType.kFloat32:
        return dict(rtol=1.0e-6, atol=1.0e-6)
    if te_dtype == constants.DType.kFloat16:
        return dict(rtol=3.0e-3, atol=1.0e-5)
    if te_dtype == constants.DType.kBFloat16:
        return dict(rtol=2.0e-2, atol=1.0e-5)
    if te_dtype == constants.DType.kFloat8E5M2 or te_dtype == constants.DType.kFloat8E4M3:
...
def _test_permutation_index_map(
    ...
    if te_dtype == constants.DType.kFloat32:
        dtype = torch.float32
    elif te_dtype == constants.DType.kFloat16:
        dtype = torch.float16
    elif te_dtype == constants.DType.kBFloat16:
...
# TE tensor dtypes
_te_dtypes: List[constants.DType] = [constants.DType.kFloat32, constants.DType.kFloat16]
if te.is_bf16_available():
    _te_dtypes.append(constants.DType.kBFloat16)
...
@pytest.mark.parametrize("te_dtype", [constants.DType.kFloat8E4M3, constants.DType.kFloat8E5M2])
@pytest.mark.parametrize("num_tokens", [4096])
@pytest.mark.parametrize("num_expert", [7, 16])
@pytest.mark.parametrize("hidden_size", [4096])
@pytest.mark.parametrize("topK", [2, 5])
...
def test_permutation_single_case():
    ...
    # te_dtype = constants.DType.kFloat32
    # te_dtype = constants.DType.kFloat16
    te_dtype = constants.DType.kBFloat16
