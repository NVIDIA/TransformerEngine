...
def parse_args(argv=None, namespace=None):
    ...
    parser.add_argument("--scale", type=float, default=1e-2,
                        help="Set scaling factor for input and weight tensors.")
    parser.add_argument("--no-atomic-pair", action="store_true", default=False,
...
def main(opts):
    ...
    ) if opts.p2p or opts.comm_type == tex.NVTE_Comm_Overlap_Type.AG else tex.UbufCommOverlap(
        sample_buffer,
        world_rank,
        world_size,
        local_rank,
    ...
    inp = torch.mul(
        torch.rand(local_inp_shape, dtype=torch.float32, device='cuda'), opts.scale)
    kernel_t = torch.mul(
    ...
    if opts.fp8:
        ...
        dist_print(f"GEMM1_INPUT amax = {inp_amax.item()} " + \
                   f"| GEMM1_WEIGHT amax = {ker_amax.item()} " + \
                   f"| GEMM1_OUTPUT amax = {ref_amax.item()}", src=0, section=True)
    ...
    if opts.check_numerics:
        ...
        if opts.fp8:
            ...
            fp8_meta_info = f"amax_reference  = {fp8_meta.amax_history[1][:3].tolist()}\n" + \
        ...
        if not error_below_tol:
            ...
            numerics_info = "NUMERICAL CHECK FAILED: " + \
