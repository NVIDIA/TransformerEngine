...
def _main(opts):
    ...
    if opts.comm_type == tex.CommOverlapType.RS:
        if opts.bulk_overlap:
            ub_algo = tex.CommOverlapAlgo.BULK_RS
        elif opts.p2p:
            ub_algo = (
                tex.CommOverlapAlgo.ATOMIC_RS_P2P
                if opts.atomic
        ...
        if opts.bulk_overlap:
            ...
            ub_algo = (
                tex.CommOverlapAlgo.ATOMIC_AG_P2P
                if opts.atomic
                else tex.CommOverlapAlgo.SPLIT_AG_P2P
            )
    else:
    ...
    if opts.comm_type == tex.CommOverlapType.AG:
        if opts.bulk_overlap:
            ub_obj.copy_input_to_ubuf(bulk_inp, tex.CommOverlapBuffer.LOCAL)
            gemm_inp = inp
        else:
            ub_obj.copy_input_to_ubuf(inp_fp8 if opts.fp8 else inp, tex.CommOverlapBuffer.LOCAL)
            gemm_inp = ub_obj.get_ubuf_output(tex.CommOverlapBuffer.GLOBAL)
        ubuf_out = None
        rs_out = None
        if ub_obj2 is not None:
            ubuf_out2 = ub_obj2.get_ubuf_output(tex.CommOverlapBuffer.GLOBAL)
            rs_out2 = torch.empty(
    ...
    def _fp8_gemm2(gemm1_out):
        ...
        return tex.fp8_gemm(
            ...
            ub_algo=(
                tex.CommOverlapAlgo.ATOMIC_RS_P2P
                if opts.atomic_rs_p2p
                else tex.CommOverlapAlgo.ATOMIC_RS
            ),
            ub=ub_obj2,
    ...
    if opts.check_numerics:
        ...
        if opts.bulk_overlap:
            ...
            if opts.comm_type == tex.CommOverlapType.AG:
                ...
                test_out = ub_obj.get_ubuf_output(tex.CommOverlapBuffer.GLOBAL)
            else:
                # Bulk overlap RS output needs to be gathered
                out_local = ub_obj.get_ubuf_output(tex.CommOverlapBuffer.LOCAL)
