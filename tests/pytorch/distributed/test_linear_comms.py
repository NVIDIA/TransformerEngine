...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run a linear layer with Transformer Engine, CUDA Graphs, and Tensor Parallelism")
    ...
    parser.add_argument('--cuda_graph', action='store_true', help='Use CUDA Graphs (pass this flag to enable)')
    ...
    parser.add_argument('--sym_type', type=str, default="multimem_all_reduce", help='sym type: one_shot, two_shot, multimem_all_reduce, ub_custom')
    parser.add_argument('--iterations', type=int, default=1000, help='Number of iterations')
    parser.add_argument('--tp_size', type=int, default=None, help='Tensor parallelism size (defaults to number of GPUs)')
    ...
    tp_size = args.tp_size if args.tp_size is not None else int(os.environ.get('WORLD_SIZE', num_devices))
    ...
    ub_cfgs = {
            "proj_fprop": {"method": "pipeline","num_splits":1,"is_reduce_scatter":True,"num_sm":32,"atomic_gemm":False,"aggregate":False,
                           "cga_size":4,"set_sm_margin":False,"fp8_buf":False,"use_ce":False}
            }
    ...
    if (args.comm_type=='sym' and os.environ.get("NVTE_USE_UB_FOR_UBNEXT")) or args.comm_type=='ub':
    ...
    if args.comm_type == "sym" and args.validate:
        ...
        allocator = te.cpp_extensions.symm_allocator.SymmAllocator(pool_size, torch.device(device),torch.distributed.group.WORLD)
    ...
    if args.comm_type == "sym" and args.validate:
        ...
        if myrank == 0:
            ...
            print(f"Tested sizes: {len(all_sizes)} different tensor sizes from {all_sizes[0]} to {all_sizes[-1]} elements")
            print(f"Total elements tested: {total_elements_tested}")
            print(f"Total different elements: {total_different_elements}")
            print(f"Overall error rate: {(total_different_elements / total_elements_tested) * 100:.6f}%")
            ...
            if total_different_elements > 0 or overall_max_delta > 0:
                ...
                for i, size in enumerate(all_sizes):
                    ...
                    print(f"  Size {size:7d}: {all_num_different[i]:6d}/{all_total_elements[i]:7d} different ({error_rate:6.3f}%), max_delta: {all_max_deltas[i]:.6f}")
            print("================================\n")
    ...
    for logbatch in range(int(math.log2(args.batch_size)) + 1):
        batch = 2**logbatch
        if args.comm_type=='ub' and batch < tp_size: batch = tp_size
        # Create input tensor
        inp = torch.randn(batch, int(args.in_features/tp_size), device=device, dtype=torch.bfloat16)
        # Warm-up run
        modelseq(inp)
        modelpar(inp)
        torch.cuda.synchronize()
        ...
        if myrank == 0:
            print(f"Batch{batch},{(seq_elapsed/ args.iterations) * 1e6:.4f}us,{(par_elapsed/ args.iterations) * 1e6:.4f} us,{(nccl_elapsed/ args.iterations) * 1e6:.4f}")
        if args.cuda_graph:
