...
def vanilla_test():
    ...
    for i in range(iterations):
        ...
        with torch.cuda.nvtx.range(f"iteration_{i}"):
            ...
            weight = (
                torch.empty((hidden_size, vocab_size), dtype=dtype, device="cuda")
                .uniform_(-0.5, 0.5)
                .requires_grad_()
            )
            ...
            if enabled_fileds["forward"]["Torch"]:
                ...
                print(
                    "[INFO]: Forward pass: Torch implementation peak memory:"
                    f" {torch_max_memory:.2f} MB"
                )
                print(
                    "[INFO]: Forward pass: Torch implementation time:"
                    f" {start_event.elapsed_time(end_event):.2f} ms"
                )
            ...
            if enabled_fileds["forward"]["Kernel"]:
                ...
                print(
                    "[INFO]: Forward pass: Kernel implementation peak memory:"
                    f" {kernel_max_memory:.2f} MB"
                )
                print(
                    "[INFO]: Forward pass: Kernel implementation time:"
                    f" {start_event.elapsed_time(end_event):.2f} ms"
                )

                if enabled_fileds["forward"]["Torch"]:
                    torch.testing.assert_close(
                        torch_logprobs, kernel_logprobs, atol=1e-3, rtol=1e-3
                    )
            ...
            if enabled_fileds["backward"]["Torch"] or enabled_fileds["backward"]["Kernel"]:
                g_entropy = torch.empty((num_tokens,), dtype=dtype, device="cuda").uniform_(
                    -0.5, 0.5
                )
                g_logprobs = torch.empty((), dtype=dtype, device="cuda").uniform_(-1, 1)
            ...
            if enabled_fileds["backward"]["Torch"]:
                ...
                print(
                    "[INFO]: Backward pass: torch implementation peak memory:"
                    f" {torch_backward_max_memory:.2f} MB"
                )
                print(
                    "[INFO]: Backward pass: torch gradient time:"
                    f" {start_event.elapsed_time(end_event):.2f} ms"
                )
            ...
            if enabled_fileds["backward"]["Kernel"]:
                ...
                print(
                    "[INFO]: Backward pass: kernel implementation peak memory:"
                    f" {kernel_backward_max_memory:.2f} MB"
                )
                print(
                    "[INFO]: Backward pass: kernel gradient time:"
                    f" {start_event.elapsed_time(end_event):.2f} ms"
                )

                if enabled_fileds["backward"]["Torch"]:
                    torch.testing.assert_close(
                        d_torch_hidden, d_kernel_hidden, atol=1e-2, rtol=1e-3
                    )
                    torch.testing.assert_close(
                        d_torch_weight, d_kernel_weight, atol=1e-2, rtol=1e-3
                    )
