...
@pytest.mark.skipif(reason='Skip at this moment, since this is ready yet.'\
                           'Will have another PR to perfectize UTs.')
class TestCustomPartitioningOpsGenerator:
    ...
    def test_softmax(self, mesh_shape, mesh_names, sharding_type, collective_ref, softmax_type):
        ...
        tokens = jnp.concatenate(
            (jnp.ones((batch_size, valid_len),
                      dtype=ops.dtype), jnp.zeros((batch_size, pad_len), dtype=ops.dtype)),
            axis=-1)
    ...
    def test_self_fused_attn(self, mesh_shape, mesh_names, sharding_type, collective_ref,
        ...
        batch_size, seq_len, num_heads, head_dim = ops.qkv_shape
        ops.check_fused_attn_inputs(seq_len, seq_len, head_dim, ops.pad_ratio, ops.dropout_prob,
                                    attn_bias_type, attn_mask_type, backend)
