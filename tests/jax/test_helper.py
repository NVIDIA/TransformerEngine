...
class TestFP8Helper(unittest.TestCase):
    ...
    def test_update_fp8_metas(self):
        ...
        fp8_scale_array1 = get_fp8_scale(fp8_max_array, fp8_amax_array1,
                                         jnp.ones(meta_shape))
        fp8_scale_inv_array1 = 1 / fp8_scale_array1
        fp8_amax_array2 = jax.random.uniform(key2, shape=meta_shape)
        fp8_scale_array2 = get_fp8_scale(fp8_max_array, fp8_amax_array2,
                                         jnp.ones(meta_shape))
        ...
        np.testing.assert_allclose(
            state_array[0 * meta_per_gemm + scale_shift], fp8_scale_array1)
        np.testing.assert_allclose(
            state_array[0 * meta_per_gemm + scale_inv_shift],
            fp8_scale_inv_array1)
        np.testing.assert_allclose(
            state_array[1 * meta_per_gemm + scale_shift], fp8_scale_array2)
        np.testing.assert_allclose(
            state_array[1 * meta_per_gemm + scale_inv_shift],
    ...
    def test_generate_fp8_max_array(self):
        ...
        for fp8_format in FP8Format:
            ...
            np.testing.assert_allclose(
                get_ref(fp8_format),
                FP8Helper.generate_fp8_max_array(num_of_meta))
            FP8Helper.finalize()
    ...
    def test_update_collections(self):
        ...
        original_state = flax.core.frozen_dict.FrozenDict({
            "test1":
            original_val,
            "test2":
            original_val,
        ...
        updated_state = FP8Helper.update_collections({"test1": updated_val},
                                                     original_state)
    ...
    def test_fp8_autocast(self):
        ...
        with self.assertRaises(AssertionError):
            with fp8_autocast(enabled=True,
                              fp8_recipe=DelayedScaling(amax_history_len=2)):
                pass
