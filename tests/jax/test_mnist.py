...
def get_datasets():
    ...
    ds_builder = tfds.builder('mnist',
                              data_dir="/tmp/tensorflow-datasets/downloads")
...
def train_step(state, others, batch, use_fp8_dense):
    ...
    def loss_fn(collections):
        logits = MLPNN(use_fp8_dense=use_fp8_dense).apply(
            collections, batch['image'])
...
def train_epoch(state, variables, train_ds, batch_size, rng, use_fp8_dense):
    ...
    for idx, perm in enumerate(perms):
        ...
        state, metrics, grads = train_step(state, variables, batch,
                                           use_fp8_dense)
...
def eval_step(variables, batch, use_fp8_dense):
    logits = MLPNN(use_fp8_dense=use_fp8_dense).apply(variables,
                                                      batch['image'])
...
class TestMnist(unittest.TestCase):
    ...
    def _mnist_baseline_runner(self):
        ...
        state, variables = create_train_state(init_rng, self.learning_rate,
                                              self.momentum, False)
        del init_rng

        _, accuracy = self._train_model(state, variables, self.num_epochs, rng,
                                        False)
        return accuracy
    ...
    def _mnist_test_runner(self, fp8_format):
        ...
        _, test_accuracy = self._train_model(state, variables, self.num_epochs,
                                             rng, True)
    ...
    def _train_model(self, state, variables, epochs, rng, use_fp8_dense):
        ...
        for _ in range(0, epochs):
            ...
            state, variables = train_epoch(state, variables, self.train_ds,
                                           self.batch_size, input_rng,
                                           use_fp8_dense)

            _, test_accuracy = eval_model(variables, self.test_ds,
                                          self.batch_size, use_fp8_dense)
