...
class TestFFICompatibility:
    ...
    @pytest.mark.skipif(os.getenv("NVTE_JAX_FFI_HLO_GENERATE", "0") != "1", reason="HLO generation not enabled")
        ...
        with autocast(enabled=True, recipe=NVFP4BlockScaling(), mesh_resource=MeshResource()):
            class Model(nn.Module):
                ...
                def __call__(self, x):
                    ...
                    mask1 = self.variable('collection', 'mask1', lambda: jax.random.bernoulli(jax.random.PRNGKey(0), shape=x.shape).astype(jnp.bfloat16)).value.astype(jnp.uint8)
                    x = softmax.softmax(x, mask=mask1, softmax_fusion_type=softmax.SoftmaxFusionType.SCALED_MASKED)
                    mask2 = self.variable('collection', 'mask2', lambda: (1.0 - jnp.tril(jnp.ones_like(x))).astype(jnp.bfloat16)).value.astype(jnp.uint8)
                    x = x.reshape((-1, 1, 32, 32))
                    x = softmax.softmax(x, mask=mask2, softmax_fusion_type=softmax.SoftmaxFusionType.SCALED_UPPER_TRIANG_MASKED)
                    return x
            ...
            def train_step(var_collect, x, grouped_kernel):
                ...
                x = grouped_dense(
                    ...
                    quantizer_set=QuantizerFactory.create_set(n_groups=1, fp8_recipe=Float8CurrentScaling(), quantize_meta_set=QuantizeMetaSet(QuantizeMeta(), QuantizeMeta(), QuantizeMeta())),
                )
                loss += jnp.mean(x)
    ...
    def test_ffi_compatibility(self, ffi_hlo_name):
        ...
        executable = client.compile_and_load(stablehlo_text.encode('utf-8'), executable_devices=jax.devices()[:1])
    ...
    def test_all_primitive_ffi_tested(self):
        ...
        for primitive_ffi_name, _ in transformer_engine_jax.registrations().items():
            if primitive_ffi_name not in tested_hlos and primitive_ffi_name not in KNOWN_MISSING_FFI_TESTS:
                unmatched_primitives.add(primitive_ffi_name)

        assert len(unmatched_primitives) == 0, f"The following primitives do not have FFI tests: {unmatched_primitives}"