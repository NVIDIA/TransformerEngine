...
def is_fp8_supported():
    ...
    Use tf for get the compute capability
    """
    gpu_devices = tf.config.list_physical_devices('GPU')
    details = tf.config.experimental.get_device_details(gpu_devices[0])
    capability = details.get('compute_capability', 'Unknown GPU')
  ...
  """
    ...
    """
        ...
        if self.fuse_qkv:
            ...
            query = projection(kernel_init=query_init, name='query')( \
                    (inputs_q / depth_scaling) if self.scale_attn_logits else inputs_q)
            key = projection(kernel_init=self.kernel_init,
                             name='key')(inputs_kv)
...
class EncoderLayer(nn.Module):
    ...
    def __call__(self, inputs, encoder_mask=None, deterministic=False):
        ...
        encoder_bias = rel_emb(inputs.shape[sequence_dim],
        ...
        # MLP block.
        residual = x
        y = LayerNorm(dtype=self.dtype, name='pre_mlp_layer_norm')(x)
        ...
        if self.output_layernorm:
            y = LayerNorm(dtype=self.dtype, name="output_layer_norm")(y)
        return y
...
class DecoderLayer(nn.Module):
    ...
    def __call__(self,
        ...
        # MLP block.
        residual = y
        z = LayerNorm(dtype=self.dtype, name='pre_mlp_layer_norm')(y)
        if self.apply_residual_connection_post_layernorm:
            residual = z
        z = MlpBlock(
            transpose_batch_sequence=self.transpose_batch_sequence,
        ...
        if self.output_layernorm:
            z = LayerNorm(dtype=self.dtype, name="output_layer_norm")(z)
