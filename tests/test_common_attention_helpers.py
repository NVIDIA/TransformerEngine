...
def test_shared_fp8_scaling_mode_policy():
    ...
    assert check_fp8_fused_attention_support(fp8, scaling_mode="delayed").supported
    assert check_fp8_fused_attention_support(fp8, scaling_mode="current").supported
    assert check_fp8_fused_attention_support(fp8, scaling_mode="mxfp8").supported
    ...
    assert not check_fp8_fused_attention_support(
        replace(fp8, cudnn_version=(9, 13, 9)), scaling_mode="current"
    ).supported
    assert not check_fp8_fused_attention_support(
        replace(fp8, cudnn_version=(9, 20, 9)), scaling_mode="mxfp8"
    ).supported
    assert not check_fp8_fused_attention_support(
        replace(fp8, cudnn_version=(9, 23, 0)), scaling_mode="mxfp8"
    ).supported
    assert check_fp8_fused_attention_support(
        replace(fp8, cudnn_version=(9, 23, 2)), scaling_mode="mxfp8"
    ).supported

    assert not check_fp8_fused_attention_support(
        replace(fp8, sm_arch=120), scaling_mode="delayed"
    ).supported


def test_shared_fp8_deterministic_backward_policy():
    ...
    assert not check_fp8_fused_attention_support(
        replace(fp8, cudnn_version=(9, 18, 9)), scaling_mode="delayed"
    ).supported
    assert check_fp8_fused_attention_support(
        replace(fp8, cudnn_version=(9, 19, 0)), scaling_mode="delayed"
    ).supported
