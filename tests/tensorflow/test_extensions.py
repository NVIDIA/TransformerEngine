...
class ExtensionsTest(test.TestCase):
  ...
  def testCastFp8(self):
    ...
    for fp8_dtype in [tex.DType.kFloat8E4M3, tex.DType.kFloat8E5M2]:
      ...
      y = tex.cast_from_fp8(x_fp8, scale_inv, fp8_dtype, TE_DType[x.dtype],
                            offset, stream_id)
  ...
  def testMatmulFp8(self):
    ...
    # We assume b is in transposed format (see above). So we transpose it back
    # to apply the ordinary row-major matmul.
    bt = tf.transpose(b)
  ...
  def testGeluForwardFp8(self):
    ...
    y_fp8 = tex.te_gelu(x, scale, fp8_dtype, amax, scale_inv, offset, stream_id)
    y = tex.cast_from_fp8(y_fp8, scale_inv, fp8_dtype, TE_DType[x.dtype],
                          offset, stream_id)
  ...
  def testGeluBackwardFp8(self):
    ...
    dbias, dgelu_c, dgelu_t = \
        tex.fp8_fused_cast_transpose_bgrad_dgelu(
  ...
  def testScaledMaskedSoftmaxFwd(self):
    ...
    # In NVTE, if the mask is true, the corresponding value is zero. Whereas, TF
    # does the opposite. In addition, NVTE requires the mask has the same num of
    # dims as the input.
