...
def train_step(dy, x_q, x_kv, x_mask, model, attn_type, use_fp8=False,
  ...
  with tf.GradientTape(persistent=True) as tape:
    ...
    with te.fp8_autocast(enabled=use_fp8, fp8_recipe=fp8_recipe):
      # The MHA won't apply the bias addition for the last projection but return
      # the bias. So, we conduct the bias addition here at the end.
...
class MultiHeadAttentionKeras(tf.keras.Model):
  def __init__(self, hidden_size, num_heads, attention_type, init_method):
    ...
    # Einsum symbols:
    # F=seq_q, T=seq_kv, B=batches, H=hidden_states, D=hidden_size, N=num_heads,
    # E=depth
    ...
    # The bias in the projection layer will be applied separately outside the
    # MHA. So, we disable the bias in the Einsum but handle the bias at the end.
  ...
  def __call__(self, q_input, mask=None, kv_input=None, training=None):
    if self.attention_type == 'self':
      ...
      # [F, B, N, 3 * E]
      qkv = tf.reshape(qkv, (*qkv.shape[:-1], self.num_heads, 3 * self.depth))
      # 3 * [F, B, N, E]
      ...
      kv = self.KV(kv_input)
      # [F, B, N, 2 * E]
      kv = tf.reshape(kv, (*kv.shape[:-1], self.num_heads, 2 * self.depth))
      # 2 * [F, B, N, E]
  ...
  def testMHAForward(self):
    ...
    for attn_type in ('self', 'cross'):
      for use_mask in (True, False):
        ...
        # The attention mask type needs to be `padding`, which will use provided
        # mask. Alternatively, the `causal` will ignore the provided mask and
        # use a upper triangular mask.
        ...
        x_mask = tf.random.uniform((seq_q, seq_kv)) > 0.5 if use_mask else None
    ...
    for attn_type in ('self', 'cross'):
      for use_mask in (False, True):
        ...
        for dx, dx_ref in zip(dxs, dxs_ref):
          self.assertAllClose(dx, dx_ref, rtol=0.1, atol=0.1, msg='dx')
        ...
        for dv, dv_ref in zip(dws, dws_ref):
          self.assertAllClose(dv, tf.reshape(dv_ref, dv.shape), rtol=0.1,
                              atol=0.1, msg='dkernel')
        for dv, dv_ref in zip(dbs, dbs_ref):
          self.assertAllClose(dv, dv_ref, rtol=0.2, atol=0.2, msg='dbias')
