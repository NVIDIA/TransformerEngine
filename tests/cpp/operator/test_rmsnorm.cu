...
void compute_ref_stats(const InputType *data, float *rsigma, const size_t N,
                       const size_t H, const double epsilon) {
...
void compute_ref_output(const InputType *data, const InputType *gamma,
                        OutputType *output, const float *rsigma, const size_t N,
                        const size_t H, float *amax, float scale) {
  using compute_t = float;
...
void compute_ref_backward(const OutputType *output_grad, const InputType *data,
                          const float *rsigma, const InputType *gamma,
                          InputType *data_grad, InputType *gamma_grad,
...
void performTest(const size_t N, const size_t H) {
  ...
  if ((itype == DType::kBFloat16 && otype == DType::kFloat16) ||
    ...
    GTEST_SKIP()
        << "RMSNorm kernel does not support mixing Float16 and BFloat16";
    return;
  }
  ...
  // Forward kernel
  float epsilon = 1e-5;
  nvte_rmsnorm_fwd(input.data(), gamma.data(), scale.data(), epsilon, z.data(),
                   rsigma.data(), 0, prop.multiProcessorCount, workspace.data(),
                   barrier.data(), amax.data(), scale_inv.data(), true);
  ...
  nvte_rmsnorm_bwd(dz.data(), input.data(), rsigma.data(), gamma.data(),
                   dx.data(), dgamma.data(), dgamma_part.data(), 0,
  ...
  rsigma.to_cpu();
  float ref_amax;
  compute_ref_stats(input.cpu_dptr<InputType>(), ref_rsigma.get(), N, H,
                    epsilon);
  compute_ref_output(input.cpu_dptr<InputType>(), gamma.cpu_dptr<WeightType>(),
                     ref_output.get(), rsigma.cpu_dptr<float>(), N, H,
                     &ref_amax, *(scale.cpu_dptr<float>()));
  compute_ref_backward(dz.cpu_dptr<WeightType>(), input.cpu_dptr<InputType>(),
                       rsigma.cpu_dptr<float>(), gamma.cpu_dptr<WeightType>(),
                       ref_dx.get(), ref_dgamma.get(), N, H);
...
std::vector<std::pair<size_t, size_t>> test_cases = {
    {2048, 12288}, {768, 1024}, {256, 65536},
    {128, 6144},   {64, 2304},  {229, 541},  // Primes 50, 100
...
class RMSNormTestSuite
    : public ::testing::TestWithParam<
          std::tuple<transformer_engine::DType, transformer_engine::DType,
