...
CommOverlapCore::CommOverlapCore(ncclComm_t nccl_comm_ptr, int tp_rank, int tp_size,
...
CublasMpDims compute_rs_dims(const TensorWrapper &A, bool transa, const TensorWrapper &B,
  ...
  int64_t k = (transa ? A1 : A0) * tp_size;
  return {m, n, k};
}
...
void CommOverlapBase::bulk_overlap(const TensorWrapper &A, bool transa, const TensorWrapper &B,
                                   ...
                                   cudaStream_t stream_main) {
  NVTE_CHECK(!_with_cublasmp, "Bulk overlap is not supported with cuBlasMp");
...
void CommOverlapP2PBase::copy_into_buffer(cudaStream_t stream, const TensorWrapper &source,
  ...
  if (_with_cublasmp) return;  // cuBlasMp executes its own copy-into-buffer op
