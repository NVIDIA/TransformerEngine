  ...
  for (int param_id = 0; param_id < NVTETensorParam::kNVTENumTensorParams; param_id++) {
    ...
    if (param_dptr != nullptr) {
      if (param_type == NVTETensorParam::kNVTERowwiseData ||
        ...
        // Offset data pointer
        param_dptr += static_cast<size_t>(chunk_offset * typeToSize(param_dtype));
        ...
        param_dptr += static_cast<size_t>(chunk_scale_start * typeToSize(param_dtype));
...
CommOverlapBase::CommOverlapBase(const std::vector<size_t> &buffer_shape, DType buffer_dtype,
  ...
  size_t buffer_bytes = static_cast<size_t>(buffer_shape[0] * buffer_shape[1] * typeToSize(buffer_dtype));
  void *buffer_ptr;
