...
userbuffers_fp16_sum_inplace_gpu_rw(const int op,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int lineoffset,const int numlines, void **commbuff,const int handleridx) {
...
__global__ void
__launch_bounds__(MAX_THREADS)
userbuffers_fp16_sum_inplace_gpu_rr(const int op,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int lineoffset,const int numlines, void **commbuff,const int handleridx) {
  __shared__ int4* userptr[RANKS];
  int *flagptr, physgpu, targetgpu, *myptr;
  int *reduceidptr, reduce_id;
  //if(blockIdx.x==0 && threadIdx.x==0) printf("%d/%d(phys %d gpustep %d firstrank %d):RRkernel(d) start, size %lld\n",myrank,RANKS,gpustep*myrank+firstrank,gpustep,firstrank,numlines*16ull);
...
__global__ void
__launch_bounds__(MAX_THREADS)
userbuffers_fp16_sum_inplace_gpu_rr_rs(const int op,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int mylineoffset,const int totallines, void **commbuff,const int handleridx) {
  __shared__ int4* userptr[RANKS];
  int *flagptr, physgpu, targetgpu, *myptr;
  int *reduceidptr, reduce_id;
  //if(blockIdx.x==0 && threadIdx.x==0) printf("%d/%d(phys %d gpustep %d firstrank %d):RRkernel(d) start, size %lld\n",myrank,RANKS,gpustep*myrank+firstrank,gpustep,firstrank,numlines*16ull);
...
__global__ void
__launch_bounds__(MAX_THREADS)
userbuffers_fp16_sum_inplace_gpu_rr_ag(const int op,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int mylineoffset,const int totallines, void **commbuff,const int handleridx) {
  __shared__ int4* userptr[RANKS];
  int *flagptr, physgpu, targetgpu, *myptr;
  int *reduceidptr, reduce_id;
  //if(blockIdx.x==0 && threadIdx.x==0) printf("%d/%d(phys %d gpustep %d firstrank %d):RRkernel(d) start, size %lld\n",myrank,RANKS,gpustep*myrank+firstrank,gpustep,firstrank,numlines*16ull);
...
__global__ void
__launch_bounds__(MAX_THREADS)
userbuffers_fp16_sum_inplace_gpu_rw_ag(const int op,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int mylineoffset,const int totallines, void **commbuff,const int handleridx) {
  __shared__ int4* userptr[RANKS];
  int *flagptr, physgpu, targetgpu, *myptr;
  int *reduceidptr, reduce_id;
  int4* localptr;
...
userbuffers_fp16_sum_inplace_gpu_rr_blocked(const int op,const int flagoffset,const int firstrank,const int myrank,const int lineoffset,
                                  const int numlines, void **commbuff,const int handleridx,const int peerblocklines, int *hostflags,int *gpuflag,const int numblocks) {
...
#define REDUCETHREADS (blockDim.x-32)
  ...
  if(threadIdx.x<32) {
    ...
    if(threadIdx.x<RANKS) {
      ...
      volatile int* flag = (volatile int*)&(((int*)commbuff[myrank+firstrank])[flagoffset+threadIdx.x+firstrank]);
    ...
    for(int nblock=0;nblock<endblock;nblock++) {
      ...
      if(threadIdx.x==0) {
        __threadfence();
        if(blockIdx.x)
          gpuflag[op*MAX_SMS*2+blockIdx.x]=nblock+basecounter+1;
...
#define ALLGATHERFLAG GF_IBSHARPDONE
...

  {
    ...
    #pragma unroll
    for(int i=0;i<RANKS;i++)
       userptr[i]=reinterpret_cast<int4*>(commbuff[((i+myrank+warp)&(RANKS-1))+handleridx+firstrank]);
  ...
  while (blocklineoffset<numlines) {
      ...
      volatile int* flag = (volatile int*)&(((int*)commbuff[myrank+firstrank])[flagoffset+mydest+32+firstrank]);
...
#define UNROLL 6
      ...
      if(threadIdx.x<maxthreadIdx)  {
        ...
        const int aligned_elem = ((end_elem - start_elem) / (myblockDim*gridDim.x*UNROLL)) * (myblockDim*gridDim.x*UNROLL);
        const int end_aligned = start_elem + aligned_elem;
...
template<int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
userbuffers_fp16_sum_inplace_gpu_rr_blocked2(const int op,const int maxcredit,const int headstart,const int myibrank, const int ibranks,const int commbufoffset,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int lineoffset,
                                  const int numlines, void **commbuff,const int handleridx,const int peerblocklines, int *hostflags,int *gpuflag,const int numblocks) {
 const int basecounter=gpuflag[GF_STATE+op];
//if(blockIdx.x==0 && threadIdx.x==0) printf("%d(%d)[%d/%d]:AR2kernel(%d) start, size %d numblocks %d(%d) basecounter %d peerblocklines %d commbufoffset %d maxcredit %d\n",
//    myrank,gpustep*myrank+firstrank,myibrank,ibranks,op,numlines*16,numblocks,headstart,basecounter,peerblocklines,commbufoffset,maxcredit);
  if(threadIdx.x<32) {
    int *flagptr;
    volatile int* localflag = (volatile int*)&(((int*)commbuff[gpustep*myrank+firstrank])[flagoffset]);
    ...
    for(int nblock=0;nblock<numblocks+headstart;nblock++) {
      ...
      if(nblock>=headstart) {
        ...
        if(!blockIdx.x && !threadIdx.x) hostflags[HF_NVREDUCEDONE+(op&1)]=nblock+basecounter+1-headstart;
      }
    }
    ...
    if(blockIdx.x==0)
      for(int nblock=0;nblock<numblocks;nblock++) {
        ...
        for(int ibflag=threadIdx.x;ibflag<ibranks;ibflag+=32) if(ibflag!=myibrank) while(localflag[REG0_IBAG+ibflag]<expected) {}
        asm volatile ("bar.sync 15, %0;" :: "r"(32));
        if(threadIdx.x<RANKS) flagptr[flagoffset+gpustep*myrank+MAX_NVLINK+firstrank]=expected;
      }
  ...
  { //reducethreads
    ...
    #pragma unroll
    for(int i=0;i<RANKS;i++)
       userptr[i]=reinterpret_cast<int4*>(commbuff[((i+myrank+warp)&(RANKS-1))*gpustep+handleridx+firstrank]);
...
#define UNROLLRS 2
      ...
      const int mydest = (myrank+1+((threadIdx.x-32)>>5)%(RANKS-1)) & (RANKS-1);
      const int mythreadIdx = (mywarp<<5) + (threadIdx.x & 31);
      volatile int* flag = (volatile int*)&(((int*)commbuff[gpustep*myrank+firstrank])[flagoffset+gpustep*mydest+MAX_NVLINK+firstrank]);
...
#define UNROLL 6
  ...
  template<int RANKS>
  __global__ void __launch_bounds__(MAX_THREADS)
  userbuffers_fp16_sum_inplace_gpu_rr_blocked2_rs(const int op,const int maxcredit,const int headstart,const int myibrank, const int ibranks,const int commbufoffset,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int lineoffset,
                                    const int numlines, void **commbuff,const int handleridx,const int peerblocklines, int *hostflags,int *gpuflag,const int numblocks) {
   const int basecounter=gpuflag[GF_STATE+op];
  //if(blockIdx.x==0 && threadIdx.x==0) printf("%d(%d)[%d/%d]:AR2kernel(%d) start, size %d numblocks %d(%d) basecounter %d peerblocklines %d commbufoffset %d maxcredit %d\n",
  //    myrank,gpustep*myrank+firstrank,myibrank,ibranks,op,numlines*16,numblocks,headstart,basecounter,peerblocklines,commbufoffset,maxcredit);
  ...
  #define UNROLLRS 2
    ...
    template<int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
userbuffers_fp16_sum_inplace_gpu_rr_blocked2_ag(const int op,const int maxcredit,const int headstart,const int myibrank, const int ibranks,const int commbufoffset,const int flagoffset,const int firstrank,const int myrank,const int gpustep,const int lineoffset,
                                  const int numlines, void **commbuff,const int handleridx,const int peerblocklines, int *hostflags,int *gpuflag,const int numblocks) {
 const int basecounter=gpuflag[GF_STATE+op];
//if(blockIdx.x==0 && threadIdx.x==0) printf("%d(%d)[%d/%d]:AR2kernel(%d) start, size %d numblocks %d(%d) basecounter %d peerblocklines %d commbufoffset %d maxcredit %d\n",
//    myrank,gpustep*myrank+firstrank,myibrank,ibranks,op,numlines*16,numblocks,headstart,basecounter,peerblocklines,commbufoffset,maxcredit);
  if(threadIdx.x<32) {
    ...
    __syncthreads();
    if(!blockIdx.x && !threadIdx.x) hostflags[HF_NVREDUCEDONE+(op&1)]=numblocks+basecounter;
...
__global__ void userbuffers_fp16_sum_inplace_gpu_null(const int op, int *hostflags,int *gpuflag,int numblocks) {
...
#define callranks_block(x)   if(comm->ar_nvsize==x) \
      userbuffers_fp16_sum_inplace_gpu_rr_blocked<x><<<sms,warps*32,0,stream>>>\
        (userbuffers_allreduceop_sharp,REG0_OFFSET(comm), \
          comm->ar_firstgpu,comm->ar_nvrank,offset/8,elements/8,(void**)(comm->gpu_ptrs),handler*comm->nvsize,blocksize/sizeof(int4)/comm->ar_nvsize,(int*)comm->hostflags,comm->flags,(elements*2+blocksize-1)/blocksize);

#define callranks2_block(x)   if(ar_nvsize==x) {\
...
#define callranks2_block_rs(x)   if(ar_nvsize==x) {\
                int numblocks=(elements*2+blocksize-1)/blocksize; \
                int headstart=numblocks-1;/*<3?numblocks-1:3;*/ \
                if(headstart>maxcredit) headstart=maxcredit; \
                if(x==1) headstart=maxcredit; \
...
#define callranks(x)   if(ar_nvsize==x) { \
    int arg1=op-MAX_OPS,arg2=REG0_OFFSET(comm)-(op==userbuffers_allreduceop_nonsharp?2:1)*REG0_SINGLENODE+MAX_OPS,\
        arg3=ar_firstgpu,arg4=ar_nvrank,arg5=ar_step,arg6=offset/8,arg7=elements/8;\
    void **arg8=(void**)(comm->gpu_ptrs);int arg9=handler*comm->nvsize;\
    void *kernelArgs[] = { (void*)&arg1,(void*)&arg2,(void*)&arg3,(void*)&arg4,(void*)&arg5,(void*)&arg6,(void*)&arg7,(void*)&arg8,(void*)&arg9 }; \
   CUDACHECK(cudaLaunchKernelExC(&cfg, (void*)(comm->use_rr_kernel?userbuffers_fp16_sum_inplace_gpu_rr<x>:userbuffers_fp16_sum_inplace_gpu_rw<x>), kernelArgs)); \
  }

  #define SETUP_LAUNCH_CONFIG(sms,threads,stream) cudaLaunchConfig_t cfg = {sms,threads,0,stream,NULL,0}; \
    cudaLaunchAttribute attribute_ub[2]; \
    attribute_ub[1].id = cudaLaunchAttributeClusterDimension; \
    attribute_ub[1].val.clusterDim.x = comm->cga_size; \
...
//CPU/SHARP part is responsibility of caller
 ...
 if(comm->ar_nvsize==1)  userbuffers_fp16_sum_inplace_gpu_null<<<1,1,0,stream>>>  (userbuffers_allreduceop_sharp,(int*)comm->hostflags,comm->flags,(elements*2+blocksize-1)/blocksize);
  callranks_block(2)
  callranks_block(4)
  callranks_block(8)
...
#define callranks_ag(x)   if(ar_nvsize==x) { \
  int arg1=op-MAX_OPS,arg2=REG0_OFFSET(comm)-(op==userbuffers_allreduceop_nonsharp?2:1)*REG0_SINGLENODE+MAX_OPS,\
      arg3=ar_firstgpu,arg4=ar_nvrank,arg5=ar_step,arg7=elements/8/x,arg6=offset/8+(comm->use_rr_kernel?0:arg4*arg7);\
  void **arg8=(void**)(comm->gpu_ptrs);int arg9=handler*comm->nvsize;\
  void *kernelArgs[] = { (void*)&arg1,(void*)&arg2,(void*)&arg3,(void*)&arg4,(void*)&arg5,(void*)&arg6,(void*)&arg7,(void*)&arg8,(void*)&arg9 }; \
  CUDACHECK(cudaLaunchKernelExC(&cfg, (void*)(comm->use_rr_kernel?userbuffers_fp16_sum_inplace_gpu_rr_ag<x>:userbuffers_fp16_sum_inplace_gpu_rw_ag<x>), kernelArgs)); \
}

#define callranks_rs(x)   if(ar_nvsize==x) { \
  int arg1=op-MAX_OPS,arg2=REG0_OFFSET(comm)-(op==userbuffers_allreduceop_nonsharp?2:1)*REG0_SINGLENODE+MAX_OPS,\
      arg3=ar_firstgpu,arg4=ar_nvrank,arg5=ar_step,arg7=elements/8/x,arg6=offset/8+arg4*arg7;\
  void **arg8=(void**)(comm->gpu_ptrs);int arg9=handler*comm->nvsize;\
  void *kernelArgs[] = { (void*)&arg1,(void*)&arg2,(void*)&arg3,(void*)&arg4,(void*)&arg5,(void*)&arg6,(void*)&arg7,(void*)&arg8,(void*)&arg9 }; \
  CUDACHECK(cudaLaunchKernelExC(&cfg, (void*)userbuffers_fp16_sum_inplace_gpu_rr_rs<x>, kernelArgs)); \
}
...
int reducescatter2_userbuff_inplace_gpu(const int maxcredit,const int handler,const  int offset,const int elements,const int blocksize, communicator* comm, cudaStream_t stream,int op) {
  ...
  if(num_nodes>1) {
    callranks2_block_rs(1)
    callranks2_block_rs(2)
    callranks2_block_rs(4)
...
int allgather2_userbuff_inplace_gpu(const int maxcredit,const int handler,const  int offset,const int elements,const int blocksize, communicator* comm, cudaStream_t stream,int op) {
  ...
  if(num_nodes>1) {
    callranks2_block_ag(1)
    callranks2_block_ag(2)
    callranks2_block_ag(4)
...
void allgather2_userbuff_inplace(const int handler,const  int offset,const int elements, communicator* comm, cudaStream_t stream) {
...
__global__ void
kuserbuffers_pullsend(int myrank, int peer,int* send_id, int* flagptr) {
...
__global__ void __launch_bounds__(MAX_THREADS)
kuserbuffers_pullrecv(int myrank, int peer, int* recv_id, int* flagptr,int4 *srcptr,int4* dstptr, const int lines) {
...
__global__ void
kuserbuffers_pushrecv(int myrank, int peer, int *recv_id, int* flagptr, int adder) {
...
void userbuffers_send(const int srchandler,const size_t srcoffset, const int dsthandler,const size_t dstoffset,
                      const size_t bytes,communicator* comm,const int peer, cudaStream_t stream) {
int peerlocal = peer % comm->nvsize;
void* flagptr = (comm->peer_ptr[0][peerlocal])+((REG0_OFFSET(comm)+REG0_RECV+comm->myrank*MAX_REGIONS+dsthandler)*sizeof(int));
bool signalonly = (bytes/16==0) || (comm->use_ce!=0);
bool intranode = INTRANODE(peer);
//printf("%d: send to %d, push %d ce %d bytes %d signalonly %d sms %d intranode %d srco %lu dsto %lu\n",comm->myrank,peer,comm->push,comm->use_ce,bytes,signalonly,comm->sms,intranode,srcoffset,dstoffset);
//fflush(NULL);
#ifdef MULTINODE
...
if(!intranode && (comm->launch_mode & LAUNCH_GPU)) {
  kuserbuffers_proxysend<<<1,1,0,stream>>> (&(comm->flags[GF_STATE+userbuffers_sendop]),comm->hostflags+userbuffers_sendop);
  return;
}
#endif
...
if(comm->push==0)
    ...
    if(comm->use_ce) CUDACHECK(cudaMemcpyAsync(dstptr,srcptr,bytes,cudaMemcpyDeviceToDevice,stream));
...
__global__ void __launch_bounds__(MAX_THREADS)
kuserbuffers_alltoall(void** baseflagptrs,int flagoffset,int4 *basesrcptr,void** dstptrs,size_t dstoffset, const int lines, const int myrank) {
...
void userbuffers_alltoall_send(const int srchandler,const size_t srcoffset, const int dsthandler,const size_t dstoffset,
  const size_t bytes,communicator* comm, cudaStream_t stream) {
#ifdef MULTINODE
...
if(comm->launch_mode & LAUNCH_GPU)
  kuserbuffers_proxysend<<<1,1,0,stream>>> (&(comm->flags[GF_STATE+userbuffers_alltoall]),comm->hostflags+userbuffers_alltoall);
#else
int lines = bytes/16;
void* srcptr = (comm->mem_ptr[srchandler])+srcoffset;
...
void *kernelArgs[] = { (void*)&arg1,(void*)&arg2,(void*)&arg3,(void*)&arg4,(void*)&arg5,(void*)&arg6,(void*)&arg7};
CUDACHECK(cudaLaunchKernelExC(&cfg, (void*)kuserbuffers_alltoall, kernelArgs));
#endif
}

void userbuffers_recv(const int srchandler,const size_t srcoffset, const int dsthandler,const size_t dstoffset,
                      const size_t bytes,communicator* comm,const int peer, cudaStream_t stream) {
  int peerlocal = peer % comm->nvsize;
  void *flagptr = (comm->mem_ptr[0])+((REG0_OFFSET(comm)+REG0_RECV+peer*MAX_REGIONS+dsthandler)*sizeof(int));
  bool signalonly = (bytes/16==0) || (comm->use_ce!=0);
  bool intranode = INTRANODE(peer);
  if(!(comm->launch_mode & LAUNCH_GPU)) return;
  //printf("%d: recv from %d, push %d ce %d bytes %d signalonly %d sms %d\n",comm->myrank,peer,comm->push,comm->use_ce,bytes,signalonly,comm->sms);
  //fflush(NULL);
  if(comm->push==0 && intranode) {
    ...
    kuserbuffers_pushrecv<<<1,1,0,stream>>> (comm->myrank,peer,&comm->recv_id[peer*MAX_REGIONS+dsthandler],(int*)flagptr,signalonly || !intranode ? 1:comm->sms);
  }
}

void userbuffers_alltoall_recv(communicator* comm, cudaStream_t stream) {
void *flagptr = (comm->mem_ptr[0])+((REG0_OFFSET(comm)+REG0_OPFLAGS*userbuffers_alltoall)*sizeof(int));

if(!(comm->launch_mode & LAUNCH_GPU)) return;
kuserbuffers_pushrecv<<<1,1,0,stream>>> (comm->myrank,-1,(int*)(flagptr+4),(int*)flagptr,comm->nranks-1);
}
