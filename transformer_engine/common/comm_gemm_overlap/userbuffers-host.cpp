...
int create_communicator_grouped2(communicator **comm, int pipegpus, int pipenodes, int tensorgpus,
  ...
  *comm = reinterpret_cast<communicator *>(malloc(sizeof(communicator)));
  ...
  memcpy((*comm)->ucxaddr + (*comm)->myrank * (*comm)->ucx_addr_len, myucxaddr,
         (*comm)->ucx_addr_len);
  MPI_Allgather((*comm)->ucxaddr + (*comm)->myrank * (*comm)->ucx_addr_len, (*comm)->ucx_addr_len,
                MPI_BYTE, (*comm)->ucxaddr, (*comm)->ucx_addr_len, MPI_BYTE, MPI_COMM_WORLD);

  ucp_ep_params_t ep_params;
  ep_params.field_mask = UCP_EP_PARAM_FIELD_REMOTE_ADDRESS;

  for (int r = 0; r < (*comm)->nranks; r++) (*comm)->ucxep[r] = NULL;
...
#else
  register_user_buffer_collective(&((*comm)->gpu_ptrs), LOCALSIZE, *comm,
                                  true);  // will use handler 0
#endif
...
int register_user_buffer_collective(void **gpubuff, size_t bytes, communicator *comm, bool alloc) {
  ...
  if (alloc) {
    ...
    CUCHECK(cuMemExportToShareableHandle(static_cast<void *>(&exphndl[comm->nvrank]),
                                         comm->mhndl[hndl], CU_MEM_HANDLE_TYPE_FABRIC, 0));
    MPI_Allgather(&exphndl[comm->nvrank], sizeof(CUmemFabricHandle), MPI_BYTE, exphndl,
    ...
    for (int i = 0; i < nranks; i++) {
      ...
      if (i == comm->nvrank) {
        ...
        CUCHECK(cuMemImportFromShareableHandle(&remhndls[i], reinterpret_cast<void *>(&exphndl[i]),
...
#ifdef MULTINODE
#ifndef NOSHARP
  if (sharp_coll_reg_mr(comm->sharp_coll_context, *gpubuff, bytes,
                        &(comm->mem_mr[comm->free_region])) != SHARP_COLL_SUCCESS)
    return -2;
#endif
  ...
  for (int r = 0; r < comm->nranks; r++) {
    ...
    UCXCHECK(ucp_ep_rkey_unpack(comm->ucxep[r],
                                comm->rkeys_packed[hndl] + r * comm->rkey_size[hndl],
                                &comm->rkeys[hndl][r]));
  }
#endif
#endif
...
void progress_sharp_allreduce(communicator *comm) {
  ...
  if (req->active == 0) {         // first run, initialize
    // printf("%d: ARSHARP proxy start op size %d blocks %d basecounter
    ...
    if (req->nblock == req->numblocks ||
      ...
      if (sharp_coll_req_test(comm->sharphndl[req->unconfirmed_ib_in_flight % MAX_SHARP])) {
        ...
        if (req->nblock == req->numblocks && req->unconfirmed_ib_in_flight == req->numblocks) {
          // printf("%d: AR SHARP proxy complete op size %d blocks %d basecounter
          // %d\n",comm->myrank,req->elements*2,req->numblocks,req->basecounter);fflush(NULL);
...
#else
    int ret = sharp_coll_do_allreduce_nb(comm->sharp_coll_comm, &reduce_spec,
                                         &comm->sharphndl[req->nblock % MAX_SHARP]);
    if (ret != SHARP_COLL_SUCCESS && !comm->my_node)
...
void progress_ucp_allreduce(communicator *comm, int op) {
  ...
  const int first_node =
      op == userbuffers_allreduceop_nonsharp ? comm->first_node : comm->first2_node;
  ...
  if (req->active == 0) {         // first run, initialize
    ...
    req->numblocks = (req->elements * 2 + req->blocksize - 1) / req->blocksize;
    req->active = 1;
    // printf("%d: AR UCP(%d) proxy start op size %d blocks %d blocksize %d  basecounter %d
    // maxcredit
  ...
  if (req->nblock == 2 * req->numblocks) {
    // printf("%d: AR UCP(%d) proxy complete op size %d blocks %d basecounter
  ...
  if (req->nblock < req->numblocks && hf[HF_NVRSDONE + (op & 1)] >= expecting &&
      hf[HF_NVREDUCEDONE + (op & 1)] >= expecting - req->maxcredit) {
    // launch IBRS
    // printf("%d: proxy IBRS START size %d blocks %d basecounter
    // %d\n",comm->myrank,req->elements*2,req->numblocks,req->basecounter);fflush(NULL);
    ...
    for (int n = 1; n < num_nodes; n++) {
      ...
      const int flagoffset =
          ((REG0_OFFSET(comm) + REG0_OPFLAGS * op + REG0_IBRS + my_node) * sizeof(int));
      // printf("proxy reg0_offset %d flags %d total
      // %d\n",REG0_OFFSET(comm),REG0_FLAGS,REG0_OFFSET(comm)+REG0_FLAGS); printf("IBRS %d(%d,%d):
      // node %d/%d dest %d ibblock %d elts %d addr %lx commoffset
      ...
      UCXPUTCHECK(ucp_put_nbi(
          comm->ucxep[dest], userbuf + useroffset + peerblock * ar_nvrank + ibblock * node, ibblock,
          (uint64_t)(comm->peeraddr[0][dest] + commoffset + ibblock * my_node),
          comm->rkeys[0][dest]));
      UCXCHECK(ucp_atomic_add32(comm->ucxep[dest], 1,
                                (uint64_t)(comm->peeraddr[0][dest] + flagoffset),
                                comm->rkeys[0][dest]));
    }
    req->nblock++;
    return;
  ...
  if (req->nblock >= req->numblocks &&
      hf[HF_NVREDUCEDONE + (op & 1)] >= expecting - req->numblocks) {
    // launch IBAG
    // printf("%d: proxy IBAG START size %d blocks %d basecounter
    // %d\n",comm->myrank,req->elements*2,req->numblocks,req->basecounter);fflush(NULL);
    ...
    // printf("IBAG bytes %ld blocks: %dx%d(ibblock/peerblock %d/%d) nblock %d blockstart %ld
    // useroffset
    // %ld\n",req->elements*2,req->numblocks,req->blocksize,ibblock,peerblock,req->nblock,blockstart,useroffset);fflush(NULL);
    for (int n = 1; n < num_nodes; n++) {
      int node = (my_node + n) % num_nodes;
      ...
      const int dest = (first_node + node * step_node) * comm->nvsize + comm->nvrank;
      const int flagoffset =
          ((REG0_OFFSET(comm) + REG0_OPFLAGS * op + REG0_IBAG + my_node) * sizeof(int));
      UCXPUTCHECK(ucp_put_nbi(comm->ucxep[dest], userbuf + useroffset, ibblock,
                              (uint64_t)(comm->peeraddr[req->handler][dest] + useroffset),
                              comm->rkeys[req->handler][dest]));
      UCXCHECK(ucp_atomic_add32(comm->ucxep[dest], 1,
                                (uint64_t)(comm->peeraddr[0][dest] + flagoffset),
                                comm->rkeys[0][dest]));
    }
...
void progress_ucp_send(communicator *comm) {
    ...
    if (comm->ucxep[dest] == NULL) {  // preconnect on first send
      ...
      for (int hndl = 0; hndl < comm->free_region; hndl++)
        UCXCHECK(ucp_ep_rkey_unpack(comm->ucxep[dest],
                                    comm->rkeys_packed[hndl] + dest * comm->rkey_size[hndl],
                                    &comm->rkeys[hndl][dest]));
    }
    return;
  }
  ...
  if (hf[op] >= expecting) {
    ...
    const int flagoffset =
        ((REG0_OFFSET(comm) + REG0_RECV + comm->myrank * MAX_REGIONS + req->handler2) *
         sizeof(int));
    ...
    if (bytes)
      ucp_put_nbi(comm->ucxep[dest], srcaddr, bytes, (uint64_t)dstaddr, comm->rkeys[dsthndl][dest]);
    UCXCHECK(ucp_atomic_add32(comm->ucxep[dest], 1,
                              (uint64_t)(comm->peeraddr[0][dest] + flagoffset),
                              comm->rkeys[0][dest]));
    comm->active_nreqs--;
...
void progress_ucp_alltoall(communicator *comm) {
  ...
  if (hf[op] >= expecting) {
    ...
    for (int i = 1; i < comm->nranks; i++) {
      ...
      if (bytes)
        ucp_put_nbi(comm->ucxep[dest], srcaddr + (dest * bytes), bytes, (uint64_t)dstaddr,
                    comm->rkeys[dsthndl][dest]);
      UCXCHECK(ucp_atomic_add32(comm->ucxep[dest], 1,
                                (uint64_t)(comm->peeraddr[0][dest] + flagoffset),
                                comm->rkeys[0][dest]));
    }
