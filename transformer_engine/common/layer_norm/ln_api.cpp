...
RMSNormFwdGeneralRegisty RMSNORM_FWD_GENERAL_FUNCS;
RMSNormFwdTunedRegistry RMSNORM_FWD_TUNED_FUNCS;
RMSNormBwdGeneralRegisty RMSNORM_BWD_GENERAL_FUNCS;
RMSNormBwdTunedRegistry RMSNORM_BWD_TUNED_FUNCS;
...
layer_norm::FwdFunction & get_rmsnorm_fwd_launcher(DType wtype,
                                           DType itype,
                                           DType otype,
                                           DType ctype,
                                           uint32_t hidden_size,
    ...
    auto tuned_key = layer_norm::get_key(wtype, itype, otype, ctype, hidden_size);
    if (batch_size % 4 == 0
        && layer_norm::RMSNORM_FWD_TUNED_FUNCS.count(tuned_key) > 0) {
        return layer_norm::RMSNORM_FWD_TUNED_FUNCS.at(tuned_key);
    }

    // Pick general kernel
    auto general_key = layer_norm::get_key(wtype, itype, otype, ctype, 0);
    if (layer_norm::RMSNORM_FWD_GENERAL_FUNCS.count(general_key) == 0) {
        NVTE_ERROR("FWD: Unsupported types.");
    }
    auto& general_func_map = layer_norm::RMSNORM_FWD_GENERAL_FUNCS.at(general_key);
    auto func_iter = general_func_map.lower_bound(hidden_size);
    if (func_iter == general_func_map.end()) {
        // Hidden size is too big, need to use multi-CTA
        return general_func_map.rbegin()->second;
...
layer_norm::BwdFunction & get_rmsnorm_bwd_launcher(DType wtype,
                                           DType itype,
                                           DType otype,
                                           DType ctype,
                                           uint32_t hidden_size,
    ...
    auto tuned_key = layer_norm::get_key(wtype, itype, otype, ctype, hidden_size);
    if (batch_size % 4 == 0
        && layer_norm::RMSNORM_BWD_TUNED_FUNCS.count(tuned_key) > 0) {
        return layer_norm::RMSNORM_BWD_TUNED_FUNCS.at(tuned_key);
    }

    // Pick general kernel
    auto general_key = layer_norm::get_key(wtype, itype, otype, ctype, 0);
    if (layer_norm::RMSNORM_BWD_GENERAL_FUNCS.count(general_key) == 0) {
        NVTE_ERROR("BWD: Unsupported types.");
    }
    auto& general_func_map = layer_norm::RMSNORM_BWD_GENERAL_FUNCS.at(general_key);
    auto func_iter = general_func_map.lower_bound(hidden_size);
    if (func_iter == general_func_map.end()) {
        // Hidden size is too big, need to use multi-CTA
        return general_func_map.rbegin()->second;
...
void rmsnorm_fwd(const Tensor& x,
    ...
    // Request the kernel launcher.
    auto launcher = layer_norm::get_rmsnorm_fwd_launcher(wtype, itype, otype, ctype,
                                                 hidden_size, rows);
...
void rmsnorm_bwd(const Tensor& dz,
                 const Tensor& x,
                 const Tensor& rsigma,
                 const Tensor& gamma,
                 Tensor* dx,
    ...
    auto launcher = layer_norm::get_rmsnorm_bwd_launcher(wtype, itype, otype, ctype,
                                                 hidden_size, rows);
...
void nvte_rmsnorm_bwd(const NVTETensor dz,       // Nxhidden_size
                      ...
                      NVTETensor barrier) {
  using namespace transformer_engine;
  rmsnorm_bwd(*reinterpret_cast<const Tensor*>(dz),
                *reinterpret_cast<const Tensor*>(x),
                *reinterpret_cast<const Tensor*>(rsigma),
                *reinterpret_cast<const Tensor*>(gamma),
                reinterpret_cast<Tensor*>(dx),
