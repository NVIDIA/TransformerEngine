...
/*! \brief Cast tensor to FP8 with option to skip the computation. Useful when using CUDA graphs.
 * In order to skip the computation a noop input has to be provided - a single element tensor
 * containing 1 (to skip) or 0 (to proceed normally).
 *
 *  \param[in]     input            Input tensor to be cast.
 *  \param[in,out] output           Output FP8 tensor.
 *  \param[in]     noop             Whether to skip the computation.
 *  \param[in]     stream           CUDA stream used for the operation.
 */
