...
__global__ void fused_scores_for_aux_loss_forward_kernel(
...
/***
    ...
    DataType * topk_logits_buf = reinterpret_cast<DataType *>(logits_buf + num_experts * num_token_per_block);
    ...
    for(int round = blockIdx.x; round < total_round; round += gridDim.x) {
         ...
         */
        naive_topk_and_mask(
            local_logits, num_experts, topk, topk_indices, topk_logits, lane_id);
        __syncwarp();
...
void fused_scores_for_aux_loss_forward_kernel_launcher(
    ...
    size_t shared_memory_size = 
        num_experts * num_token_per_block * sizeof(DataType) // logits
        + topk * num_token_per_block * sizeof(DataType) // topk_logits
        + topk * num_token_per_block * sizeof(int); // topk_indices
    fused_scores_for_aux_loss_forward_kernel<DataType><<<grid_size, kThreadsPerBlock, shared_memory_size, stream>>>(
        logits,
        num_tokens,
        num_experts,
        topk,
...
void fused_scores_for_aux_loss_forward(
    const Tensor &logits,
    int num_tokens,
    int num_experts,
    int topk,
    ...
    TE_ROUTER_PROBS_TYPE_SWITCH_ALL(
        ...
        fused_scores_for_aux_loss_forward_kernel_launcher<DataType>(
            ...
            topk,
            score_function,
            reinterpret_cast<DataType *>(scores.data.dptr),
...
template<typename DataType>
__global__ void fused_scores_for_aux_loss_backward_kernel(
    ...
    DataType * act_from_fwd_buf = reinterpret_cast<DataType *>(grad_scores_buf + num_experts * num_token_per_block);
...
template<typename DataType>
void fused_scores_for_aux_loss_backward_kernel_launcher(
    const DataType* intermediate_output,
    const DataType* grad_scores,
    int num_tokens,
    int num_experts,
    ...
    size_t grid_size = (num_tokens + num_token_per_block - 1) / num_token_per_block;
    size_t shared_memory_size = 
        num_experts * num_token_per_block * sizeof(DataType) // grad_scores
        + num_experts * num_token_per_block * sizeof(DataType) // act_from_fwd
        + num_experts * num_token_per_block * sizeof(DataType); // comp_buf
    fused_scores_for_aux_loss_backward_kernel<DataType><<<grid_size, kThreadsPerBlock, shared_memory_size, stream>>>(
        intermediate_output,
        grad_scores,
        num_tokens,
        num_experts,
...
void fused_scores_for_aux_loss_backward(
    ...
    TE_ROUTER_PROBS_TYPE_SWITCH_ALL(
        ...
        fused_scores_for_aux_loss_backward_kernel_launcher<DataType>(
            reinterpret_cast<DataType *>(intermediate_output.data.dptr),
            reinterpret_cast<DataType *>(grad_scores.data.dptr),
            num_tokens,
            num_experts,
            topk,
            score_function,
...
void nvte_fused_scores_for_aux_loss_forward(const NVTETensor logits, int num_tokens, int num_experts, int topk, int score_function, NVTETensor scores, const NVTETensor routing_map, const NVTETensor intermediate_output, cudaStream_t stream){
    ...
    fused_scores_for_aux_loss_forward(
        *convertNVTETensorCheck(logits),
        num_tokens,
        num_experts,
        topk,
        score_function,
        *convertNVTETensorCheck(scores),
...
void nvte_fused_scores_for_aux_loss_backward(const NVTETensor intermediate_output, const NVTETensor grad_scores, int num_tokens, int num_experts, int topk, int score_function, NVTETensor grad_logits, cudaStream_t stream){
    ...
    fused_scores_for_aux_loss_backward(
        *convertNVTETensorCheck(intermediate_output),
        *convertNVTETensorCheck(grad_scores),
        num_tokens,
        num_experts,
        topk,
        score_function,
