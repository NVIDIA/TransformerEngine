...
constexpr int kThreadsPerBlock = 128;  // Using 4 warps in 1 CTA, Each warp is responsible for 1 token.
...
template <typename T>
__device__ inline T warp_reduce_on_shmem(T *data_ptr, int data_size, T (*reduce_func)(T, T), int lane_id) {
...
__device__ inline void apply_sigmoid_on_float(
    DataType * scores,
    int data_size,
    int lane_id
) {
...
template <typename T>
__device__ inline T masked_warp_reduce_on_shmem(T *data_ptr, bool *mask, int data_size, T (*reduce_func)(T, T), int lane_id) {
  // Some value is hanlded in local thread
  // Thread 0 is responsible for the: 0-th, 32-th, 64-th, 96-th ...
  // Reduce the value in local thread
...
template<typename DataType>
__device__ inline void apply_sigmoid_bwd_on_float(
    DataType *grad,
    DataType *fwd_output,
    int data_size,
    int lane_id
...
template<typename DataType>
__device__ inline void apply_softmax_bwd_on_float(
    DataType *grad,
    DataType *fwd_output,
    DataType *comp_buf,
    bool *mask,
    ...
    for(int i = lane_id; i < data_size; i += kThreadsPerWarp) {
        if(mask){
            if(mask[i]) comp_buf[i] = float(grad[i]) * float(fwd_output[i]);
            else comp_buf[i] = 0.0f;
        }else{
            comp_buf[i] = float(grad[i]) * float(fwd_output[i]);
        }
    }
    __syncwarp();
    ...
    for(int i = lane_id; i < data_size; i += kThreadsPerWarp) {
        if(mask){   
            if(mask[i]) grad[i] = float(fwd_output[i]) * (float(grad[i]) - sum_Output_x_Grad);
            else grad[i] = 0.0f;
        }else{
            grad[i] = float(fwd_output[i]) * (float(grad[i]) - sum_Output_x_Grad);
        }
    }
}

template<typename DataType>
__device__ inline void apply_softmax_on_float(
    DataType * scores,
    int data_size,
    int lane_id
) {
    // 1. compute the max of value
    float max_val = float(warp_reduce_on_shmem(scores, data_size, max, lane_id));
    // 2. value -> exp_value
    for(int i = lane_id; i < data_size; i += kThreadsPerWarp) {
        scores[i] = float(exp(float(scores[i]) - max_val));
    }
    __syncwarp();
    // 3. compute the sum of exp_value
    float sum_val = float(warp_reduce_on_shmem(scores, data_size, sum, lane_id));
    // 4. update the softmax value
    for(int i = lane_id; i < data_size; i += kThreadsPerWarp) {
        scores[i] = float(scores[i]) / sum_val;
    }
    __syncwarp();
}

template<typename T>
__device__ inline void naive_topk_and_mask(
    T * scores,
    int data_size,
    int topk,
    int * topk_indices,
