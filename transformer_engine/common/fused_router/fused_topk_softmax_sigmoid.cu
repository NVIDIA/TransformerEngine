...
__global__ void fused_topk_softmax_sigmod_forward_kernel(
    ...
    DataType * topk_scores_buf = reinterpret_cast<DataType *>(scores_buf + num_experts * num_token_per_block);
    ...
    if(group_topk > 0) {
        ...
        group_scores_buf = reinterpret_cast<DataType *>(masked_scores_buf + num_experts * num_token_per_block);
    ...
    for(int round = blockIdx.x; round < total_round; round += gridDim.x) {
        ...
        if( group_topk > 0 ) {
            ...
            __syncwarp();
            naive_topk_and_mask(
                masked_scores, num_experts, topk, topk_indices, topk_scores, lane_id);

        }else {
            naive_topk_and_mask(
                scores, num_experts, topk, topk_indices, topk_scores, lane_id);
        }
        __syncwarp();
...
void fused_topk_softmax_sigmoid_forward_kernel_launcher(
    ...
    fused_topk_softmax_sigmod_forward_kernel<DataType, BiasType><<<grid_size, kThreadsPerBlock, shared_memory_size, stream>>>(
        logits,
        num_tokens,
        num_experts,
        topk,
...
void fused_topk_softmax_sigmoid_forward(const Tensor logits, int num_tokens, int num_experts, int topk, bool use_pre_softmax, int num_groups, int group_topk, float scaling_factor, int score_function, const Tensor expert_bias, Tensor probs, Tensor routing_map, Tensor intermediate_output, cudaStream_t stream){
...
__global__ void fused_topk_softmax_sigmod_backward_kernel(
    ...
    const DataType * intermediate_output,
    ...
    DataType * act_from_fwd_buf = reinterpret_cast<DataType *>(grad_probs_buf + num_experts * num_token_per_block);
    DataType * comp_buf = reinterpret_cast<DataType *>(act_from_fwd_buf + num_experts * num_token_per_block);
    ...
    for(int round = blockIdx.x; round < total_round; round += gridDim.x) {
        ...
        if(topk > 1 && score_function == 0) {
            ...
            for(int i = lane_id; i < num_experts; i += kThreadsPerWarp) {
                local_comp_buf[i] = (local_routing_map[i] ? 
                    local_grad[i] * local_act_from_fwd[i] : 0.0f);
            }
            __syncwarp();
            ...
            for(int i = lane_id; i < num_experts; i += kThreadsPerWarp) {
                if (local_routing_map[i]) {
                    local_grad[i] = local_grad[i] / (sum_fwd_input + epsilon) - sum_Output_x_Grad / ( (sum_fwd_input + epsilon) * (sum_fwd_input + epsilon) );
                } else {
                    local_grad[i] = 0.0f;
                }
            }
        ...
        // Softmax bwd if use_pre_softmax is false
        if(!use_pre_softmax && score_function == 1) {
            apply_softmax_bwd_on_float(local_grad, local_act_from_fwd, local_comp_buf, local_routing_map, num_experts, lane_id);
            __syncwarp();
        }
        ...
        if(score_function == 1 && use_pre_softmax) {
            apply_softmax_bwd_on_float(local_grad, local_act_from_fwd, local_comp_buf, nullptr, num_experts, lane_id);
            __syncwarp();
        }
        // Sigmoid bwd
        if(score_function == 0) {
...
void fused_topk_softmax_sigmoid_backward_kernel_launcher(
    ...
    size_t grid_size = (num_tokens + num_token_per_block - 1) / num_token_per_block;
    size_t shared_memory_size = 
        num_experts * num_token_per_block * sizeof(DataType) // grad_probs
    ...
    fused_topk_softmax_sigmod_backward_kernel<DataType><<<grid_size, kThreadsPerBlock, shared_memory_size, stream>>>(
        routing_map,
        intermediate_output,
        grad_probs,
        num_tokens,
...
void fused_topk_softmax_sigmoid_backward(
    const Tensor &routing_map,
    ...
    TE_ROUTER_PROBS_TYPE_SWITCH_ALL(
        ...
        fused_topk_softmax_sigmoid_backward_kernel_launcher<DataType>(
            reinterpret_cast<bool *>(routing_map.data.dptr),
            reinterpret_cast<DataType *>(intermediate_output.data.dptr),
            reinterpret_cast<DataType *>(grad_probs.data.dptr),
            num_tokens,
            num_experts,
            topk,
            use_pre_softmax,
            scaling_factor,
            score_function,
            reinterpret_cast<DataType *>(grad_logits.data.dptr),
            stream
        );
    );
}
...
void nvte_fused_topk_softmax_sigmoid_forward(const NVTETensor logits, int num_tokens, int num_experts, int topk, bool use_pre_softmax, int num_groups, int group_topk, float scaling_factor, int score_function, const NVTETensor expert_bias, NVTETensor probs, NVTETensor routing_map, NVTETensor intermediate_output, cudaStream_t stream){
    ...
    fused_topk_softmax_sigmoid_forward(
        *convertNVTETensorCheck(logits),
        ...
        scaling_factor,
        score_function,
        *convertNVTETensorCheck(expert_bias),
        *convertNVTETensorCheck(probs),
        *convertNVTETensorCheck(routing_map),
        *convertNVTETensorCheck(intermediate_output),
        stream
    );
}
...
void nvte_fused_topk_softmax_sigmoid_backward(const NVTETensor routing_map, const NVTETensor intermediate_output, const NVTETensor grad_probs, int num_tokens, int num_experts, int topk, bool use_pre_softmax, float scaling_factor, int score_function, NVTETensor grad_logits, cudaStream_t stream){
    ...
    fused_topk_softmax_sigmoid_backward(
        ...
        *convertNVTETensorCheck(grad_probs),
        ...
        scaling_factor,
        score_function,
        *convertNVTETensorCheck(grad_logits),
        stream
    );
}
