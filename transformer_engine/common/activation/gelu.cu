...
void gelu_cast(const Tensor &input, Tensor *output, cudaStream_t stream) {
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          ...
          VectorizedUnaryKernelLauncher<nvec, detail::GELUParam, detail::gelu>(
              ...
              reinterpret_cast<fp32 *>(output->amax.dptr), tot_elts, {},
              stream););  // NOLINT(*)
  );                      // NOLINT(*)
}
...
void nvte_gelu(const NVTETensor input, NVTETensor output, cudaStream_t stream) {
  ...
  gelu_cast(*reinterpret_cast<const Tensor *>(input), reinterpret_cast<Tensor *>(output), stream);
}
