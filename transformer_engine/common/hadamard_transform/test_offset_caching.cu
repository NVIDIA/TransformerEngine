...
__device__ __forceinline__ size_t get_current_tensor_id(
    ...
    const size_t current_offset, const size_t first_logical_dim,
  ...
  if (shape_rep == SAME_BOTH_DIMS) {
    ...
    while (low < hi) {
      ...
      if (mid_offset <= current_offset) low = mid + 1;
      else                              hi  = mid;
    }
...
__global__ void test_offset_caching_kernel(
  ...
  for (size_t q = threadIdx.x; q < num_queries; q += blockDim.x) {
    ...
    result_gmem[q] = get_current_tensor_id(
        shape_rep, num_tensors, offset,
        first_logical_dim, last_logical_dim, gmem_offsets);

    result_smem[q] = get_current_tensor_id(
        shape_rep, num_tensors, offset,
        first_logical_dim, last_logical_dim, offsets_smem);
  }
}
...
static size_t ref_get_tensor_id(ShapeRepresentation shape_rep,
                                size_t num_tensors,
  ...
  if (shape_rep == SAME_BOTH_DIMS) {
    ...
    while (low < hi) {
      ...
      if (static_cast<size_t>(offsets[mid]) <= current_offset) low = mid + 1;
      else                                                       hi  = mid;
    }
    return (low == 0) ? 0 : (low - 1);
  }
...
static void run_test(ShapeRepresentation shape_rep,
                     const std::vector<int64_t>   &offsets_host,
                     const std::vector<size_t>    &queries_host,
                     size_t first_logical_dim,
                     size_t last_logical_dim) {
  ...
  CUDA_CHECK(cudaMemcpy(d_offsets, offsets_host.data(),
                        (num_tensors + 1) * sizeof(int64_t), cudaMemcpyHostToDevice));
  CUDA_CHECK(cudaMemcpy(d_queries, queries_host.data(),
  ...
  test_offset_caching_kernel<<<1, 128, smem_bytes>>>(
      d_offsets, d_queries,
      d_result_gmem, d_result_smem,
      num_tensors, num_queries,
      shape_rep, first_logical_dim, last_logical_dim);
  ...
  for (size_t q = 0; q < num_queries; ++q) {
    size_t ref = ref_get_tensor_id(shape_rep, num_tensors, queries_host[q],
                                   first_logical_dim, last_logical_dim, offsets_host);
    EXPECT_EQ(h_gmem[q], ref)
        << "query=" << queries_host[q] << " gmem result mismatch at q=" << q;
    EXPECT_EQ(h_smem[q], ref)
        << "query=" << queries_host[q] << " smem result mismatch at q=" << q;
    EXPECT_EQ(h_gmem[q], h_smem[q])
        << "query=" << queries_host[q] << " gmem != smem at q=" << q;
  }
...
TEST(OffsetCaching, SameBothDims) {
  ...
  for (size_t i = 0; i <= N; ++i)
    offsets[i] = static_cast<int64_t>(i * rows_per * hidden);
...
TEST(OffsetCaching, ExactBoundary) {
  ...
  std::vector<size_t> queries;
  for (size_t t = 0; t < N; ++t)
    queries.push_back(static_cast<size_t>(offsets[t]));
