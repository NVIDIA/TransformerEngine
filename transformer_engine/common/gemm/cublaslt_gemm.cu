  ...
  if (use_fp8 || use_fp4) {
    ...
    if (is_tensor_scaling(inputA->scaling_mode) && is_tensor_scaling(inputB->scaling_mode)) {
      ...
      auto get_scale_mode = [] (DType dtype) -> cublasLtMatmulMatrixScale_t {
        ...
        if (dtype == DType::kFloat8UE5M3) {
          /// TODO Use cuBLAS APIs once e5m3 support is added
          return static_cast<cublasLtMatmulMatrixScale_t>(8);
          // return CUBLASLT_MATMUL_MATRIX_SCALE_MASK_VEC16_UE5M3;
        }
