...
#define CUBLAS_MXFP8_GROUPED_GEMM_VERSION 130200
 ...
 inline size_t validate_grouped_gemm_input_list(
   ...
   NVTE_CHECK(alpha_numel == num_tensors, "Grouped GEMM: alpha must have num_tensors (",
              num_tensors, ") elements, got ", alpha_numel);
 ...
 inline MultiTensorGroupGemmOutputArgs build_grouped_gemm_multi_out_args(
   ...
   NVTE_CHECK(list_size <= static_cast<size_t>(kMaxTensorsPerKernel), "Grouped GEMM: ", name,
              "_list supports up to ", kMaxTensorsPerKernel, " tensors per kernel, got ",
              list_size);
   ...
   for (size_t i = 0; i < list_size; ++i) {
     ...
     NVTE_CHECK(t->dtype() == expected_dtype, "Grouped GEMM: ", name, "_list tensor ", i,
                " dtype mismatch. Expected ", transformer_engine::to_string(expected_dtype),
                " got ", transformer_engine::to_string(t->dtype()));
 ...
 inline MultiTensorListInfo validate_grouped_gemm_multi_inputA_list(const NVTETensor *tensor_list,
   ...
   const transformer_engine::Tensor *t0 =
       transformer_engine::convertNVTETensorCheck(tensor_list[0]);
 ...
 __global__ void grouped_bias_add_kernel(char *d_base, const char *bias_base,
                                         TensorShapeInfo d_meta, TensorShapeInfo bias_meta,
                                         size_t num_tensors) {
...
#pragma unroll
 ...
 void nvte_grouped_gemm(const NVTEGroupedTensor A, int transa, const NVTEGroupedTensor B,
                        int transb, const NVTEGroupedTensor C, NVTEGroupedTensor D,
 ...
 void nvte_grouped_gemm_with_discrete_inputA(const NVTETensor *A_list, size_t num_a_tensors,
   ...
   // Compute average dimensions for heuristics
   int64_t avg_m_val = config_.avg_m.value_or(compute_avg_first_dim(outputD));
   int64_t avg_n_val = config_.avg_n.value_or(transb ? compute_avg_first_dim(inputB)
 ...
 void nvte_grouped_gemm_with_discrete_out(const NVTEGroupedTensor A, int transa,
   ...
   // Compute average dimensions for heuristics
   int64_t avg_m_val = config_.avg_m.value_or(transa ? compute_avg_last_dim(inputA)
