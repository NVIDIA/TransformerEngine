...
// Backend-agnostic grouped GEMM: device-side setup kernels, operand selection and
// validation, backend dispatch, and the public nvte_grouped_* entry points. The
// cuBLAS(Lt) backend lives in cublaslt_grouped_gemm.cu and the CUTLASS NVFP4
// backend in nvfp4_cutlass_grouped_gemm.cu; the shared types are in
// grouped_gemm_common.h.
...
inline bool maybe_run_nvfp4_cutlass_grouped(const GroupedGemmSetupWorkspace &setup_workspace,
                                            ...
                                            bool with_beta, void *cublas_workspace_ptr,
                                            cudaStream_t stream) {
  // Cheap operand check before the (more expensive) env-var lookup, so non-NVFP4
  // recipes never pay the getenv cost.
  ...
  // Reuse the caller-provided cuBLAS grouped-GEMM workspace as scratch for the
  // CUTLASS path (metadata + CUTLASS workspace are carved from it). This branch
  // returns before execute_grouped_gemm, so that buffer is otherwise unused
  // here; no allocation happens inside TE common, keeping the launch graph-safe.
  //
  // Note the two backends' workspace requirements are NOT the same quantity:
  // cuBLAS uses a fixed kGroupedGemmCublasWorkspaceSize (32 MiB), whereas the
  // CUTLASS need is computed independently (per-group metadata arrays +
  // Gemm::get_workspace_size) and is only a few KB-MB for the MoE shapes we
  // target -- comfortably within that 32 MiB. We pass the size through and
  // run_nvfp4_graph_safe_grouped_gemm NVTE_CHECKs it fits rather than assuming
  // the two are equal, so an unexpectedly large CUTLASS requirement fails loudly
  // instead of overrunning the buffer.
  transformer_engine::nvfp4_cutlass::run_nvfp4_graph_safe_grouped_gemm(
      ...
      setup_workspace.d_cols, A_sel.trans, static_cast<int>(num_tensors), fp32_out,
...
void nvte_grouped_gemm_with_discrete_out(const NVTEGroupedTensor A, int transa,
  ...
  if (maybe_run_nvfp4_cutlass_grouped(workspace.setup_workspace, A_sel, B_sel, d_dtype, num_tensors,
                                      /*with_beta=*/true, workspace.cublas_workspace_ptr, stream)) {
    return;
  }
