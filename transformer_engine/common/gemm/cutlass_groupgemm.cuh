...
#include "cutlass/util/packed_stride.hpp"
  ...
  template <typename GemmT, typename ElementA, typename ElementB, typename ElementC,
  ...
  typename GemmT::Arguments MakeArguments(
      ...
      StrideC* stride_C, float alpha, float beta, int device, int math_sm_count) {
    ...
    arguments = typename GemmT::Arguments{cutlass::gemm::GemmUniversalMode::kGrouped,
                                          {num_experts, (ProblemShapeType*)problem_sizes,
                                           (ProblemShapeType const*)problem_sizes_host},
  ...
  void CutlassGroupedGemm(bool transa, bool transb, const NVTETensor* A, const NVTETensor* B,
    ...
    ElementB** ptr_B_host =
        reinterpret_cast<ElementB**>(host_workspace + gemm_coord_size + ptr_size);
    ElementC** ptr_C_host =
  ...
  void CutlassGroupedGemm(bool transa, bool transb, const NVTETensor* A, const NVTETensor* B,
                          ...
                          int num_gemms, cudaStream_t stream, int device, int math_sm_count) {
    if (!transa && !transb) {
      CutlassGroupedGemm<false, false, T>(transa, transb, A, B, D, workspace, alpha, beta,
