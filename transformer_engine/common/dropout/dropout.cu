...
template <typename T>
  __global__  void dropout_kernel_fwd(T *input, T *output, uint16_t *mask,
                            int64_t *rng_state, int n_rng_blocks, unsigned p_dropout_in_uint32_t, float inv_prob) {
  ...
  __global__  void dropout_kernel_fwd_fp8(T *input, float *scale_inv, O *output, uint16_t *mask,
    ...
    while (tid < n_rng_blocks) {
        ...
        for (int j=0; j<2; j++) {
              ...
              #pragma unroll
              for (int i=0; i<8; i++) {
                output_h_8[i] = (result_copy & 0x1 == 0x1) ? (O)0.f : static_cast<O>(static_cast<float>(input_h_8[i]) * scale_inv_val * inv_prob);
                result_copy = result_copy >> 1;
              }
...
template <typename T>
__global__ void dropout_kernel_bwd(T *grad_out, uint16_t *in_mask, T *grad_in, const size_t n_rng_blocks, const float inv_prob) {
    int tid = (blockIdx.y * gridDim.x + blockIdx.x) * blockDim.x + threadIdx.x;
    int num_threads = gridDim.y * gridDim.x * blockDim.x;
    uint16_t result = 0;
    ull2 input8;
...
void prepare_dropout_fwd(Tensor input, Tensor output, Tensor mask,
                         Tensor rng_state, float dropout_probability,
                         cudaStream_t stream) {
  ...
  NVTE_CHECK(input.numel() >= blk_size*rng_block_size, "numel must be greater than or equal to blk_size*rng_block_size");
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_16BIT(
      ...
      dropout_kernel_fwd<dtype><<<dim_grid, dim_block, 0, stream>>>(
          ...
          reinterpret_cast<uint16_t *>(mask.data.dptr), reinterpret_cast<int64_t *>(rng_state.data.dptr), n_rng_blks, p_dropout_in_uint32_t_8bit, inv_prob);
  );
}

void prepare_dropout_fwd_fp8(Tensor input, Tensor output, Tensor mask,
                         Tensor rng_state, float dropout_probability,
                         cudaStream_t stream) {
  using namespace transformer_engine;
  ...
  NVTE_CHECK(is_tensor_scaling(input.scaling_mode), "Only per-tensor scaling is supported for fp8 dropout");
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(
    ...
    TRANSFORMER_ENGINE_TYPE_SWITCH_16BIT(
      output.dtype(), otype,
      dropout_kernel_fwd_fp8<itype, otype><<<dim_grid, dim_block, 0, stream>>>(
        reinterpret_cast<itype *>(input.data.dptr), reinterpret_cast<float *>(input.scale_inv.dptr), reinterpret_cast<otype *>(output.data.dptr),
        reinterpret_cast<uint16_t *>(mask.data.dptr), reinterpret_cast<int64_t *>(rng_state.data.dptr), n_rng_blks, p_dropout_in_uint32_t_8bit, inv_prob);
...
void prepare_dropout_bwd(Tensor grad_output, Tensor mask, Tensor grad_input, float dropout_probability,
                         cudaStream_t stream) {
  using namespace transformer_engine;
    ...
    TRANSFORMER_ENGINE_TYPE_SWITCH_16BIT(
        ...
        dropout_kernel_bwd<dtype><<<dim_grid, dim_block, 0, stream>>>(
            reinterpret_cast<dtype *>(grad_output.data.dptr), reinterpret_cast<uint16_t *>(mask.data.dptr),
...
void nvte_dropout_fwd(NVTETensor input, NVTETensor output, NVTETensor mask,
  ...
  prepare_dropout_fwd(*convertNVTETensorCheck(input),
                      *convertNVTETensorCheck(output),
                      *convertNVTETensorCheck(mask),
                      *convertNVTETensorCheck(rng_state),
...
void nvte_dropout_fwd_fp8(NVTETensor input, NVTETensor output, NVTETensor mask,
  ...
  prepare_dropout_fwd_fp8(*convertNVTETensorCheck(input),
                          *convertNVTETensorCheck(output),
                          *convertNVTETensorCheck(mask),
                          *convertNVTETensorCheck(rng_state),
                          dropout_probability, stream);
...
void nvte_dropout_bwd(NVTETensor grad_output, NVTETensor mask, NVTETensor grad_input, float dropout_probability,
                      cudaStream_t stream) {
  NVTE_API_CALL(nvte_dropout_bwd);
  using namespace transformer_engine;

  prepare_dropout_bwd(*convertNVTETensorCheck(grad_output),
                      *convertNVTETensorCheck(mask),
