...
void cast_transpose(const Tensor &input, Tensor *cast_output, Tensor *transposed_output,
                    cudaStream_t stream) {
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          cast_output->data.dtype, OutputType, constexpr int itype_size = sizeof(InputType);
          ...
          if (full_tile) {
            ...
            cast_transpose_kernel<nvec_in, nvec_out, fp32, InputType, OutputType>
                <<<n_blocks, cast_transpose_num_threads,
                   ...
                   stream>>>(reinterpret_cast<const InputType *>(input.data.dptr),
                             ...
                             reinterpret_cast<fp32 *>(cast_output->scale_inv.dptr), row_length,
                             num_rows, n_tiles);
          } else {
            cudaFuncSetAttribute(
...
void nvte_cast_transpose(const NVTETensor input, NVTETensor cast_output,
  ...
  cast_transpose(*reinterpret_cast<const Tensor *>(input), reinterpret_cast<Tensor *>(cast_output),
