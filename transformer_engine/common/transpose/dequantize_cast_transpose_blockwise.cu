...
__global__ void dequantize_cast_transpose_1x128_aligned_kernel(
    ...
    for(auto w = warp_id; w < kTileDim/4 && offset_row < seq_len; w += blockDim.x / 32, offset_row += 4){
        float buffer_fp32[4];
...
template<typename FP8_TYPE>
__global__ void dequantize_cast_transpose_1x128_kernel(
    FP8_TYPE * input,
    float * input_scale_inv,
    FP8_TYPE * output,
    float * output_scale_inv,
    ...
    for(auto x = warp_id; (block_offset_y + x < hidden_dim) && (x < kTileDim); x += blockDim.x / 32 ){
...
void nvte_transpose_blockwise(
    ...
    NVTE_CHECK(rowwise_shape[0] == colwise_shape[1] && rowwise_shape[1] == colwise_shape[0], "The shape of rowwise_data and inversed colwise_data must be the same");
    NVTE_CHECK(rowwise_shape[0] == rowwise_scale_inv_shape[1], "num of rows of rowwise_data must be equal to num of cols of rowwise_scale_inv, got rowwise_shape[0]:", rowwise_shape[0],", rowwise_scale_inv_shape[1]:", rowwise_scale_inv_shape[1]);
    NVTE_CHECK(colwise_shape[0] == colwise_scale_inv_shape[1], "num of cols of colwise_data must be equal to num of rows of colwise_scale_inv, got colwise_shape[0]:", colwise_shape[0],", colwise_scale_inv_shape[1]:", colwise_scale_inv_shape[1]);
    ...
    if(hidden_dim % 4 == 0 && seq_len % 4 == 0){
        TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(itype, FP8_TYPE,
            transformer_engine::detail::dequantize_cast_transpose_1x128_aligned_kernel<FP8_TYPE>
                <<<grid, block,0,stream>>>(
                reinterpret_cast<FP8_TYPE*>(rowwise_data.dptr),
