...
void cast_transpose_dbias(const Tensor &input, Tensor *cast_output, Tensor *transposed_output,
...
void cast_transpose_dbias_dgelu(const Tensor &input, const Tensor &gelu_input, Tensor *cast_output,
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          ...
          Param param; param.input = reinterpret_cast<const InputType *>(input.data.dptr);
          ...
          param.amax = reinterpret_cast<ComputeType *>(cast_output->amax.dptr);
          param.scale_inv = reinterpret_cast<ComputeType *>(cast_output->scale_inv.dptr);
          param.workspace = reinterpret_cast<ComputeType *>(workspace->data.dptr); if (full_tile) {
...
void nvte_cast_transpose_dbias(const NVTETensor input, NVTETensor cast_output,
  ...
  cast_transpose_dbias(
      *reinterpret_cast<const Tensor *>(input), reinterpret_cast<Tensor *>(cast_output),
      reinterpret_cast<Tensor *>(transposed_output), reinterpret_cast<Tensor *>(dbias),
