...
__global__ void
__launch_bounds__(cast_transpose_num_threads)
cast_transpose_dgated_gelu_kernel(const IType * const input,
...
__global__ void
__launch_bounds__(cast_transpose_num_threads)
cast_transpose_dgated_gelu_kernel_notaligned(const IType * const input,
                                             const IType * const gelu_input,
                                             OType * const output_c,
                                             OType * const output_t,
                                             const CType * const scale_ptr,
  ...
  const size_t output_stride = num_rows / nvec_out;
  const size_t row_length_rest = stride - tile_id_x * THREADS_PER_WARP;
  const size_t row_length_rest2 = stride2 - tile_id_x * THREADS_PER_WARP;
  const size_t row_height_rest = output_stride - tile_id_y * THREADS_PER_WARP;
  const unsigned int tile_length = row_length_rest > THREADS_PER_WARP ? THREADS_PER_WARP
                                                                      : row_length_rest;
  const unsigned int tile_height = row_height_rest > THREADS_PER_WARP ? THREADS_PER_WARP
...
void cast_transpose_dgated_gelu(const Tensor &input,
                                const Tensor &gelu_input,
                                const Tensor &scale,
                                Tensor *cast_output,
                                Tensor *transposed_output,
                                ...
                                cudaStream_t stream) {
  NVTE_CHECK(input.shape.size() == 2, "Input must have 2 dimensions.");
  NVTE_CHECK(gelu_input.shape.size() == 2, "Input must have 2 dimensions.");
  NVTE_CHECK(cast_output->shape.size() == 2, "C output must have 2 dimensions.");
  NVTE_CHECK(transposed_output->shape.size() == 2,
             "T output must have 2 dimensions.");
  const size_t row_length = input.shape[1];
  ...
  NVTE_CHECK(gelu_input.shape[0] == num_rows, "Wrong dimension of output.");
  NVTE_CHECK(gelu_input.shape[1] == row_length * 2, "Wrong dimension of output.");
  NVTE_CHECK(cast_output->shape[0] == num_rows, "Wrong dimension of output.");
  NVTE_CHECK(cast_output->shape[1] == row_length * 2, "Wrong dimension of output.");
  NVTE_CHECK(transposed_output->shape[0] == row_length * 2, "Wrong dimension of T output.");
  NVTE_CHECK(transposed_output->shape[1] == num_rows, "Wrong dimension of T output.");
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(input.dtype, InputType,
    TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(cast_output->dtype, OutputType,
      ...
      const bool full_tile = row_length % (nvec_in * THREADS_PER_WARP) == 0 &&
                             num_rows % (nvec_out * THREADS_PER_WARP) == 0;
      if (full_tile) {
        cudaFuncSetAttribute(cast_transpose_dgated_gelu_kernel<nvec_in, nvec_out, fp32,
                                                   InputType, OutputType>,
                             cudaFuncAttributePreferredSharedMemoryCarveout,
                             100);
        cast_transpose_dgated_gelu_kernel<nvec_in, nvec_out, fp32, InputType, OutputType>
            <<<n_blocks,
               cast_transpose_num_threads,
               ...
               stream>>>(
                ...
                reinterpret_cast<const InputType *>(gelu_input.dptr),
        ...
        cudaFuncSetAttribute(cast_transpose_dgated_gelu_kernel_notaligned<nvec_in, nvec_out, fp32,
                                                              InputType, OutputType>,
                             cudaFuncAttributePreferredSharedMemoryCarveout,
                             100);
        cast_transpose_dgated_gelu_kernel_notaligned<nvec_in, nvec_out, fp32, InputType, OutputType>
            <<<n_blocks,
               cast_transpose_num_threads,
               cast_transpose_num_threads / n_warps_per_tile *
               (THREADS_PER_WARP + 1) * sizeof(Vec<OutputType, nvec_out>),
...
void nvte_cast_transpose_dgated_gelu(const NVTETensor input,
                                     ...
                                     cudaStream_t stream) {
  using namespace transformer_engine;
  cast_transpose_dgated_gelu(*reinterpret_cast<const Tensor*>(input),
                             *reinterpret_cast<const Tensor*>(gelu_input),
                             *reinterpret_cast<const Tensor*>(scale),
                             reinterpret_cast<Tensor*>(cast_output),
                             reinterpret_cast<Tensor*>(transposed_output),
