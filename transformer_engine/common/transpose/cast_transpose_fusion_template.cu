...
template <bool IS_DBIAS, bool IS_DACT,
          typename ComputeType, typename ParamOP,
...
void cast_transpose_fused(const Tensor &input,
    ...
    NVTE_CHECK(input.data.shape == cast_output->data.shape, "Input and C output must have the same shape.");
    ...
    TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(input.data.dtype, InputType,
        TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(cast_output->data.dtype, OutputType,
            ...
            constexpr int desired_load_size_dact  = (IS_DACT ? 4 : 8);
            constexpr int desired_store_size_dact = (IS_DACT ? 4 : 8);
