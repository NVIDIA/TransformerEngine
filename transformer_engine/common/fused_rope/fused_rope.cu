...
#pragma unroll
  for (int d_id = threadIdx.x; d_id < d2; d_id += blockDim.x) {
    ...
    float v_sin =
        (d_id + d2 / 2 < d2)
            ? sinf(freqs[s_id * freqs_stride_s + b_id * freqs_stride_b + d_id + d2 / 2])
...
__global__ void fused_rope_forward_kernel(
    const scalar_t *src, const float *freqs, const int *start_positions, scalar_t *dst, const int h,
    const int d, const int d2, const int stride_s, const int stride_b, const int stride_h,
    const int stride_d, const int freqs_stride_s, const int freqs_stride_b, const int o_stride_s,
  ...
  fused_rope_block_forward(src, freqs, dst, s_id, offset_block, offset_block_dst, h, d, d2,
                           stride_h, stride_d, freqs_stride_s, freqs_stride_b, o_stride_h,
                           o_stride_d);
}
...
__global__ void fused_rope_thd_forward_kernel(const scalar_t *src, const int *cu_seqlens,
                                              ...
                                              scalar_t *dst, const int cp_size, const int cp_rank,
...
void fused_rope_forward_launcher(const scalar_t *input, const float *freqs,
                                 ...
                                 const int freqs_stride_b, const int o_stride_s,
                                 const int o_stride_b, const int o_stride_h, const int o_stride_d,
                                 cudaStream_t stream) {
...
void fused_rope_thd_forward_launcher(const scalar_t *input, const int *cu_seqlens,
                                     const float *freqs, const int *start_positions,
                                     scalar_t *output, const int cp_size, const int cp_rank,
  ...
  fused_rope_thd_forward_kernel<<<blocks, threads, 0, stream>>>(
      input, cu_seqlens, freqs, start_positions, output, cp_size, cp_rank, h, d, d2, stride_t,
...
void fused_rope_thd_backward_launcher(const scalar_t *output_grads, const int *cu_seqlens,
                                      const float *freqs, const int *start_positions,
                                      scalar_t *input_grads, const int cp_size, const int cp_rank,
                                      const int max_s, const int b, const int h, const int d,
                                      const int d2, const int stride_t, const int stride_h,
                                      const int stride_d, const int o_stride_t,
                                      const int o_stride_h, const int o_stride_d,
  ...
  fused_rope_thd_backward_kernel<<<blocks, threads, 0, stream>>>(
      output_grads, cu_seqlens, freqs, start_positions, input_grads, cp_size, cp_rank, h, d, d2,
...
void fused_rope_forward(const Tensor &input, const Tensor &freqs, const Tensor &start_positions,
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      fused_rope_forward_launcher(reinterpret_cast<const scalar_t *>(input.data.dptr),
                                  ...
                                  stride_s, stride_b, stride_h, stride_d, freqs_stride_s,
                                  freqs_stride_b, o_stride_s, o_stride_b, o_stride_h, o_stride_d,
                                  stream););
}
...
void fused_rope_backward(const Tensor &output_grads, const Tensor &freqs,
                         ...
                         const int freqs_stride_s, const int freqs_stride_b, const int o_stride_s,
                         const int o_stride_b, const int o_stride_h, const int o_stride_d,
                         cudaStream_t stream) {
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      fused_rope_backward_launcher(reinterpret_cast<const scalar_t *>(output_grads.data.dptr),
                                   ...
                                   d2, stride_s, stride_b, stride_h, stride_d, freqs_stride_s,
                                   freqs_stride_b, o_stride_s, o_stride_b, o_stride_h, o_stride_d,
                                   stream););
}
...
void fused_rope_thd_backward(const Tensor &output_grads, const Tensor &cu_seqlens,
                             ...
                             Tensor *input_grads, const int cp_size, const int cp_rank,
...
void nvte_fused_rope_forward(const NVTETensor input, const NVTETensor freqs,
  ...
  fused_rope_forward(
      *reinterpret_cast<const Tensor *>(input), *reinterpret_cast<const Tensor *>(freqs),
      *reinterpret_cast<const Tensor *>(start_positions), reinterpret_cast<Tensor *>(output), s, b,
      h, d, d2, stride_s, stride_b, stride_h, stride_d, freqs_stride_s, freqs_stride_b, o_stride_s,
...
void nvte_fused_rope_backward(const NVTETensor output_grads, const NVTETensor freqs,
  ...
  using namespace transformer_engine;
  fused_rope_backward(
      *reinterpret_cast<const Tensor *>(output_grads), *reinterpret_cast<const Tensor *>(freqs),
      *reinterpret_cast<const Tensor *>(start_positions), reinterpret_cast<Tensor *>(input_grads),
      s, b, h, d, d2, stride_s, stride_b, stride_h, stride_d, freqs_stride_s, freqs_stride_b,
...
void nvte_fused_rope_thd_forward(const NVTETensor input, const NVTETensor cu_seqlens,
  ...
  fused_rope_thd_forward(
      ...
      *reinterpret_cast<const Tensor *>(freqs), *reinterpret_cast<const Tensor *>(start_positions),
      reinterpret_cast<Tensor *>(output), cp_size, cp_rank, max_s, b, h, d, d2, stride_t, stride_h,
      stride_d, o_stride_t, o_stride_h, o_stride_d, stream);
}
...
void nvte_fused_rope_thd_backward(const NVTETensor output_grads, const NVTETensor cu_seqlens,
  ...
  fused_rope_thd_backward(
      ...
      *reinterpret_cast<const Tensor *>(start_positions), reinterpret_cast<Tensor *>(input_grads),
      cp_size, cp_rank, max_s, b, h, d, d2, stride_t, stride_h, stride_d, o_stride_t, o_stride_h,
      o_stride_d, stream);
}
