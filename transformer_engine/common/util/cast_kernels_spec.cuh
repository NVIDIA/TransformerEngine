...
__device__ __forceinline__
void mul_cvt_4x(fp8e4m3x4 &out, floatx4 &in, const floatx4 &scale) {
    ptx::floatx2 * in2 = reinterpret_cast<ptx::floatx2 *>(&in);
    ptx::floatx2 const * scale2 = reinterpret_cast<ptx::floatx2 const*>(&scale);
    asm volatile (
        "{\n\t"
...
__global__ void cast_mxfp8_kernel(
    ...
    for (int32_t iter = 0; iter < CastTraits::iterLayout::num; iter++) {
        ...
        if constexpr (CastTraits::_colwise_source_coming_from_rowwise) {
            ...
            if constexpr (std::is_same_v<typename CastTraits::IType, float>) {
                ...
                {
                    typename CastTraits::rowOutputUnitType rOutput[CastTraits::rowNumOutUnitsPerChunk];
                    ptx::floatx2 block_scale_inverse_2{row_scale_inverse, row_scale_inverse};
                    if constexpr (CastTraits::_use_cvt_4x) {
                        using OType4 = FPx4<typename CastTraits::OType>;
                        using IType4 = FPx4<typename CastTraits::IType>;
                        IType4 *rInput4 = reinterpret_cast<IType4 *>(&rInput);
                        ...
                        for (int32_t i = 0; i < CastTraits::rowChunkElems / 4; i++) {
                            IType4 in = rInput4[i];
                            OType4 out;
                            mul_cvt_4x(out, in, block_scale_inverse_2);
                            rOutput4[i] = out;
                        }
                    } else {
                        IType2 *rInput2 = reinterpret_cast<IType2 *>(&rInput);
                        ...
                        for (int32_t i = 0; i < CastTraits::rowChunkElems / 2; i++) {
                            IType2 in = rInput2[i];
                            OType2 out;
                            ptx::mul_cvt_2x(out, in, block_scale_inverse_2);
                            rOutput2[i] = out;
                        }
                    }
                    {
                ...
                // colwise
                {
                    ...
                    if constexpr (CastTraits::_use_cvt_4x) {
                        floatx4 block_scale_inverse_4x[2];
                        floatx4 *sColwiseScale4x = reinterpret_cast<floatx4 *>(sColwiseReduce);
                        block_scale_inverse_4x[0] = sColwiseScale4x[0];
                        ...
                        OType4 *rOutput4 = reinterpret_cast<OType4 *>(&rOutput);
                        #pragma unroll
                        for (int32_t i = 1; i < CastTraits::colChunkElems / 4; i++) {
                            {
                                block_scale_inverse_4x[i % 2] = 
                                    sColwiseScale4x[i];
                            }
                            IType4 in = rInput4[i - 1];
                            OType4 out;
                            mul_cvt_4x(out, in, block_scale_inverse_4x[(i - 1) % 2]);
                            rOutput4[i - 1] = out;
                        }
                        {
                            constexpr int32_t i = (CastTraits::colChunkElems / 4) - 1;
                            IType4 in = rInput4[i];
                            OType4 out;
                            mul_cvt_4x(out, in, block_scale_inverse_4x[i % 2]);
                            rOutput4[i] = out;
                        }
                    } else {
                        ptx::floatx2 block_scale_inverse_2x[2];
                        ptx::floatx2 *sColwiseScale2x = reinterpret_cast<ptx::floatx2 *>(sColwiseReduce);
                        block_scale_inverse_2x[0] = sColwiseScale2x[0];

                        IType2 *rInput2 = reinterpret_cast<IType2 *>(&rInput);
                        OType2 *rOutput2 = reinterpret_cast<OType2 *>(&rOutput);
                        #pragma unroll
                        for (int32_t i = 1; i < CastTraits::colChunkElems / 2; i++) {
                            {
                                block_scale_inverse_2x[i % 2] = 
                                    sColwiseScale2x[i];
                            }

                            IType2 in = rInput2[i - 1];
                            OType2 out;
                            ptx::mul_cvt_2x(out, in, block_scale_inverse_2x[(i - 1) % 2]);
                            rOutput2[i - 1] = out;
                        }
                        {
                            constexpr int32_t i = (CastTraits::colChunkElems / 2) - 1;
                            IType2 in = rInput2[i];
                            OType2 out;
                            ptx::mul_cvt_2x(out, in,
                                block_scale_inverse_2x[i % 2]);
                            rOutput2[i] = out;
                        }
                    }

                    {
                        int32_t base = thread_base_offset / (CastTraits::rowOutNumElemsPerUnit / CastTraits::rowNumElemsPerUnit)
                            + warp_offset / CastTraits::rowOutNumElemsPerUnit;
                        typename CastTraits::rowOutputUnitType *rOutputUnit = 
                            reinterpret_cast<typename CastTraits::rowOutputUnitType *>(rOutput);
                        #pragma unroll
                        for (int32_t i = 0; i < CastTraits::rowNumOutUnitsPerChunk; i++) {
                            sColOutputUnit[CastTraits::rowOutputChunkSwz::swz(base + i)] = rOutputUnit[i];
                        }
                    }
                }
    ...
    if constexpr (CastTraits::_cache_rowwise_scale_in_smem) {
        ...
        using DataType = std::conditional_t<stride_in_smem % 16 == 0, uint4,
        ...
        constexpr int32_t stride_in_group = stride_in_smem / num_elems_per_group;
        constexpr int32_t num_groups = CastTraits::blockDIM::M * stride_in_group;
        constexpr int32_t needed_warps = num_groups / 32;
        constexpr int32_t actual_warps = std::min(needed_warps, CastTraits::warpLayout::num);
        constexpr int32_t warp_iters = std::min(needed_warps / CastTraits::warpLayout::num, 1);
        constexpr int32_t num_rows_per_warp = 32 / stride_in_group;

        int2 coords_base;
        coords_base.y = (threadIdx.x / stride_in_group)
                        + warpId * num_rows_per_warp;
        coords_base.x = (threadIdx.x % stride_in_group);

        int32_t gmem_offset_base = block_coords.y * (scale_stride_rowwise / num_elems_per_group)
                                   + block_coords.x / CastTraits::rowChunkElems / num_elems_per_group;

        int32_t current_warp_iters = (warpId < needed_warps) * warp_iters;
        #pragma unroll
        for (int32_t iter = 0; iter < warp_iters; iter++) {
            if (iter >= current_warp_iters) { break; }

            int32_t coords_y = coords_base.y + iter * (actual_warps * num_rows_per_warp);
            int32_t smem_offset = coords_y * stride_in_group + coords_base.x;
            int32_t gmem_offset = gmem_offset_base
                + coords_y * (scale_stride_rowwise / num_elems_per_group) + coords_base.x;

            reinterpret_cast<DataType *>(scales_rowwise)[gmem_offset] =
                reinterpret_cast<DataType *>(sRowwiseScale)[smem_offset];
        }
    }
