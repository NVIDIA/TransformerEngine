...
#define DISPATCH_AIR_TOPK(T, d_in_cast, d_out_cast)                                             \
  do {                                                                                          \
    const T *d_in = reinterpret_cast<const T *>(keys_in_tensor->data.dptr);                     \
    T *d_out = reinterpret_cast<T *>(keys_out_tensor->data.dptr);                               \
    nv::standalone_air_topk<T, int>(d_workspace, workspace_bytes, d_in, batch_size, seq_len, k, \
                                    d_out, d_indices, /*greater=*/true, stream,                 \
                                    const_cast<int *>(d_lengths), /*is_prefill=*/false);        \
  } while (0)

  if (dtype == DType::kBFloat16) {
    DISPATCH_AIR_TOPK(__nv_bfloat16, , );
  } else if (dtype == DType::kFloat32) {
    DISPATCH_AIR_TOPK(float, , );
  } else {
    NVTE_ERROR("nvte_air_topk: unsupported key dtype (supported: float32, bfloat16)");
  }

#undef DISPATCH_AIR_TOPK
}
...
size_t nvte_get_air_topk_workspace_bytes(int batch_size, int seq_len, int k) {
  ...
  size_t buf_size = 0;
  nv::standalone_air_topk<float, int>(nullptr, buf_size, nullptr, batch_size, seq_len, k, nullptr,
