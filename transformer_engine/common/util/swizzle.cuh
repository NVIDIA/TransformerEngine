...
struct Swizzle {
    ...
    using yyy_mask = constant<int32_t, bit_mask{} << (num_base + std::max(decltype(num_shft){0}, num_shft))>;
    using zzz_mask = constant<int32_t, bit_mask{} << (num_base - std::min(decltype(num_shft){0}, num_shft))>;
    ...
    constexpr static int32_t apply(Offset const &offset) {
        return offset ^ shiftr<Offset, typename msk_shft::value_type, msk_shft::value>(offset & yyy_mask{});
    }
