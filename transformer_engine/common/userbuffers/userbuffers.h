...
/*
  Registers GPU buffers and returns handler. Assumed to be collective, i.e. you use same groups and
  do not mix buffers for different operations. Returns -1 if cant register (too many preregistered
  regions already). Will allocate memory and fill the pointers if alloc == true (required for NVL
  SHARP and NSO/MNNVL).
*/
...
/*
  Producer/consumer switches for atomic GEMM counters
*/
...
// Helper function to walk across allreduce1 x allreduce2 groups. Preserves data-parallel and
// tensor-parallel positions within data and tensor groups.
...
/*
  ...
  Two matching pairs, intended to work as push from sender or pull by receiver. In either case, the
  signal is a write by sender.
  - Push model: data arrived and visible at receiver (barrier enforced)
  - Pull model: data ready to be pulled by receiver (no barrier enforced)
*/
...
/*
  ...
  - Recv returns once data has been received.
...
/*
  Device-local FP8 sum-reduce kernel. Used in conjunction with point-to-point communication calls
  when the DL framework does not natively support FP8 operations (e.g. PyTorch).
*/
