  ...
  UbufExecutorBase(
    ...
    if (!_comm_created) {
      ...
      create_communicator_grouped2(&_ub_comm,
        ...
        [this](void *sendbuf, size_t sendbytes, void *recvbuf, size_t recvbytes, const char *group) {
          _ub_allgather(sendbuf, sendbytes, recvbuf, recvbytes, group);
        },
  ...
  */
  void atomic_gemm_overlap_rs(cudaStream_t stream_main,
    ...
    nvte_cublas_atomic_gemm(A.data(), B.data(), output_d.data(), bias.data(), pre_gelu_out.data(),
                            A_trans, B_trans, grad, workspace_chunk.data(),
