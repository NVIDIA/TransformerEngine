...
from transformer_engine.common.attention.cache_debug import time_call
...
def build_cudnn_graph(
    ...
    *,
    ...
    if debug_callback is not None:
        debug_callback("CREATE_GRAPH", 0)
    time_call(debug_callback, "validate", graph.validate)
    time_call(debug_callback, "build_operation_graph", graph.build_operation_graph)
    try:
        time_call(
            debug_callback,
    ...
    time_call(
        debug_callback,
        "build_plans",
        lambda: graph.build_plans(cudnn.build_plan_policy.HEURISTICS_CHOICE),
    )
    if debug_callback is not None:
        debug_callback("BUILD_PLANS", 0)
