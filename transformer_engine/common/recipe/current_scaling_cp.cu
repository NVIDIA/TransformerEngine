...
__launch_bounds__(amax_kernel_threads) __global__
    void amax_kernel(const InputType *input_a, 
                     const InputType *input_b, 
                     const InputType *input_c,
  ...
  // Grid-Stride Loop
  for (size_t tid_base = blockIdx.x * blockDim.x; tid_base < M; tid_base += gridDim.x * blockDim.x) {
    ...
    if (tid < N / nvec) { // 同样的边界保护
        ...
        for (int i = 0; i < nvec; ++i) {
            ...
            // 计算 A + B - C
            // 转为 float 计算以防溢出
            float res_f = static_cast<float>(val_a) + static_cast<float>(val_b) - static_cast<float>(val_c);
...
template <int nvec, typename InputType>
void launch_amax_kernel(const InputType *input0,const InputType *input1,const InputType *input2,InputType *output, float *amax, const size_t N, const float *noop_ptr,
                        cudaStream_t stream) {
  // Zero out amax so we can update with atomic max
  zero_amax_kernel<<<1, 1, 0, stream>>>(amax, noop_ptr);
  NVTE_CHECK_CUDA(cudaGetLastError());
  ...
  switch (align) {
    case Alignment::SAME_ALIGNED:
      amax_kernel<nvec, true, InputType>
          <<<num_blocks, threads, 0, stream>>>(input0,input1,input2,output, amax, N, num_aligned_elements, noop_ptr);
      break;
    case Alignment::SAME_UNALIGNED:
      amax_kernel<nvec, false, InputType>
          <<<num_blocks, threads, 0, stream>>>(input0,input1,input2,output, amax, N, num_aligned_elements, noop_ptr);
    ...
    case Alignment::DIFFERENT: {
      ...
      // in the alignment check. Still safe to process without vectorization.
      amax_kernel<1, true, InputType>
          <<<num_blocks, threads, 0, stream>>>(input0,input1,input2,output, amax, N, N, noop_ptr);
      break;
    }
  }
...
void verify_results(float* h_a, float* h_b, float* h_c, float* h_out, float gpu_amax, int N) {
    ...
    std::cout << "Max Output Error: " << max_error << (max_error < epsilon ? " [PASS]" : " [FAIL]") << std::endl;
...
int main() {
    ...
    TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      t, IType, constexpr int nvec = 32 / sizeof(IType); launch_amax_kernel<nvec>(
          reinterpret_cast<const IType *>(d_a),reinterpret_cast<const IType *>(d_b),reinterpret_cast<const IType *>(d_c),reinterpret_cast<IType *>(d_out), d_amax, N, nullptr,
          stream););  // NOLINT(*)
    ...
    cudaFree(d_out); cudaFree(d_amax); cudaFree(d_noop);
