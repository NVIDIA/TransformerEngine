...
static_assert(PACK_SIZE == 2);  // loads a pair of elements (4 Bytes of 2x BF16, i.e. 1x SMEM bank per load)
...
compute_bwd_gated_swiglu_tuned_ptx(IType2& out_act,
                                   IType2& out_gate,
  ...
  if constexpr (std::is_same<IType2, typename ptx::bf16x2>::value) {
    asm volatile (
      ...
      "}\n"
      : "=r"(reinterpret_cast<uint32_t&>(out_act)),
        "=r"(reinterpret_cast<uint32_t&>(out_gate)),
...
template <typename IType2>
__device__ __forceinline__ void
compute_bwd_gated_swiglu(IType2& out_act,
                         IType2& out_gate,
                         IType2& out_xormax_act,
                         IType2& out_xormax_gate,
                         const float2 in_act,
...
template <typename IType2, typename ParamOP,
          float (*ActOP)(float, const ParamOP &),
          float (*DActOP)(float, const ParamOP &)>
__device__ __forceinline__ void
compute_bwd_gated_activation(IType2& out_act,
                             IType2& out_gate,
                             IType2& out_xormax_act,
                             IType2& out_xormax_gate,
                             const float2 in_act,
  ...
  if constexpr (!IS_CLAMPED_SWIGLU) {
    if constexpr ((ActOP == &silu<fp32, fp32>) && (DActOP == &dsilu<fp32, fp32>)) {
      compute_bwd_gated_swiglu<IType2>(out_act, out_gate, out_xormax_act, out_xormax_gate,
                                       in_act, in_gate, in_grad);
      return;
    }
  }
...
__device__ __forceinline__ void
compute_fwd_gated_activation(IType2& out_act,
                             IType2& out_xormax_act,
                             const float2 in_act,
                             const float2 in_gate,
                             const ParamOP p) {
...
__global__ void __launch_bounds__(THREADS_NUM)
    quantize_gated_mxfp8_rowwise_kernel(const __grid_constant__ CUtensorMap tensor_map_grad,
  ...
  OType *out_gate_rowwise_sh_ptr = reinterpret_cast<OType *>(dshmem + grad_mem + in_mem + out_act_mem);
  ...
  for (size_t stage = 0; stage < PREFETCH_STAGES; ++stage) {
    ...
    if (leading_thread) {
      ...
      ptx::cp_async_bulk_tensor_2d_global_to_shared(dst_act, src_act, global_offset_X, global_offset_Y, barrier);
      ptx::cp_async_bulk_tensor_2d_global_to_shared(dst_gate, src_gate, global_offset_X, global_offset_Y, barrier);
      ...
      if constexpr (IS_BWD) {
        ...
        ptx::cp_async_bulk_tensor_2d_global_to_shared(dst_grad, src_grad, global_offset_X, global_offset_Y, barrier);
      }
    }
  }
  ...
  while (!job_finished) {
      ...
      for (int it = 0; it < ITERS; ++it) {
        ...
        for (int w = 0; w < WAVES; ++w) {
          ...
          if constexpr (IS_BWD) {
            ...
            compute_fwd_gated_activation<IType2, ParamOP, ActOP>
              (ACT[w], xormax_act, act_elt, gate_elt, p);
          }
        }
    ...
    if (aligned_SFs) {
      ...
      for (int it = 0; it < iters; ++it) {
        ...
        const size_t scale_idx = scale_idx_Y * scale_stride_rowwise + scale_idx_X + channel * gate_scale_idx_offset;
...
void quantize_gated_rowwise(const Tensor &grad, const Tensor &gated_input, Tensor *output,
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_16BIT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(
          ...
          create_2D_tensor_map(tensor_map_output_act, output->data, rows, cols,
                               BUFF_DIM_Y, BUFF_DIM_X, tensor_stride_elems, 0,
                               output_type_bit_size);
          create_2D_tensor_map(tensor_map_output_gate, output->data, rows, cols,
          ...
          auto kernel = quantize_gated_mxfp8_rowwise_kernel<IS_BWD, ParamOP, ActOP, DActOP, IType, OType>;
          ...
          kernel<<<grid, block_size, shmem_size, stream>>>(
              ...
              tensor_map_output_act, tensor_map_output_gate,
              scales_ptr, rows, cols, scale_stride, p);
