...
#pragma unroll
    for (int i = 0; i < SCALE_DIM_Y; i += 4) {
      ...
      const uint32_t dst_smem_ptr =
          __cvta_generic_to_shared(&out_colwise_data_sh[shmem_offset_elt]);
...
#pragma unroll
  ...
  __global__ void __launch_bounds__(THREADS_PER_CHUNK) group_quantize_mxfp8_kernel(
      ...
      const int64_t *const __restrict__ first_dims_ptr,
      const int64_t *const __restrict__ last_dims_ptr,
      e8m0_t *const __restrict__ scales_rowwise_ptr, e8m0_t *const __restrict__ scales_colwise_ptr,
      const float *__restrict__ noop, float *const __restrict__ dbias_workspace,
      float *const __restrict__ amax_ptr, const size_t work_blocks_X, const size_t work_blocks_Y) {
...
#pragma unroll
    ...
    while (!job_finished) {
      ...
      const JobDescriptor current_job =
          decode_job<SHAPE_REP>(num_tensors, first_logical_dim, last_logical_dim, work_blocks_X,
...
#pragma unroll
      for (int stage = 0; stage < PREFETCH_STAGES; ++stage) {
        ...
        prefetch_input_stage<IType, IS_DACT>(in_sh, act_in_sh, tensor_map_input,
      ...
      for (int stage = 0; stage < STAGES; ++stage) {
        ...
        if (stage < STAGES - PREFETCH_STAGES) {
          ...
          prefetch_input_stage<IType, IS_DACT>(
              in_sh, act_in_sh, tensor_map_input, tensor_map_act_input, global_offset_X,
        ...
        if constexpr (ROWWISE_SCALING) {
          thread_amax =
              process_rowwise_stage<IS_DBIAS, IS_DACT, IS_ACT, ParamOP, OP, IType, OType,
                  ...
                  buff, stage_offset_Y, thread_offset_Y_rowwise, thread_offset_X_rowwise,
                  bank_group, scales_offset_Y_rowwise, scales_offset_X_rowwise,
                  scale_stride_rowwise, rowwise_scale_is_within_bounds, cols, in_sh, act_in_sh,
                  cached_act_sh, out_rowwise_data_sh, scales_rowwise, thread_dbias_rowwise);
        }
        ...
        store_output_stage<OType, ROWWISE_SCALING, COLWISE_SCALING>(
            ...
            tensor_map_output_colwise, global_offset_X, global_offset_Y, buff_offset,
            leading_thread);
                  ...
                  TRANSFORMER_ENGINE_GROUP_TENSOR_SHAPE_REPRESENTATION_SWITCH(
                      ...
                      TRANSFORMER_ENGINE_SWITCH_CONDITION(
                          use_fast_math, USE_FAST_MATH,
                          {
                            ...
                            if (use_colwise_scaling) {
                              create_2D_tensor_map(tensor_map_output_colwise,
                                                   output->columnwise_data, first_logical_dim,
                                                   last_logical_dim, BUFF_DIM_Y, BUFF_DIM_X,
                            ...
                            auto kernel = group_quantize_mxfp8_kernel<
                                IS_DBIAS, IS_DACT, IS_ACT, ParamOP, OP, IType, OType, SCALING_TYPE,
                                WITH_GEMM_SWIZZLED_SCALES, SHAPE_REP, USE_FAST_MATH>;
