...
struct CastTraitsImpl : tuned_1D_scaling_common::KernelTraits<Config> {
  ...
  static constexpr int STATIC_PERSISTENT_BLOCKS_PER_SM = Config::STATIC_PERSISTENT_BLOCKS_PER_SM;
...
struct WorkProvider {
  ...
  __device__ __forceinline__ WorkProvider(const size_t num_tensors, const int common_rows,
    ...
    if constexpr (FIXED_X_DIM) {
      ...
      launch_block_id_ =
          (static_cast<size_t>(blockIdx.z) * gridDim.y + blockIdx.y) * gridDim.x + blockIdx.x;
  ...
  __device__ __forceinline__ void prepare_next(bool &job_finished, int &prefetch_block_offset_Y,
    ...
    if constexpr (FIXED_X_DIM) {
      ...
      if (!job_finished) {
        ...
        const size_t prefetch_block_id_X =
            next_tile_id_ - prefetch_block_id_Y * blocks_X_per_tensor_;
...
__global__ void __launch_bounds__(CastTraits<SHAPE_REP>::THREADS_NUM)
    group_quantize_transpose_nvfp4_tuned_1D_kernel(
        const size_t num_tensors, nvfp4_scale_t *const scales_ptr,
        ...
        const size_t amax_colwise_numel, const int common_rows, const int common_cols,
        const int common_scale_stride, const int common_scale_stride_t,
        const size_t common_blocks_Y_per_tensor, const size_t *rng_state) {
  ...
  nvfp4_scale_t *const scales_colwise =
      RETURN_TRANSPOSE ? (scales_t_ptr + work.colwise_scale_base()) : nullptr;
...
inline void launch_group_quantize_transpose_kernel(
    ...
    const size_t amax_rowwise_numel, const size_t amax_colwise_numel, const size_t work_blocks_X,
    const size_t work_blocks_Y, const size_t *const rng_state, const int dshmem_size,
    cudaStream_t stream) {
  using ScalingTraits = CastTraits<SHAPE_REP>;
  constexpr int CHUNK_DIM_Y = ScalingTraits::CHUNK_DIM_Y;
  constexpr int THREADS_NUM = ScalingTraits::THREADS_NUM;
  ...
  auto kernel = group_quantize_transpose_nvfp4_tuned_1D_kernel<SHAPE_REP, USE_STOCHASTIC_ROUNDING,
                                                               USE_FAST_MATH, RETURN_TRANSPOSE>;
  NVTE_CHECK_CUDA(
      cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, dshmem_size));

  int active_blocks_per_sm = 0;
  NVTE_CHECK_CUDA(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, kernel,
                                                                block_size, dshmem_size));
  NVTE_CHECK(active_blocks_per_sm > 0,
             "Grouped NVFP4 optimized kernel has zero active blocks per SM.");
  ...
  if constexpr (SHAPE_REP == ShapeRepresentation::SAME_BOTH_DIMS) {
    ...
    const int scale_stride =
        DIVUP_TO_MULTIPLE(DIVUP(last_logical_dim, static_cast<size_t>(NVFP4_SCALE_DIM)), 4);
    const int scale_stride_t =
        DIVUP_TO_MULTIPLE(DIVUP(rows_per_tensor, static_cast<size_t>(NVFP4_SCALE_DIM)), 4);
    ...
    const size_t requested_workers_Y_per_tensor =
        std::max<size_t>(size_t{1}, requested_grid_size / workers_X_total);
    const size_t workers_Y_per_tensor =
        std::min(blocks_Y_per_tensor, requested_workers_Y_per_tensor);
    ...
    const size_t requested_workers_Y_per_tensor =
        std::max<size_t>(size_t{1}, requested_grid_size / workers_X_total);
    const size_t avg_blocks_Y_per_tensor =
        std::max<size_t>(size_t{1}, DIVUP(work_blocks_Y, num_tensors));
    const size_t workers_Y_per_tensor =
        std::min(avg_blocks_Y_per_tensor, requested_workers_Y_per_tensor);
    NVTE_CHECK(workers_Y_per_tensor > 0,
               "VARYING_FIRST_DIM persistent grid size must be greater than zero.");
    const dim3 grid(work_blocks_X, workers_Y_per_tensor, num_tensors);
    ...
    const size_t requested_workers_per_tensor =
        std::max<size_t>(size_t{1}, (sm_num * persistent_blocks_per_sm) / num_tensors);
    const size_t workers_per_tensor = std::min(total_work_blocks, requested_workers_per_tensor);
    NVTE_CHECK(workers_per_tensor > 0,
               "Tensor-local persistent grid size must be greater than zero.");
...
inline void group_quantize_transpose(const GroupedTensor *input, const Tensor *noop,
  ...
  TRANSFORMER_ENGINE_GROUP_TENSOR_SHAPE_REPRESENTATION_SWITCH(shape_rep, SHAPE_REP, {
    ...
    constexpr int BUFF_SIZE_ROWWISE_SCALES = ActiveCastTraits::BUFF_SIZE_ROWWISE_SCALES;
    constexpr int BUFF_SIZE_COLWISE_SCALES = ActiveCastTraits::BUFF_SIZE_COLWISE_SCALES;
    ...
    if constexpr (SHAPE_REP == ShapeRepresentation::SAME_BOTH_DIMS) {
      ...
      NVTE_CHECK((first_logical_dim / num_tensors) % CHUNK_DIM_Y == 0,
                 "First dimension of each tensor in a group must be divisible by ", CHUNK_DIM_Y,
                 ".");
    } else if constexpr (SHAPE_REP == ShapeRepresentation::VARYING_FIRST_DIM) {
      NVTE_CHECK(first_logical_dim % CHUNK_DIM_Y == 0,
                 "First logical dimension of a grouped tensor must be divisible by ", CHUNK_DIM_Y,
                 ".");
      ...
      NVTE_CHECK(last_logical_dim % CHUNK_DIM_Y == 0,
                 "Last logical dimension of a grouped tensor must be divisible by ", CHUNK_DIM_Y,
                 ".");
    } else {
    ...
    update_tma_descriptors<IType, void, true><<<num_tensors, 1, 0, stream>>>(
        ...
        input_dptr, nullptr, output_dptr, output_t_dptr, shape_rep, num_tensors, first_logical_dim,
        last_logical_dim, offsets_ptr, first_dims_ptr, last_dims_ptr, true, return_transpose,
        false);
    NVTE_CHECK_CUDA(cudaGetLastError());
        ...
        TRANSFORMER_ENGINE_SWITCH_CONDITION(
            ...
            TRANSFORMER_ENGINE_SWITCH_CONDITION(return_transpose, RETURN_TRANSPOSE, {
              launch_group_quantize_transpose_kernel<SHAPE_REP, USE_STOCHASTIC_ROUNDING,
                  ...
                  amax_colwise_numel, work_blocks_X, work_blocks_Y, rng_state, dshmem_size, stream);
            });););
  });
#else
