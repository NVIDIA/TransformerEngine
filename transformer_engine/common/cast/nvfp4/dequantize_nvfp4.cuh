...
#if FP4_TYPE_SUPPORTED
template <typename OType, typename ScaleType, bool WITH_GEMM_SWIZZLED_SCALES, bool ROW_SCALED_NVFP4,
          int SCALE_TYPE_MAX>
__global__ void __launch_bounds__(512)
    dequantize_fp4_kernel(const void *const input, OType *output, const ScaleType *const scales,
  ...
  constexpr float unit_global_scale_amax = fp4_max * SCALE_TYPE_MAX;
...
inline void dequantize(const Tensor &input, Tensor *output, cudaStream_t stream) {
  ...
  const DType scale_dtype = input.scale_inv.dtype;
  const int e4m3_max = input.get_nvfp4_scale_max();
  ...
  if (static_cast<float>(e4m3_max) != typeToMax(scale_dtype)) {
    NVTE_CHECK(scale_dtype == DType::kFloat8E4M3,
               "NVFP4 dequantization with non-default scale max "
               "is only supported with FP8E4M3 scales (found ",
               to_string(scale_dtype), ").");
    NVTE_CHECK(e4m3_max == 256,
               "NVFP4 dequantization with non-default scale max "
