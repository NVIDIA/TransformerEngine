...
__global__ void __launch_bounds__(256, 4)  // Optimize for 4 blocks/SM
    grouped_fp8_quantize_optimized_kernel(const void* const* __restrict__ input_ptrs,
...
#pragma unroll 4  // Unroll outer loop for better ILP
    for (size_t vec_idx = vector_tile_start + threadIdx.x; vec_idx < vector_size;
    ...
    using VecType = typename std::conditional<std::is_same<InputType, __half>::value, __half2,
                                              __nv_bfloat162>::type;
  ...
  for (size_t idx = remainder_start + blockIdx.y * blockDim.x + threadIdx.x; idx < size;
...
__global__ void __launch_bounds__(256, 4)  // 4 blocks/SM for better occupancy
    grouped_fp8_quantize_ultra_optimized_kernel(const void* const* __restrict__ input_ptrs,
                                                void* const* __restrict__ output_ptrs,
                                                const float* __restrict__ scales,
                                                const size_t* __restrict__ tensor_sizes,
                                                const int num_tensors) {
...
#pragma unroll
    ...
    using InputVec = typename std::conditional<std::is_same<InputType, __half>::value, __half2,
                                               __nv_bfloat162>::type;
...
#pragma unroll
    ...
    for (size_t idx = blockIdx.y * blockDim.x + threadIdx.x; idx < size;
         idx += blockDim.x * gridDim.y) {
      float val = static_cast<float>(input[idx]) * scale;
      output[idx] = static_cast<OutputType>(val);
    }
...
__global__ void __launch_bounds__(256, 4)
    grouped_fp8_quantize_transpose_optimized_kernel(const void* const* __restrict__ input_ptrs,
                                                    void* const* __restrict__ output_ptrs,
                                                    const float* __restrict__ scales,
...
__global__ void __launch_bounds__(256)
    grouped_fp8_quantize_transpose_warp_optimized_kernel(const void* const* __restrict__ input_ptrs,
                                                         void* const* __restrict__ output_ptrs,
                                                         const float* __restrict__ scales,
                                                         const size_t* __restrict__ first_dims,
                                                         const size_t* __restrict__ last_dims,
  ...
  for (size_t idx = blockIdx.y * blockDim.x + threadIdx.x; idx < total_elements;
       idx += blockDim.x * gridDim.y) {
 ...
 */
void compute_optimized_grid_config(int num_tensors, size_t max_tensor_size, int vectorization,
  ...
  if (effective_size < elements_per_block) {
    ...
    num_tiles = min((effective_size + elements_per_block - 1) / elements_per_block, (size_t)256);
  }
 ...
 * @param block_dim Output block dimensions
 */
void compute_transpose_grid_config(int num_tensors, size_t max_m, size_t max_n, int tile_size,
                                   dim3& grid_dim, dim3& block_dim) {
...
void launch_grouped_fp8_quantize_rowwise(const GroupedTensor& input, GroupedTensor& output,
  ...
  for (int i = 0; i < num_tensors; i++) {
    const size_t offset =
        input.offsets ? input.offsets[i] : (i * input.shapes[0][0] * input.shapes[0][1]);
    const size_t numel = input.shapes[i][0] * input.shapes[i][1];

    h_input_ptrs[i] =
        static_cast<void*>(reinterpret_cast<char*>(input.data) + offset * input.element_size());
    h_output_ptrs[i] =
        static_cast<void*>(reinterpret_cast<char*>(output.data) + offset * output.element_size());
  ...
  cudaMemcpyAsync(d_input_ptrs, h_input_ptrs.data(), num_tensors * sizeof(void*),
                  cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(d_output_ptrs, h_output_ptrs.data(), num_tensors * sizeof(void*),
                  cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(d_scales, h_scales.data(), num_tensors * sizeof(float), cudaMemcpyHostToDevice,
                  stream);
  cudaMemcpyAsync(d_sizes, h_sizes.data(), num_tensors * sizeof(size_t), cudaMemcpyHostToDevice,
                  stream);
    ...
    if (output_dtype == DType::kFloat8E4M3) {
      if (all_aligned_16) {
        ...
        grouped_fp8_quantize_ultra_optimized_kernel<float, __nv_fp8_e4m3, 4, 4>
            <<<grid_dim, block_dim, 0, stream>>>(d_input_ptrs, d_output_ptrs, d_scales, d_sizes,
                                                 num_tensors);
      } else {
...
void launch_grouped_fp8_quantize_columnwise(const GroupedTensor& input, GroupedTensor& output,
  ...
  for (int i = 0; i < num_tensors; i++) {
    ...
    h_input_ptrs[i] =
        static_cast<void*>(reinterpret_cast<char*>(input.data) + offset * input.element_size());
    h_output_ptrs[i] = static_cast<void*>(reinterpret_cast<char*>(output.columnwise_data) +
                                          offset * output.element_size());
    h_scales[i] = output.scale[i];
    h_first_dims[i] = M;
    h_last_dims[i] = N;
  ...
  cudaMemcpyAsync(d_scales, h_scales.data(), num_tensors * sizeof(float), cudaMemcpyHostToDevice,
                  stream);
  cudaMemcpyAsync(d_first_dims, h_first_dims.data(), num_tensors * sizeof(size_t),
                  cudaMemcpyHostToDevice, stream);
  cudaMemcpyAsync(d_last_dims, h_last_dims.data(), num_tensors * sizeof(size_t),
                  cudaMemcpyHostToDevice, stream);
  ...
  if (input_dtype == DType::kFloat32) {
    if (output_dtype == DType::kFloat8E4M3) {
      grouped_fp8_quantize_transpose_kernel<float, __nv_fp8_e4m3>
          <<<grid_dim, block_dim, 0, stream>>>(d_input_ptrs, d_output_ptrs, d_scales, d_first_dims,
                                               d_last_dims, num_tensors);
    } else if (output_dtype == DType::kFloat8E5M2) {
