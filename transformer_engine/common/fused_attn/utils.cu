...
__device__ void cu_seqlens_to_offsets_impl(int64_t b, int64_t h, int64_t d, int32_t *cu_seqlens_q,
                                           int32_t *actual_seqlens_q, OFFSETS_T *qkv_ragged_offset,
                                           OFFSETS_T *o_ragged_offset) {
...
__global__ void cu_seqlens_to_offsets(int64_t b, int64_t h, int64_t d, int32_t *cu_seqlens_q,
  ...
  if (offset_dtype == DType::kInt32) {
    cu_seqlens_to_offsets_impl<int32_t>(b, h, d, cu_seqlens_q, actual_seqlens_q,
                                        reinterpret_cast<int32_t *>(qkv_ragged_offset),
                                        reinterpret_cast<int32_t *>(o_ragged_offset));
  } else {
    assert(offset_dtype == DType::kInt64 && "expected int64");
    cu_seqlens_to_offsets_impl<int64_t>(b, h, d, cu_seqlens_q, actual_seqlens_q,
                                        reinterpret_cast<int64_t *>(qkv_ragged_offset),
                                        reinterpret_cast<int64_t *>(o_ragged_offset));
  }
}
