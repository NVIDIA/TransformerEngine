...
// convert cu_seqlens_with_offset to offsets
__global__ void cu_seqlens_with_offset_to_offsets(
  ...
  size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid < b+1) {
    offsets_o[tid] = h * d * cu_seqlens_q_with_offset[tid];
    switch (layout_group) {
        case NVTE_QKV_Layout_Group::NVTE_HD_HD_HD:
            offsets_q[tid] = h * d * cu_seqlens_q_with_offset[tid];
            offsets_k[tid] = hg * d * cu_seqlens_kv_with_offset[tid];
            offsets_v[tid] = offsets_k[tid];
            break;
        case NVTE_QKV_Layout_Group::NVTE_3HD:
        case NVTE_QKV_Layout_Group::NVTE_H3D:
            offsets_q[tid] = 3 * h * d * cu_seqlens_q_with_offset[tid];
        ...
        case NVTE_QKV_Layout_Group::NVTE_HD_H2D:
            offsets_q[tid] = h * d * cu_seqlens_q_with_offset[tid];
            offsets_k[tid] = 2 * hg * d * cu_seqlens_kv_with_offset[tid];
            offsets_v[tid] = offsets_k[tid];
            break;
    }
  }
