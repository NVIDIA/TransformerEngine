...
void nvte_fused_attn_fwd_qkvpacked(
  ...
  const Tensor *input_cu_seqlens_with_padding = \
      reinterpret_cast<const Tensor*>(cu_seqlens_with_padding);
...
void nvte_fused_attn_fwd_kvpacked(
  ...
  const Tensor *input_cu_seqlens_q_with_padding = \
      reinterpret_cast<const Tensor*>(cu_seqlens_q_with_padding);
  const Tensor *input_cu_seqlens_kv_with_padding = \
      reinterpret_cast<const Tensor*>(cu_seqlens_kv_with_padding);
  const Tensor *input_rng_state = reinterpret_cast<const Tensor*>(rng_state);
  ...
  if (fused_attention_backend == NVTE_Fused_Attn_Backend::NVTE_F16_max512_seqlen) {
    ...
    NVTE_ERROR("cuDNN 8.9.1 is required for BF16/FP16 fused attention with max_seqlen<=512. \n");
      ...
      fused_attn_arbitrary_seqlen_fwd_kvpacked(
          ...
          Aux_CTX_Tensors,
          input_cu_seqlens_q, input_cu_seqlens_kv,
          input_cu_seqlens_q_with_padding, input_cu_seqlens_kv_with_padding,
          input_rng_state,
          wkspace, stream, handle);
#else
    NVTE_ERROR(
