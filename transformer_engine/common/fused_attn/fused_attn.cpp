...
NVTE_Fused_Attn_Backend nvte_get_fused_attn_backend(
  ...
  if (((q_dtype == NVTEDType::kNVTEFloat8E4M3) || (q_dtype == NVTEDType::kNVTEFloat8E5M2)) &&
    ...
    if (!supported_ragged_offset_size && backend != NVTE_Fused_Attn_Backend::NVTE_No_Backend) {
      backend = NVTE_Fused_Attn_Backend::NVTE_No_Backend;
      std::cout << "Warning: Tensor sizes are too large for 32-bit THD/ragged offsets."
                << " Please upgrade your cuDNN version to 9.5.0+ if possible." << std::endl;
    }
