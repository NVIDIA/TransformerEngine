...
createCausalMask(int64_t b, int64_t h, int64_t s_q, int64_t s_kv, int64_t d,
    ...
    cudnn_frontend::throw_if(ops->size() == 0,
           "Padding Mask constructed incorrectly as the first one", CUDNN_STATUS_BAD_PARAM);
...
void fused_attn_arbitrary_seqlen_fwd_impl(
    ...
    try {
        ...
        auto get_plan = [&](CacheType &cache, const FADescriptor &descriptor) {
            ...
            cudnn_frontend::throw_if(dropout_probability != 0.0f && !is_training,
        ...
        __half scale_dropout = __float2half_rn(
...
void fused_attn_arbitrary_seqlen_bwd_impl(
    ...
    try {
        ...
        // add all the data pointers to be used in the variant pack
        float negInfinity = -1.0E+20f;  // change this if you have access to float_min
        __half scale_dropout = __float2half_rn(static_cast<float>(1/(1 - dropout_probability)));
