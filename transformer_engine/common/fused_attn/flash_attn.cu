...
__launch_bounds__(TRANSPOSE_BLOCK) __global__
    void permute_to_grouped_tensor_fwd_fallback_not_vec_aligned_kernel(
        ...
        size_t d_v_out, unsigned int s_tiles) {
...
__launch_bounds__(TRANSPOSE_BLOCK) __global__
    void permute_to_grouped_tensor_bwd_fallback_not_vec_aligned_kernel(
        ...
        size_t d_v_in, unsigned int s_tiles) {
  const int which = blockIdx.z;
...
__launch_bounds__(tma_permute_threads) __global__
    void permute_to_grouped_tensor_fwd_kernel(const __grid_constant__ CUtensorMap tma_q_in,
                                              ...
                                              size_t h_q, size_t d_qk, size_t d_qk_out, size_t s_kv,
                                              size_t h_kv, size_t d_v, size_t d_v_out,
...
__launch_bounds__(tma_permute_threads) __global__ void permute_to_grouped_tensor_bwd_kernel(
    ...
    const __grid_constant__ CUtensorMap tma_v_out, size_t b, size_t s_q, size_t h_q, size_t d_qk,
    size_t d_qk_in, size_t s_kv, size_t h_kv, size_t d_v, size_t d_v_in,
    unsigned int permute_s_splits, size_t s_tile_size) {
#if (defined __CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  const int which = blockIdx.z;
...
void permute_to_grouped_tensor_fwd(Tensor q, Tensor k, Tensor v, Tensor q_out, Tensor k_out,
      ...
      if (is_bshd) {
        TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
            ...
            <<<grid, TRANSPOSE_BLOCK, smem_bytes, stream>>>(
                ...
                reinterpret_cast<dtype *>(v_out.data.dptr), b, s_q, h_q, d_qk, d_qk_out, s_kv, h_kv,
                d_v, d_v_out, st););
      } else {
        TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
            q.dtype(), dtype,
  ...
  NVTE_CHECK((s_tile * d_qk * elem_size) % sizeof(uint4) == 0 &&
             ...
             "permute_to_grouped_tensor_fwd: S_TILE(", s_tile,
             ") * D * elem_size must "
             "be divisible by ",
             sizeof(uint4), ". d_qk=", d_qk, ", d_v=", d_v, ", elem_size=", elem_size, ".");
  ...
  if (is_bshd) {
    TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
        ...
        kernel<<<grid, tma_permute_threads, smem_bytes, stream>>>(
            ...
            b, s_q, h_q, d_qk, d_qk_out, s_kv, h_kv, d_v, d_v_out, permute_s_splits, s_tile););
  } else {
    TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
...
void permute_to_grouped_tensor_bwd(Tensor grad_q, Tensor grad_k, Tensor grad_v, Tensor q, Tensor k,
                                   Tensor v, NVTE_QKV_Format original_format, size_t num_tensors,
                                   cudaStream_t stream) {
  using namespace transformer_engine;
      ...
      if (is_bshd) {
        TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
            ...
            <<<grid, TRANSPOSE_BLOCK, smem_bytes, stream>>>(
                ...
                reinterpret_cast<dtype *>(q.data.dptr), reinterpret_cast<dtype *>(k.data.dptr),
                reinterpret_cast<dtype *>(v.data.dptr), b, s_q, h_q, d_qk, d_qk_in, s_kv, h_kv, d_v,
                d_v_in, st););
      } else {
        TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
            grad_q.dtype(), dtype,
  ...
  NVTE_CHECK((s_tile * d_qk * elem_size) % sizeof(uint4) == 0 &&
                 (s_tile * d_v * elem_size) % sizeof(uint4) == 0,
             "permute_to_grouped_tensor_bwd: S_TILE(", s_tile,
             ") * D * elem_size must "
             "be divisible by ",
             sizeof(uint4), ". d_qk=", d_qk, ", d_v=", d_v, ", elem_size=", elem_size, ".");
  ...
  create_strided_tensor_map(tma_k_out, k.data.dptr, k.dtype(), b, s_kv, h_kv, d_qk, s_tile,
                            is_bshd);
  ...
  if (is_bshd) {
    TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
        ...
        kernel<<<grid, tma_permute_threads, smem_bytes, stream>>>(
            ...
            s_q, h_q, d_qk, d_qk_in, s_kv, h_kv, d_v, d_v_in, permute_s_splits, s_tile););
  } else {
    TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
