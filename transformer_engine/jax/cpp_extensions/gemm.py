...
class GemmPrimitive(BasePrimitive):
    ...
    def batcher(
        ...
        assert (
            GemmPrimitive.outer_primitive is not None
        ), "GemmPrimitive.outer_primitive has not been registered"
...
def _te_gemm(
    ...
    if scaling_mode.is_nvfp4_scaling:
        assert (
            lhs_amax is not None and rhs_amax is not None
...
class GroupedGemmCopySizesPrimitive(BasePrimitive):
    ...
    def impl(
        ...
        assert (
            GroupedGemmCopySizesPrimitive.inner_primitive is not None
...
class GroupedGemmPrimitive(BasePrimitive):
    ...
    def impl(
        ...
        assert (
            GroupedGemmPrimitive.inner_primitive is not None
        ), "GroupedGemmPrimitive.inner_primitive has not been registered"
...
def _jax_scaled_matmul(
    ...
    if lhs.scaling_mode.is_nvfp4_scaling:
        assert (
            lhs.amax is not None and rhs.amax is not None
        ), "NVFP4 scaling requires non-None amax for both LHS and RHS operands"
...
def grouped_gemm(
    ...
    if (
        ...
        assert isinstance(
            quantizer_set.x, GroupedQuantizer
    ...
    assert (
        group_offset.size == 1
    ), f"Expected group_offset.size == 1, but got group_offset.size={group_offset.size}"

    has_bias = bias is not None
    assert not has_bias or bias.shape == (
        group_sizes.size,
        N,
