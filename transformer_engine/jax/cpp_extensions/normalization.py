...
class NormFwdPrimitive(BasePrimitive):
    ...
    def abstract(
        ...
        *,
        ...
        assert x_dtype in [jnp.float32, jnp.float16, jnp.bfloat16], f"Unsupported x_dtype={x_dtype}, expected one of [float32, float16, bfloat16]"
        assert scale_aval is None or scale_aval.dtype == jnp.float32, f"Expected scale_aval.dtype=float32, but got scale_aval.dtype={scale_aval.dtype}"
        assert amax_aval is None or amax_aval.dtype == jnp.float32, f"Expected amax_aval.dtype=float32, but got amax_aval.dtype={amax_aval.dtype}"
        ...
        if norm_type == NVTE_Norm_Type.LayerNorm:
            assert gamma_aval.size == beta_aval.size, f"Expected gamma_aval.size == beta_aval.size, but got gamma_aval.size={gamma_aval.size}, beta_aval.size={beta_aval.size}"
    ...
    def lowering(
        ...
        *,
        ...
        assert x_aval.dtype in [jnp.float32, jnp.float16, jnp.bfloat16], f"Unsupported x_aval.dtype={x_aval.dtype}, expected one of [float32, float16, bfloat16]"
        assert scale_aval is None or scale_aval.dtype == jnp.float32, f"Expected scale_aval.dtype=float32, but got scale_aval.dtype={scale_aval.dtype}"
        assert amax_aval is None or amax_aval.dtype == jnp.float32, f"Expected amax_aval.dtype=float32, but got amax_aval.dtype={amax_aval.dtype}"
        ...
        if norm_type == NVTE_Norm_Type.LayerNorm:
            assert gamma_aval.dtype == beta_aval.dtype, f"Expected gamma and beta to have the same dtype, but got gamma_aval.dtype={gamma_aval.dtype}, beta_aval.dtype={beta_aval.dtype}"
            ...
            assert g_shape == b_shape, f"Expected gamma and beta to have the same shape, but got gamma_shape={g_shape}, beta_shape={b_shape}"
    ...
    def impl(
        ...
        """
        del is_outer
        assert NormFwdPrimitive.inner_primitive is not None, "NormFwdPrimitive.inner_primitive has not been registered"
        (
            out,
            colwise_out,
            scale_inv,
    ...
    def batcher(
        ...
        *,
        ...
        assert NormFwdPrimitive.outer_primitive is not None, "NormFwdPrimitive.outer_primitive has not been registered"
...
class NormBwdPrimitive(BasePrimitive):
    ...
    def abstract(dz_aval, x_aval, mu_aval, rsigma_aval, gamma_aval, norm_type, zero_centered_gamma):
        ...
        assert dtypes.canonicalize_dtype(dz_aval.dtype) == w_dtype, f"Expected dz_aval.dtype={w_dtype} (matching gamma dtype), but got dz_aval.dtype={dtypes.canonicalize_dtype(dz_aval.dtype)}"
        assert dz_aval.shape == x_aval.shape, f"Expected dz_aval.shape == x_aval.shape, but got dz_aval.shape={dz_aval.shape}, x_aval.shape={x_aval.shape}"

        if norm_type == NVTE_Norm_Type.LayerNorm:
            mu_dtype = dtypes.canonicalize_dtype(mu_aval.dtype)
            assert mu_aval.shape == rsigma_aval.shape == x_aval.shape[:-1], f"Expected mu_aval.shape == rsigma_aval.shape == x_aval.shape[:-1], but got mu_aval.shape={mu_aval.shape}, rsigma_aval.shape={rsigma_aval.shape}, x_aval.shape[:-1]={x_aval.shape[:-1]}"
            assert mu_dtype == rsigma_dtype == jnp.float32, f"Expected mu_dtype == rsigma_dtype == float32, but got mu_dtype={mu_dtype}, rsigma_dtype={rsigma_dtype}"
    ...
    def impl(dz, x, mu, rsigma, gamma, norm_type, zero_centered_gamma):
        assert NormBwdPrimitive.inner_primitive is not None, "NormBwdPrimitive.inner_primitive has not been registered"
    ...
    def batcher(batched_args, batch_dims, *, norm_type, zero_centered_gamma):
        check_valid_batch_dims(batch_dims)
        assert NormBwdPrimitive.outer_primitive is not None, "NormBwdPrimitive.outer_primitive has not been registered"
