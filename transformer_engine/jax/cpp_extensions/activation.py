...
class ActLuPrimitive(BasePrimitive):
    ...
    def shardy_sharding_rule(
        ...
        out = (*x_axes[:-2], 'j', x_axes[-1])
        if scaling_mode == ScalingMode.NVTE_MXFP8_1D_SCALING.value:
            scale_inv = scale_rules.rowwise_rule[:-1] + ('k', scale_rules.rowwise_rule[-1])
            colwise_scale_inv = scale_rules.colwise_rule[:-1] + ('k', scale_rules.colwise_rule[-1])
        else:
