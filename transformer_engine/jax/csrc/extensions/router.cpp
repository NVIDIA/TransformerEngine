...
Error_Type FusedTopkWithScoreFunctionForwardFFI(
  ...
  auto flat_shape = std::vector<size_t>{static_cast<size_t>(num_tokens),
...
Error_Type FusedTopkWithScoreFunctionBackwardFFI(
  ...
  auto intermediate_tensor =
      TensorWrapper(intermediate_buf.untyped_data(), flat_shape, dtype);
  auto grad_probs_tensor =
      TensorWrapper(grad_probs_buf.untyped_data(), flat_shape, dtype);
  auto grad_logits_tensor =
      TensorWrapper(grad_logits_buf->untyped_data(), flat_shape, dtype);

  if (compute_aux_scores) {
    nvte_fused_score_for_moe_aux_loss_backward(
        intermediate_tensor.data(), grad_probs_tensor.data(), num_tokens, num_experts,
        static_cast<int>(topk), static_cast<int>(score_function), grad_logits_tensor.data(),
        stream);
  } else {
    ...
    nvte_fused_topk_with_score_function_backward(
        routing_map_tensor.data(), intermediate_tensor.data(), grad_probs_tensor.data(),
        num_tokens, num_experts, static_cast<int>(topk), static_cast<int>(use_pre_softmax),
