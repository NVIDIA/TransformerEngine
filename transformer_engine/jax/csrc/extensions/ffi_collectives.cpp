...
std::vector<std::vector<int64_t>> ReplicaGroupsFromFlat(const int64_t* flat, size_t count,
                                                        int64_t group_size) {
  NVTE_CHECK(group_size > 0, "ReplicaGroupsFromFlat: group_size must be > 0, got ", group_size);
  NVTE_CHECK(count % static_cast<size_t>(group_size) == 0,
             "ReplicaGroupsFromFlat: flat replica-group buffer size (", count,
             ") must be a multiple of group_size (", group_size, ")");
