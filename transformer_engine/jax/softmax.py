...
def softmax(
    ...
    softmax_fusion: Optional[SoftmaxFusion] = SoftmaxFusion.SCALED,
):
    """
    Softmax wrapper
    """
    output = _softmax(logits, mask, scale_factor, softmax_fusion)
    return output


@partial(jax.custom_vjp, nondiff_argnums=(2, 3))
def _softmax(logits, mask, scale_factor, softmax_fusion):

    output, _ = _softmax_fwd_rule(logits, mask, scale_factor, softmax_fusion)
    return output


def _softmax_fwd_rule(logits, mask, scale_factor, softmax_fusion):
    if softmax_fusion is SoftmaxFusion.SCALED_MASKED:
        assert mask is not None
        output = tex.scaled_masked_softmax_fwd(logits, mask, scale_factor)
    elif softmax_fusion is SoftmaxFusion.SCALED_UPPER_TRIANG_MASKED:
...
def _softmax_bwd_rule(scale_factor, softmax_fusion, ctx, dz):
