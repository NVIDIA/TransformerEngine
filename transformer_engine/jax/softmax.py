...
def _softmax_bwd_rule(scale_factor, softmax_type, ctx, dz):
    ...
    if softmax_type is SoftmaxType.SCALED_MASKED:
        dgrad = scaled_masked_softmax_bwd(dz, softmax_output, logits, mask, scale_factor)
    elif softmax_type is SoftmaxType.SCALED_UPPER_TRIANG_MASKED:
        dgrad = scaled_upper_triang_masked_softmax_bwd(dz, softmax_output, logits, scale_factor)
    else:
        dgrad = scaled_softmax_bwd(dz, softmax_output, logits, scale_factor)
