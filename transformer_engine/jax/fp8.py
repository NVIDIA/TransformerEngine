...
from .utils import get_cublasLt_version, get_cuda_version, get_device_compute_capability
...
def _check_fp8_support(gpu_id) -> Tuple[bool, str]:
    ...
    if gpu_arch >= 9.0:    # hopper and above
        return True, ""
    if gpu_arch < 8.9:    # pre-ada
        return False, "Device compute capability 8.9 or higher required for FP8 execution."
    if get_cublasLt_version() < 120103:
        return False, "CublasLt version 12.1.3.x or higher required for FP8 execution on Ada."
    if get_cuda_version() < 12.01:
