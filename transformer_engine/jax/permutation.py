...
def token_dispatch(
    ...
    tokens_per_expert : jnp.ndarray
        Token counts per expert of shape [num_experts]. The semantics depend on padding:
        ...
        which is needed for ragged_all_to_all in expert parallelism scenarios.
...
def _token_dispatch_fwd_rule(
    ...
    # Compute tokens_per_expert (actual counts) from routing_map
    ...
    if use_padding:
        # Calculate aligned token counts per expert for padding
