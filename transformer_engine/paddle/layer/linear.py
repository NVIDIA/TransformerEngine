...
def _linear_fwd_fp8(
    ...
    weight_fp8_index: tex.FP8FwdTensors,
    bias: paddle.Tensor,
    use_bias: bool,
...
def _linear_fwd_non_fp8(
    ...
    if fp8_calibration:
        # amax of input
        fp8_meta["scaling_fwd"].amax_history[0, inputmat_fp8_index] = \
            paddle.max(paddle.abs(inputmat)).item()
        # amax of weight
        fp8_meta["scaling_fwd"].amax_history[0, weight_fp8_index] = \
...
def _linear_bwd_fp8(
    ...
    grad_output_fp8_index: tex.FP8BwdTensors,
...
def _linear_bwd_non_fp8(
    ...
    """
    Performs Linear Backward. Optionally, fuses GELU backward and grad_bias.
    """
    ...
    def backward(ctx, grad_output: paddle.Tensor) -> Tuple[Union[paddle.Tensor, None], ...]:
        with _prepare_backward(ctx.fp8, ctx.fp8_meta, name="_Linear"):
            ...
            return (
                wgrad if not weight.stop_gradient else None,
                dgrad.reshape(ctx.inp_shape) if ctx.requires_dgrad else None,
                grad_bias if ctx.requires_bgrad else None,
            )
