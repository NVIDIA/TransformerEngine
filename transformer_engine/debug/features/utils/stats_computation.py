...
def get_zero_values(device):
    if device not in _zero_values_cache:
        _zero_values_cache[device] = torch.tensor([0, FP8_NEGATIVE_ZERO], device=device)
    return _zero_values_cache[device]
