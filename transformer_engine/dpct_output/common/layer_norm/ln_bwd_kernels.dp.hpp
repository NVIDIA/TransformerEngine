...
/*
DPCT1110:170: The total declared local variable size in device function
ln_bwd_tuned_kernel exceeds 128 bytes and may cause high register pressure.
...
void ln_bwd_tuned_kernel(layer_norm::BwdParams params,
    ...
    for ( int row = r; row < params.rows; row += params.ctas_per_col * ROWS_PER_CTA ) {
        ...
        /*
        DPCT1084:171: The function call "Reducer::allreduce" has multiple
        migration results in different template instantiations that could not be
        unified. You may need to adjust the code.
        */
        reduce_t result =
            reducer.allreduce({mdy_local, mdyy_local}, sum, item_ct1);
    ...
    if ( WARPS_M == 1 ) {
        ...
        /*
        DPCT1118:167: SYCL group functions and algorithms must be encountered in
...
ln_bwd_finalize_tuned_kernel exceeds 128 bytes and may cause high register
        ...
        */
        ...
        for ( int w = warp; w < THREADS_PER_WARP; w += Kernel_traits::ROWS_PER_CTA ) {
            ...
            for ( int it = 0; it < NUM_ELT; it++ ) {
                ...
                b_i = reducer.allreduce(b_i, sum, item_ct1);
        ...
        // All writes done.
        /*
        DPCT1118:173: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
...
ln_bwd_general_kernel exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
...
void ln_bwd_finalize_general_kernel(layer_norm::BwdParams params,
    ...
    */
    ...
    for ( int nrows = WARPS_M / 2; nrows > 0; nrows /= 2 ) {
        /*
        DPCT1118:177: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
