...
void layernorm_fwd(const Tensor &x,     // BxSxhidden_size
    ...
    // Clear buffers
    if ( params.fp8_out ) {
        stream->memset(params.amax, 0,
                       layer_norm::product(z->amax.shape) *
                           typeToSize(z->amax.dtype));
    }
    if ( launch_params.barrier_size > 0 ) {
        stream->memset(params.barrier, 0,
...
void layernorm_bwd(const Tensor &dz, const Tensor &x, const Tensor &mu,
                   const Tensor &rsigma, const Tensor &gamma, Tensor *dx,
                   Tensor *dgamma, Tensor *dbeta, Tensor *dgamma_part,
                   Tensor *dbeta_part, dpct::queue_ptr stream,
                   const int multiprocessorCount, Tensor *workspace,
                   Tensor *barrier, const bool zero_centered_gamma) {
    using namespace transformer_engine;
...
void nvte_layernorm_fwd(const NVTETensor x,     // BxSxhidden_size
                        ...
                        const float epsilon, NVTETensor z, NVTETensor mu,
                        NVTETensor rsigma, dpct::queue_ptr stream,
...
void nvte_layernorm_bwd(const NVTETensor dz,     // BxSxhidden_size
                        ...
                        NVTETensor dgamma_part, NVTETensor dbeta_part,
                        dpct::queue_ptr stream, const int multiprocessorCount,
