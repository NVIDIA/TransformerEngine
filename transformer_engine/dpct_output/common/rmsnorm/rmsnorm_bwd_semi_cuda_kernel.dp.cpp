...
#include "rmsnorm.h"
#include "rmsnorm_bwd_kernels.dp.hpp"
...
void launch_tuned_(LaunchParams<BwdParams> &launch_params,
    ...
    if (configure_params) {
        int ctas_per_sm;
        /*
        DPCT1007:513: Migration of cudaOccupancyMaxActiveBlocksPerMultiprocessor
        is not supported.
        */
    ...
    if (Kernel_traits::SMEM_BYTES >= 48 * 1024) {
        /*
        DPCT1009:512: SYCL uses exceptions to report errors and does not use the
        error codes. The original code was commented out and a warning string
        was inserted. You need to rewrite this code.
        */
        /*
        DPCT1027:514: The call to cudaFuncSetAttribute was replaced with 0
    ...
    if (ctas_per_row == 1) {
        /*
        DPCT1049:162: The work-group size passed to the SYCL kernel may exceed
        ...
        */
        ...
        /*
        DPCT1007:163: Migration of cudaLaunchCooperativeKernel is not supported.
        */
    ...
    /*
    DPCT1049:161: The work-group size passed to the SYCL kernel may exceed the
    limit. To get the device limit, query info::device::max_work_group_size.
    Adjust the work-group size if needed.
    */
    stream->parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(1, 1, Kernel_traits_f::CTAS) *
                sycl::range<3>(1, 1, Kernel_traits_f::THREADS_PER_CTA),
            sycl::range<3>(1, 1, Kernel_traits_f::THREADS_PER_CTA)),
        [=](sycl::nd_item<3> item_ct1) {
            (launch_params.params);
        });
...
void launch_general_(LaunchParams<BwdParams> &launch_params, const bool configure_params) {  // NOLINT(*)
    ...
    if (configure_params) {
        int ctas_per_sm;
        /*
        DPCT1007:515: Migration of cudaOccupancyMaxActiveBlocksPerMultiprocessor
        is not supported.
        */
    ...
    if (ctas_per_row == 1) {
        /*
        DPCT1049:165: The work-group size passed to the SYCL kernel may exceed
        the limit. To get the device limit, query
        info::device::max_work_group_size. Adjust the work-group size if needed.
        */
        ...
        void *params_ = reinterpret_cast<void *>(&launch_params.params);
        /*
        DPCT1007:166: Migration of cudaLaunchCooperativeKernel is not supported.
        */
        cudaLaunchCooperativeKernel(reinterpret_cast<void *>(kernel), grid,
                                    block, reinterpret_cast<void **>(&params_),
                                    0, stream);
    ...
    sycl::range<3> block_final(1, WARPS_M_FINAL,
                               Kernel_traits::THREADS_PER_WARP * WARPS_N_FINAL);
    sycl::range<3> grid_final(1, 1, ceil_div(cols, ELTS_N_PER_CTA_FINAL));
    /*
    ...
    */
    stream->parallel_for(
        sycl::nd_range<3>(grid_final * block_final, block_final),
        [=](sycl::nd_item<3> item_ct1) {
            (launch_params.params);
        });
}
