...
/*
DPCT1110:154: The total declared local variable size in device function
rmsnorm_bwd_tuned_kernel exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
void rmsnorm_bwd_tuned_kernel(BwdParams params,
...
#pragma unroll
        ...
        /*
        DPCT1084:155: The function call "Reducer::allreduce" has multiple
        migration results in different template instantiations that could not be
        unified. You may need to adjust the code.
        */
        reduce_t result = reducer.allreduce({0, mdyy_local}, sum, item_ct1);
...
/*
DPCT1110:158: The total declared local variable size in device function
rmsnorm_bwd_finalize_tuned_kernel exceeds 128 bytes and may cause high register
pressure. Consult with your hardware vendor to find the total register size
available and adjust the code, or use smaller sub-group size to avoid high
register pressure.
*/
void rmsnorm_bwd_finalize_tuned_kernel(BwdParams params,
                                       const sycl::nd_item<3> &item_ct1,
                                       char *smem_) {
    ...
    const uint32_t bidn = item_ct1.get_group(2);
    const uint32_t tidx = item_ct1.get_local_id(2);
...
#pragma unroll
        ...
        /*
        DPCT1118:156: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
...
#pragma unroll
            for (int it = 0; it < NUM_ELT; it++) {
                ...
                g_i = reducer.allreduce(g_i, sum, item_ct1);
        ...
        // All writes done.
        /*
        DPCT1118:157: SYCL group functions and algorithms must be encountered in
        converged control flow. You may need to adjust the code.
        */
        /*
...
rmsnorm_bwd_general_kernel exceeds 128 bytes and may cause high register
pressure. Consult with your hardware vendor to find the total register size
available and adjust the code, or use smaller sub-group size to avoid high
register pressure.
*/
void rmsnorm_bwd_general_kernel(BwdParams params,
                                const sycl::nd_item<3> &item_ct1, char *smem_,
                                sycl::local_accessor<Cvec, 4> vecs_shared) {
    enum { LDGS = Ktraits::LDGS };
    enum { NUM_ELTS = Ktraits::ELTS_PER_LDG };
    enum { WARPS_M = Ktraits::WARPS_M };
    enum { WARPS_N = Ktraits::WARPS_N };
...
#pragma unroll
        ...
        // Reduce over row
        reduce_t result = reducer.allreduce({mdy, mdyy}, sum, item_ct1);
...
void rmsnorm_bwd_finalize_general_kernel(BwdParams params,
                                                                                 const sycl::nd_item<3> &item_ct1,
                                                                                 sycl::local_accessor<Cvec, 3> vecs_shared) {
    enum { NUM_ELTS = BYTES_PER_LDG / sizeof(compute_t) };
    using Wvec = Vec<weight_t, NUM_ELTS>;
    using Cvec = Vec<compute_t, NUM_ELTS>;

    const int lane = item_ct1.get_local_id(2) % THREADS_PER_WARP;
    const int warp_m = item_ct1.get_local_id(1);
    const int warp_n = item_ct1.get_local_id(2) / THREADS_PER_WARP;
    const int col = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
...
#pragma unroll
    for (int nrows = WARPS_M / 2; nrows > 0; nrows /= 2) {
        ...
        */
        /*
        DPCT1065:511: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
