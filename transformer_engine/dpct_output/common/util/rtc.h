...
class Kernel {
 public:
  ...
  void launch(int device_id, const sycl::range<3> grid_dim,
              const sycl::range<3> block_dim, unsigned int shared_mem_bytes,
              dpct::queue_ptr stream, ArgTs &&...args) {
    void* arg_ptrs[] = { const_cast<void*>(static_cast<const void*>(&args))... };
    NVTE_CALL_CHECK_CUDA_DRIVER(
        cuLaunchKernel, get_function(device_id), grid_dim[2], grid_dim[1],
        grid_dim[0], block_dim[2], block_dim[1], block_dim[0], shared_mem_bytes,
 ...
 private:
  ...
  std::vector<dpct::kernel_library> modules_;
  /*! CUDA function for each CUDA device */
  std::vector<dpct::kernel_function> functions_;
...
class KernelManager {
 public:
   ...
   */
  template <typename... ArgTs>
  void launch(const std::string &kernel_label, const sycl::range<3> grid_dim,
              const sycl::range<3> block_dim, unsigned int shared_mem_bytes,
              dpct::queue_ptr stream, ArgTs &&...args) {
