...
template <int nvec_in, int nvec_out, typename CType, typename IType,
          typename OType>
/*
DPCT1110:37: The total declared local variable size in device function
cast_transpose_kernel exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
SYCL_EXTERNAL void

cast_transpose_kernel(const IType *const input, OType *const output_c,
                      OType *const output_t, const CType *const scale_ptr,
                      CType *const amax, const size_t row_length,
...
void cast_transpose(const Tensor &input, Tensor *cast_output,
                    Tensor *transposed_output, dpct::queue_ptr stream) {
...
#define LAUNCH_KERNEL(kernel, nvec_in, nvec_out, n_tiles, n_blocks, InputType, \
                      OutputType)                                              \
    do {                                                                       \
        ;                                                                      \
        kernel<nvec_in, nvec_out, fp32, InputType, OutputType>                 \
            <<<n_blocks, cast_transpose_num_threads,                           \
               ...
               stream>>>(                                                      \
                ...
                reinterpret_cast<fp32 *>(cast_output->amax.dptr), row_length,  \
                num_rows, n_tiles);                                            \
    } while (false)
...
#define LAUNCH_KERNEL_VEC_SIZES(load_size, store_size, InputType, OutputType) \
  ...
  /*
  DPCT1038:41: When the kernel function name is used as a macro argument, the
  migration result may be incorrect. You need to verify the definition of the
  macro.
  */
  /*
  DPCT1026:327: The call to cudaFuncSetAttribute was removed because SYCL
  ...
  */
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
          ...
          // Note: Directly querying number of SMs with cudaGetDeviceProperties
          // is slow (>1 ms). Consider querying once and caching.
          const int n_sms = 128;
