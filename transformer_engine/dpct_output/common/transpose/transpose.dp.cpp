...
transpose_general_kernel(const Type * __restrict__ const input,
                         ...
                         sycl::local_accessor<OVec, 2> shared_output) {
...
#pragma unroll
  for (size_t j2 = 0; j2 < nvec_in; ++j2) {
    ...
    */
    /*
    DPCT1065:464: Consider replacing sycl::nd_item::barrier() with
...
void transpose(const Tensor &input, Tensor *output_, dpct::queue_ptr stream) {
  ...
  /*
  DPCT1038:57: When the kernel function name is used as a macro argument, the
  migration result may be incorrect. You need to verify the definition of the
  macro.
  */
  TRANSFORMER_ENGINE_TYPE_SWITCH_OUTPUT(
      input.data.dtype, Type,
      ...
      rtc_manager.launch(
          kernel_label, sycl::range<3>(1, 1, num_blocks(load_size, store_size)),
          sycl::range<3>(1, 1, block_size), 0, stream,
                ...
                cgh.parallel_for(
                    ...
                    [=](sycl::nd_item<3> item_ct1) {
                        transpose_general_kernel<load_size, store_size, Type>(
                            input_data_dptr_ct0, output_data_dptr_ct1,
                            row_length, num_rows, item_ct1,
                            shared_output_acc_ct1);
                    });
            });
        }
      }); // NOLINT(*)
...
void nvte_transpose(const NVTETensor input, NVTETensor output,
