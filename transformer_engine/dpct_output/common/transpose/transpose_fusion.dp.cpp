...
inline void transpose_regs_partial_dbias(const IVec (&in)[nvec_out],
...
template <int nvec_in, int nvec_out, typename Param>
/*
DPCT1110:61: The total declared local variable size in device function transpose_dbias_kernel_notaligned exceeds 128 bytes and may cause high register pressure. Consult with your hardware vendor to find the total register size available and adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
void
...
#pragma unroll
  ...
  my_dbias_scratch[item_ct1.get_local_id(2)] = partial_dbias;
  /*
  DPCT1065:469: Consider replacing sycl::nd_item::barrier() with
  sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
  performance if there is no access to global memory.
  */
  item_ct1.barrier();
...
void fp8_transpose_dbias(const Tensor &input, Tensor *transposed_output,
  ...
  /*
  DPCT1038:64: When the kernel function name is used as a macro argument, the
  migration result may be incorrect. You need to verify the definition of the
  macro.
  */
  /*
  DPCT1083:65: The size of local memory in the migrated code may be different
  from the original code. Check that the allocated memory size in the migrated
  code is correct.
  */
  /*
  DPCT1026:472: The call to cudaFuncSetAttribute was removed because SYCL
  ...
  TRANSFORMER_ENGINE_TYPE_SWITCH_INPUT(
      ...
      TRANSFORMER_ENGINE_TYPE_SWITCH_FP8ONLY(
        ...
        return;
          ...
          param.scale_inv = reinterpret_cast<const ComputeType *>(
              transposed_output->scale_inv.dptr);
          param.workspace =
        ...
        ;
        {
            ...
            stream->submit([&](sycl::handler &cgh) {
                ...
                cgh.parallel_for(
                    ...
                    [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(
                        32)]] {
                        transpose_dbias_kernel_notaligned<nvec_in, nvec_out,
                                                          Param>(
                            param, row_length, num_rows, n_tiles, item_ct1,
                            dpct_local_acc_ct1
                                .get_multi_ptr<sycl::access::decorated::no>()
          ...
          reduce_dbias<BiasType>(*workspace, dbias, row_length, num_rows,
