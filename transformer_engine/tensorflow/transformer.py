...
class MultiHeadAttention(layers.Layer):
  ...
  def call(
    ...
    if self.attention_type == "self":
      ...
      new_tensor_shape = (*mixed_kv_layer.shape[:-1],
                          self.num_attention_heads_per_partition,
      ...
      # [sq, b, hp] --> [sq, b, np, hn]
      new_tensor_shape = (*query_layer.shape[:-1],
                          self.num_attention_heads_per_partition,
  ...
  def __call__(
    ...
    with autocast_variable.enable_auto_cast_variables(
      ...
      if self.drop_path is None:
        ...
        out = tf.nn.experimental.stateless_dropout(
            attention_output + attention_bias,
            rate=self.hidden_dropout,
            seed=[1, 0],
        ) if training else attention_output + attention_bias
    ...
    with autocast_variable.enable_auto_cast_variables(
      ...
      if self.drop_path is None:
        ...
        # TODO(kaixih): Use stateless_dropout and specify the seed mainly for
        # debugging purpose. Should allow random seed.
        output = tf.nn.experimental.stateless_dropout(
            mlp_output + mlp_bias,
            rate=self.hidden_dropout,
            seed=[1, 0],
        ) if training else mlp_output + mlp_bias
