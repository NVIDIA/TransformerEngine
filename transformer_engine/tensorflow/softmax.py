...
class FusedScaleMaskSoftmax(tf.keras.Model):
    ...
    def scaled_masked_softmax(
        self, x: tf.Tensor, mask: tf.Tensor, scale: float):
    ...
    def scaled_upper_triang_masked_softmax(
      ...
      def grad_fn(upstream):
        dx = tex.scaled_upper_triang_masked_softmax_backward(upstream, y, scale,
                                                             self.stream)
        return dx, None
    ...
    def forward_fused_softmax(
        self, inp: tf.Tensor, mask: tf.Tensor
    ) -> tf.Tensor:
