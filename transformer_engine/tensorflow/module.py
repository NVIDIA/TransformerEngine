...
def cast_from_fp8_wrapper(x, fp8_meta, amax_index, fwd, idtype, odtype, sid):
  ...
  scale_inv = fp8_meta[scaling_key]["scale_inv"].value()
  x_fp8 = tex.cast_from_fp8(
      x, scale_inv, idtype, odtype, amax_index, sid)
  return x_fp8
...
def fp8_cast_transpose_bgrad_fused_wrapper(x, fp8_meta, amax_index, fwd,
                                           output_dtype, sid):
  scaling_key = get_meta_tensor_key(fwd)
  scale = fp8_meta[scaling_key]["scale"].value()
  amax = fp8_meta[scaling_key]["amax_history"].value()
...
def fp8_cast_transpose_bgrad_dgelu_fused_wrapper(dy, x, fp8_meta, amax_index,
                                                 fwd, output_dtype, sid):
  scaling_key = get_meta_tensor_key(fwd)
  scale = fp8_meta[scaling_key]["scale"].value()
  amax = fp8_meta[scaling_key]["amax_history"].value()
  scale_inv = fp8_meta[scaling_key]["scale_inv"].value()
...
def matmul_wrapper(inp, weight, mode, output_dtype, sid, use_bias=False,
                   bias=None, grad=False, gelu=False, gelu_input=None):
  A = inp
  B = weight
  ...
  return tex.te_gemm(B, None, B_dtype, B_offset,
                     A, None, A_dtype, A_offset,
                     get_workspace(),
                     use_bias, bias,
                     gelu, gelu_input,
                     transB, transA, grad,
                     False, # accumulate
                     False, # accumulate
                     TE_DType[output_dtype], sid)

def fp8_matmul_wrapper(inp, weight, fp8_meta, mode, A_dtype, B_dtype,
                       output_dtype, use_split_accumulate, sid, use_bias=False,
                       bias=None):
  A = inp
  B = weight
  ...
  return tex.te_gemm(B, B_scale_inv, B_dtype, B_offset,
                     A, A_scale_inv, A_dtype, A_offset,
                     get_workspace(), use_bias, bias,
                     False, # use_gelu
                     None,  # gelu_input
                     True,  # transa
                     False, # transb
                     False, # grad
                     False, # accumulate
                     use_split_accumulate, TE_DType[output_dtype],
                     sid)

def layernorm_fwd_fp8_wrapper(x, ln_gamma, ln_beta, epsilon, fp8_meta,
                              amax_index, output_dtype, sid):
  scaling_key = "scaling_fwd"
  scale = fp8_meta[scaling_key]["scale"].value()
  amax = fp8_meta[scaling_key]["amax_history"].value()
  scale_inv = fp8_meta[scaling_key]["scale_inv"].value()
  ln_out, mu, rsigma = tex.layernorm_fwd_fp8(
      x, ln_gamma, ln_beta, epsilon, scale, output_dtype, amax, scale_inv,
      amax_index, sid)
...
class TransformerEngineBaseModule:
  ...
  def set_meta_tensor(self, fwd):
    ...
    self.fp8_meta[fp8_meta_tensor_key]["amax_history"] = tf.Variable(
        tf.zeros((self.fp8_meta["recipe"].amax_history_len, num_fp8_tensors),
...
class Dense(TransformerEngineBaseModule, layers.Layer):
  ...
  def non_fp8_matmul(
    ...
    def non_fp8_matmul_func(x):
      ...
      bias = get_autocast_bias(self._compute_dtype_object, bias_var,
                               self.use_bias)

      output_dtype = self._compute_dtype_object
      outputs = matmul_wrapper(x, kernel_val, 'fwd', output_dtype,
                               self.stream_id, self.use_bias, bias)
    ...
    def fp8_matmul_func(x):
      ...
      if not override_linear_precision.wgrad:
        x_fp8, x_t_fp8 = fp8_cast_transpose_fused_wrapper(
            x, fp8_meta, tex.FP8FwdTensors.GEMM1_INPUT, True, fp8_dtype_fwd,
            self.stream_id)
      else:
        x_fp8 = cast_to_fp8_wrapper(x, fp8_meta, tex.FP8FwdTensors.GEMM1_INPUT,
                                    True, fp8_dtype_fwd, self.stream_id)

      weight_fp8, weight_t_fp8 = fp8_cast_transpose_fused_wrapper(
          kernel_val, fp8_meta, tex.FP8FwdTensors.GEMM1_WEIGHT, True,
          fp8_dtype_fwd, self.stream_id)

      output_dtype = self._compute_dtype_object
      outputs = fp8_matmul_wrapper(
          x_fp8, weight_t_fp8, fp8_meta, 'fwd', fp8_dtype_fwd, fp8_dtype_fwd,
          output_dtype, _2X_ACC_FPROP, self.stream_id, self.use_bias, bias)

      def grad_fn(upstream, variables=None):
        self.pre_backward(fp8_meta)
        if self.use_bias:
          grad_bias, grad_fp8, grad_t_fp8 = \
              fp8_cast_transpose_bgrad_fused_wrapper(
                  upstream, fp8_meta, tex.FP8BwdTensors.GRAD_OUTPUT1, False,
                  fp8_dtype_bwd, self.stream_id)
        else:
          if not override_linear_precision.wgrad:
        ...
        grad_x = fp8_matmul_wrapper(grad_fp8, weight_fp8, fp8_meta, 'bwd_input',
                                    fp8_dtype_bwd, fp8_dtype_fwd, output_dtype,
                                    _2X_ACC_DGRAD, self.stream_id)

        if not override_linear_precision.wgrad:
          grad_weight = fp8_matmul_wrapper(x_t_fp8, grad_t_fp8, fp8_meta,
                                           'bwd_weight', fp8_dtype_fwd,
                                           fp8_dtype_bwd, output_dtype,
                                           _2X_ACC_WGRAD, self.stream_id)
        else:
          grad_weight = matmul_wrapper(
              x, upstream, 'bwd_weight', output_dtype, self.stream_id)
...
class LayerNormDense(TransformerEngineBaseModule, layers.Layer):
  ...
  def non_fp8_layernorm_matmul(
    ...
    def non_fp8_layernorm_matmul_func(x):
      ...
      output_dtype = self._compute_dtype_object
      outputs = matmul_wrapper(ln_out, kernel_val, 'fwd', output_dtype,
                               self.stream_id, self.use_bias, bias)
    ...
    def fp8_layernorm_matmul_func(x):
      ...
      if not self.return_layernorm_output:
        ln_out, mu, rsigma = layernorm_fwd_fp8_wrapper(
            x, gamma_val, beta_val, self.epsilon, fp8_meta,
            tex.FP8FwdTensors.GEMM1_INPUT, fp8_dtype_fwd, self.stream_id)
      else:
        ln_out_return, mu, rsigma = tex.layernorm_fwd(
            x, gamma_val, beta_val, self.epsilon, self.stream_id)
        ln_out = cast_to_fp8_wrapper(
            ln_out_return, fp8_meta, tex.FP8FwdTensors.GEMM1_INPUT, True,
            fp8_dtype_fwd, self.stream_id)
      ...
      output_dtype = self._compute_dtype_object
      outputs = fp8_matmul_wrapper(
          ln_out, weight_t_fp8, fp8_meta, 'fwd', fp8_dtype_fwd, fp8_dtype_fwd,
          output_dtype, _2X_ACC_FPROP, self.stream_id, self.use_bias, bias)
      ...
      def grad_fn(*upstream, variables=None):
        ...
        if self.use_bias:
          grad_bias, grad_fp8, grad_t_fp8 = \
              fp8_cast_transpose_bgrad_fused_wrapper(
                  upstream[0], fp8_meta, tex.FP8BwdTensors.GRAD_OUTPUT1, False,
                  fp8_dtype_bwd, self.stream_id)
        else:
          if not override_linear_precision.wgrad:
            grad_fp8, grad_t_fp8 = fp8_cast_transpose_fused_wrapper(
        ...
        if not override_linear_precision.wgrad:
          ln_out_t = tex.fp8_transpose(
              ln_out, fp8_dtype_fwd, self.stream_id)
          grad_weight = fp8_matmul_wrapper(
              ln_out_t, grad_t_fp8, fp8_meta, 'bwd_weight', fp8_dtype_fwd,
              fp8_dtype_bwd, output_dtype, _2X_ACC_WGRAD, self.stream_id)
        else:
          ln_out_c = cast_from_fp8_wrapper(
              ln_out, fp8_meta, tex.FP8FwdTensors.GEMM1_INPUT, True,
              fp8_dtype_fwd, TE_DType[x.dtype], self.stream_id)
          grad_weight = matmul_wrapper(
              ln_out_c, upstream[0], 'bwd_weight', output_dtype, self.stream_id)
  ...
  def call(
    ...
    inputmat = tf.reshape(inputs, shape=(-1, inputs.shape[-1]))
    if self.fp8:
      outputs = self.fp8_layernorm_matmul(inputmat, self.gamma, self.beta,
                                          kernel_var, bias_var)
    else:
      outputs = self.non_fp8_layernorm_matmul(inputmat, self.gamma, self.beta,
                                              kernel_var, bias_var)
    if self.return_layernorm_output:
...
class LayerNormMLP(TransformerEngineBaseModule, layers.Layer):
  ...
  def non_fp8_layernorm_mlp(
    ...
    def non_fp8_layernorm_mlp_func(x):
      ...
      fc1_bias = get_autocast_bias(self._compute_dtype_object, fc1_bias_var,
                                   use_bias=True)
      fc2_bias = get_autocast_bias(self._compute_dtype_object, fc2_bias_var,
                                   self.use_bias)
      ...
      fc1_out = matmul_wrapper(
          ln_out, fc1_kernel_val, 'fc1_fwd', output_dtype, self.stream_id,
          use_bias=True, bias=fc1_bias)
      gelu_out = tex.te_gelu(
          fc1_out, None, TE_DType[output_dtype], None, None, 0, self.stream_id)

      fc2_out = matmul_wrapper(
          gelu_out, fc2_kernel_val, 'fc2_fwd', output_dtype, self.stream_id,
          use_bias=self.use_bias, bias=fc2_bias)

      def grad_fn(*upstream, variables=None):
        fc2_dgrad = matmul_wrapper(
            upstream[0], fc2_kernel_val, 'fc2_bwd_input', output_dtype,
            self.stream_id, grad=True, gelu=True, gelu_input=fc1_out)
  ...
  def fp8_layernorm_mlp(
    ...
    def fp8_layernorm_mlp_func(x):
      ...
      fc1_weight_fp8, fc1_weight_t_fp8 = fp8_cast_transpose_fused_wrapper(
          fc1_kernel_val, fp8_meta, tex.FP8FwdTensors.GEMM1_WEIGHT, True,
          fp8_dtype_fwd, self.stream_id)

      fc2_weight_fp8, fc2_weight_t_fp8 = fp8_cast_transpose_fused_wrapper(
          fc2_kernel_val, fp8_meta, tex.FP8FwdTensors.GEMM2_WEIGHT, True,
          fp8_dtype_fwd, self.stream_id)

      output_dtype = self._compute_dtype_object
      fc1_out = fp8_matmul_wrapper(
          ln_out, fc1_weight_t_fp8, fp8_meta, 'fc1_fwd', fp8_dtype_fwd,
          fp8_dtype_fwd, output_dtype, _2X_ACC_FPROP, self.stream_id,
          use_bias=True, bias=fc1_bias)

      gelu_out = fp8_gelu_wrapper(
          fc1_out, fp8_meta, tex.FP8FwdTensors.GEMM2_INPUT, True, fp8_dtype_fwd,
          self.stream_id)

      fc2_out = fp8_matmul_wrapper(
          gelu_out, fc2_weight_t_fp8, fp8_meta, 'fc2_fwd', fp8_dtype_fwd,
          fp8_dtype_fwd, output_dtype, _2X_ACC_FPROP, self.stream_id,
          use_bias=self.use_bias, bias=fc2_bias)
      ...
      def grad_fn(*upstream, variables=None):
        self.pre_backward(self.fp8_meta)
        if self.use_bias:
          fc2_bias_grad, grad_fp8, grad_t_fp8 = \
              fp8_cast_transpose_bgrad_fused_wrapper(
                  upstream[0], fp8_meta, tex.FP8BwdTensors.GRAD_OUTPUT1, False,
                  fp8_dtype_bwd, self.stream_id)
        else:
        ...
        fc2_dgrad = fp8_matmul_wrapper(
            grad_fp8, fc2_weight_fp8, fp8_meta, 'fc2_bwd_input', fp8_dtype_bwd,
            fp8_dtype_fwd, output_dtype, _2X_ACC_DGRAD, self.stream_id)

        if not override_linear_precision.wgrad:
          gelu_out_t = tex.fp8_transpose(gelu_out, fp8_dtype_fwd,
                                         self.stream_id)
          fc2_wgrad = fp8_matmul_wrapper(
              gelu_out_t, grad_t_fp8, fp8_meta, 'fc2_bwd_weight', fp8_dtype_fwd,
              fp8_dtype_bwd, output_dtype, _2X_ACC_WGRAD, self.stream_id)

          fc1_bias_grad, dgelu, dgelu_t = \
            fp8_cast_transpose_bgrad_dgelu_fused_wrapper(
                fc2_dgrad, fc1_out, fp8_meta, tex.FP8BwdTensors.GRAD_OUTPUT2,
                False, fp8_dtype_bwd, self.stream_id)
        else:
          gelu_out_c = cast_from_fp8_wrapper(
              gelu_out, fp8_meta, tex.FP8FwdTensors.GEMM2_INPUT, True,
              fp8_dtype_fwd, TE_DType[x.dtype], self.stream_id)
          fc2_wgrad = matmul_wrapper(gelu_out_c, upstream[0], 'bwd_weight',
                                     output_dtype, self.stream_id)
          ...
          dgelu = cast_to_fp8_wrapper(
              dgelu_no_fp8, fp8_meta, tex.FP8BwdTensors.GRAD_OUTPUT2, False,
              fp8_dtype_bwd, self.stream_id)
          dgelu_t = None
        
        fc1_dgrad = fp8_matmul_wrapper(
            dgelu, fc1_weight_fp8, fp8_meta, 'fc1_bwd_input', fp8_dtype_bwd,
            fp8_dtype_fwd, output_dtype, _2X_ACC_DGRAD, self.stream_id)

        if not override_linear_precision.wgrad:
          ln_out_t = tex.fp8_transpose(ln_out, fp8_dtype_fwd, self.stream_id)
          fc1_wgrad = fp8_matmul_wrapper(
              ln_out_t, dgelu_t, fp8_meta, 'fc1_bwd_weight', fp8_dtype_fwd,
              fp8_dtype_bwd, output_dtype, _2X_ACC_WGRAD, self.stream_id)
        else:
          ln_out_c = cast_from_fp8_wrapper(
              ln_out, fp8_meta, tex.FP8FwdTensors.GEMM1_INPUT, True,
              fp8_dtype_fwd, TE_DType[x.dtype], self.stream_id)
          fc1_wgrad = matmul_wrapper(ln_out_c, dgelu_no_fp8, 'bwd_weight',
                                     output_dtype, self.stream_id)
  ...
  def call(
    ...
    inputmat = tf.reshape(inputs, shape=(-1, inputs.shape[-1]))
    if self.fp8:
      outputs = self.fp8_layernorm_mlp(inputmat, self.gamma, self.beta,
                                       self.fc1_kernel, self.fc1_bias,
                                       self.fc2_kernel, self.fc2_bias)
    else:
      outputs = self.non_fp8_layernorm_mlp(inputmat, self.gamma, self.beta,
                                           self.fc1_kernel, self.fc1_bias,
                                           self.fc2_kernel, self.fc2_bias)
    if self.return_layernorm_output:
      outputmat, ln_outputmat = outputs
    ...
    config.update(
        {
            ...
            "return_layernorm_output": self.return_layernorm_output,
            "use_bias": self.use_bias,
            "init_method": initializers.serialize(
                self.kernel1_initializer
            ),
