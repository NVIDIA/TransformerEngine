...
inline void runWithWorkspace(Fn&& fn, int32_t device_idx) {
  ...
  // workspace_data must outlive the second fn() call because the kernel
  // launched by fn() reads from the workspace via a raw data_ptr().
  // Declaring it here (outside the if block) keeps the tensor alive until
  // after the kernel is submitted.
  torch::stable::Tensor workspace_data;
