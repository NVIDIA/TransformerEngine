...
static int oob_gather(void *comm_context, int root, void *sbuf, void *rbuf,
                      int len) {
...
int pipe_rank(communicator *comm, int step) {
  ...
  int newnode = mynode;
  newnode +=
      (newlocal1 - newlocal) / numlocal * comm->num_nodes * comm->num2_nodes;
  int allnodes = comm->nranks / comm->nvsize;
  newnode = (allnodes + (newnode % allnodes)) % allnodes;
  return newnode * numlocal + newlocal;
}

int create_communicator_grouped2(communicator **comm, int pipegpus,
                                 int pipenodes, int tensorgpus,
                                 int tensornodes) {
  *comm = reinterpret_cast<communicator *>(malloc(sizeof(communicator)));
  ...
  (*comm)->ar_firstgpu =
      mylocal - ((mylocal / tensorgpus) % datagpus) * tensorgpus;
  ...
  (*comm)->pipe_id =
      pipegpus * pipenodegroup_id + mylocal / (datagpus * tensorgpus);

  CUDACHECK(cudaFree(0));
  int datanodegroup_id =
      myrank / numlocal /
      datanodes;  // data reduction group node belongs, equals 0 for all if both
                  // pipenodes=1 and tensornodes=1
  // mpi communicator only needed for SHARP which is always
  // allreduce1/data-parallel
  MPI_Comm_split(MPI_COMM_WORLD, mylocal + numlocal * datanodegroup_id, rank,
                 &(*comm)->comm_inter);
  ...
  (*comm)->fifo = reinterpret_cast<ub_request *>(
      malloc(sizeof(ub_request) * NVTE_MAX_REQUESTS));
  (*comm)->nblocks = 8;
...
#define NBUF 2
  if ((*comm)->sm_arch >= 9 && (*comm)->ar2_nvsize > 1 &&
      !getenv(
          "UB_SKIPMC")) {  // multicast init only for TP ops (____2 operations)
    ...
    CUCHECK(cuMulticastGetGranularity(&gran, &mcProp,
    ...
    NCCLCHECK(ncclIpcSocketInit(&ipcSock, (*comm)->ar2_nvrank, (uint64_t)opId,
                                &abortFlag));
    ...
    if ((*comm)->ar2_nvrank == 0) {
      ...
      CUCHECK(cuMemExportToShareableHandle(
          &fd, (*comm)->mc_handle, CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR,
          0 /*flags*/));
      for (int p = 1; p < (*comm)->ar2_nvsize; p++) {
        MPI_Barrier((*comm)->comm_intra);
        NCCLCHECKGOTO(ncclIpcSocketSendFd(&ipcSock, fd, p, (uint64_t)opId), ret,
                      error);
      }
    } else {
      ...
      CUCHECK(cuMemImportFromShareableHandle(
          &(*comm)->mc_handle, reinterpret_cast<void *>(fd),
...
#define LOCALSIZE                                                              \
  4 * (NVTE_REG0_OFFSET(*comm) + NVTE_REG0_FLAGS + NVTE_REG0_COMMBUFFER * NBUF)
...
#define GPU_PAGE_MASK (~GPU_PAGE_OFFSET)
  ...
  (*comm)->flags = reinterpret_cast<int *>(
      ((CUdeviceptr)(*comm)->flags + GPU_PAGE_SIZE - 1) & GPU_PAGE_MASK);
  ...
  gdr_mh_t mh;
  ret = gdr_pin_buffer((*comm)->g, (CUdeviceptr)(*comm)->flags, GPU_PAGE_SIZE,
                       0, 0, &mh);
  if (ret) {
  ...
  if (getenv("NVTE_UBDEBUG"))
    printf("%d/%d:(%d x %d): DP %d x %d TP %d x %d, DPGROUP %dx%d TPGROUP "
           "%dx%d PIPE_ID %d/%d\n",
           myrank, nranks, myrank / numlocal, myrank % numlocal,
           (*comm)->my_node, (*comm)->ar_nvrank, (*comm)->my2_node,
           (*comm)->ar2_nvrank, (*comm)->num_nodes, (*comm)->ar_nvsize,
           (*comm)->num2_nodes, (*comm)->ar2_nvsize, (*comm)->pipe_id,
           pipegpus * pipenodes);
  fflush(NULL);

  return 0;
}
int create_communicator_grouped(communicator **comm, int pipegpus,
                                int pipenodes) {
...
int register_user_buffer_collective(void **gpubuff, size_t bytes,
  ...
  comm->peer_ptr[hndl] =
      reinterpret_cast<void **>(malloc(sizeof(void *) * (comm->nvsize)));
  ...
  if (alloc) {
    ...
    if (comm->use_mc) {
      ...
      CUCHECK(cuMulticastGetGranularity(&granularity, &mcProp,
                                        CU_MULTICAST_GRANULARITY_MINIMUM));
      aligned_size =
          (aligned_size + granularity - 1) / granularity * granularity;
    }

    prop.location.id = comm->mydev;
    comm->uchandles[hndl] = reinterpret_cast<CUmemGenericAllocationHandle *>(malloc(
        nranks * sizeof(CUmemGenericAllocationHandle)));
    CUCHECK(
        cuMemCreate(&(comm->uchandles[hndl][myrank]), aligned_size, &prop, 0));

    int *peerfd = reinterpret_cast<int *>(malloc(nranks * sizeof(int)));
    CUCHECK(cuMemExportToShareableHandle(
        &peerfd[myrank], comm->uchandles[hndl][myrank],
    ...
    for (int p = 1; p < nranks; p++) {
      MPI_Barrier(comm->comm_intra);
      NCCLCHECKGOTO(ncclIpcSocketSendFd(&ipcSock, peerfd[myrank],
                                        (myrank + p) % nranks, (uint64_t)opId),
                    ret, error);
      NCCLCHECKGOTO(ncclIpcSocketRecvFd(
                        &ipcSock, &peerfd[(myrank + nranks - p) % nranks]),
                    ret, error);
    }
  error:
    NCCLCHECK(ncclIpcSocketClose(&ipcSock));
    ...
    for (int i = 0; i < nranks; i++) {
      CUCHECK(cuMemMap(ptr + (aligned_size * i), aligned_size, 0,
                       comm->uchandles[hndl][i], 0));
    ...
    CUDACHECK(cudaMemcpy(
        (reinterpret_cast<char *>(comm->gpu_ptrs)) + (hndl * nranks * sizeof(void *)), remptrs,
    ...
    if (comm->use_mc && comm->mc_maxsize >= comm->mc_offset + aligned_size) {
      CUCHECK(cuMulticastBindMem(comm->mc_handle, comm->mc_offset,
    ...
    cudaIpcMemHandle_t *memhndl = reinterpret_cast<cudaIpcMemHandle_t *>(
        malloc(sizeof(cudaIpcMemHandle_t) * (comm->nvsize)));

    CUDACHECK(cudaIpcGetMemHandle(&memhndl[comm->nvrank], *gpubuff));

    MPI_Allgather(&memhndl[comm->nvrank], sizeof(cudaIpcMemHandle_t), MPI_BYTE,
                  memhndl, sizeof(cudaIpcMemHandle_t), MPI_BYTE,
                  comm->comm_intra);

    for (int i = 0; i < comm->nvsize; i++)
      if (i != comm->nvrank)
        CUDACHECK(cudaIpcOpenMemHandle(
            (void **)&(comm->peer_ptr[hndl][i]), // NOLINT(*)
            memhndl[i], cudaIpcMemLazyEnablePeerAccess));
    comm->peer_ptr[hndl][comm->nvrank] = *gpubuff;
    CUDACHECK(cudaDeviceSynchronize());

    CUDACHECK(cudaMemcpy(reinterpret_cast<char *>(comm->gpu_ptrs) +
                             (hndl * comm->nvsize * sizeof(void *)),
...
void allreduce_nonsharp_inplace(const int handler, const int offset,
  ...
  const int num_nodes = op == userbuffers_allreduceop_nonsharp
                            ? comm->num_nodes
                            : comm->num2_nodes;
  blocksize = (comm->nblocks - 1 +
               (comm->alignblock - 1 + elements * 2) / comm->alignblock) /
  ...
  size_t peerblock =
      sizeof(int) * NVTE_REG0_COMMBUFFER / maxcredit;  // max size we can fit
  if (blocksize > peerblock * ar_nvsize)
    blocksize = peerblock * ar_nvsize;
  int sms = allreduce2_userbuff_inplace_gpu(
...
void allreduce2_userbuff_inplace(const int handler, const int offset,
                                 const int elements, communicator *comm,
                                 cudaStream_t stream) {
...
void allreduce_userbuff_inplace(const int handler, const int offset,
                                const int elements, communicator *comm,
                                cudaStream_t stream) {
  if (elements < 64)
    NVTE_UB_ERROR("Userbuffer comm for given config not implemented.");
...
void reducescatter_userbuff_inplace(const int handler, const int offset,
                                    const int elements, communicator *comm,
                                    cudaStream_t stream) {
  if (elements < 64)
    NVTE_UB_ERROR("Userbuffer comm for given config not implemented.");
  ...
  int sms = reducescatter2_userbuff_inplace_gpu(
      maxcredit, handler, offset, elements, blocksize, comm, stream, op);
...
void allgather_userbuff_inplace(const int handler, const int offset,
                                const int elements, communicator *comm,
                                cudaStream_t stream) {
  if (elements < 64)
    NVTE_UB_ERROR("Userbuffer comm for given config not implemented.");
  ...
  int sms = allgather2_userbuff_inplace_gpu(
      maxcredit, handler, offset, elements, blocksize, comm, stream, op);
}
