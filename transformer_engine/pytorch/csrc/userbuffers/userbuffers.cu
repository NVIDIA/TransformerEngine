...
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rw(const int op, const int flagoffset,
                                        const int firstrank, const int myrank,
                                        const int gpustep, const int lineoffset,
                                        const int numlines, void **commbuff,
                                        const int handleridx) {
...
#pragma unroll
  ...
  __syncthreads();
  for (int line = threadIdx.x + blockDim.x * (myrank + RANKS * blockIdx.x);
       line < numlines; line += blockDim.x * gridDim.x * RANKS) {
    int4 val[RANKS];
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr(const int op, const int flagoffset,
                                        const int firstrank, const int myrank,
                                        const int gpustep, const int lineoffset,
                                        const int numlines, void **commbuff,
                                        const int handleridx) {
...
#pragma unroll
  ...
  for (int line = threadIdx.x + blockDim.x * RANKS * blockIdx.x;
       line < numlines; line += blockDim.x * gridDim.x * RANKS) {
    int4 val[RANKS - 1];
...
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_rs_oop(
        const int op, const int flagoffset, const int firstrank,
        const int myrank, const int gpustep, const int mylineoffset,
        const int totallines, const int rowlines, const int skiplines,
        void **commbuff, const int handleridx, void *outbuf) {
  __shared__ int4 *userptr[RANKS];
  volatile int *flagptr;
  int physgpu, targetgpu, *myptr;
  int *reduceidptr, reduce_id;
...
#pragma unroll
    ...
    (reinterpret_cast<int4 *>(
        outbuf))[(line / rowlines) * skiplines + (line % rowlines)] = sum;
  }
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_mc(const int op, const int flagoffset,
                                        const int firstrank, const int myrank,
                                        const int gpustep, const int lineoffset,
                                        const int numlines, void **commbuff,
                                        const int handleridx, float4 *mc_ptr) {
  int *flagptr, physgpu, targetgpu, *myptr;
  int *reduceidptr, reduce_id;
...
#define UNROLL_MC 8
  ...
  const int start_elem =
      threadIdx.x + blockDim.x * (myrank + RANKS * blockIdx.x);
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_mc_rs(
        const int op, const int flagoffset, const int firstrank,
        const int myrank, const int gpustep, const int mylineoffset,
        const int totallines, void **commbuff, const int handleridx,
        float4 *mc_ptr) {
  ...
  int physgpu, targetgpu, *myptr;
  int *reduceidptr, reduce_id;
  uint4 *localptr = reinterpret_cast<uint4 *>(
      commbuff[myrank * gpustep + firstrank + handleridx]);
  int lastSM = 0;
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_mc_rs_oop(
        const int op, const int flagoffset, const int firstrank,
        const int myrank, const int gpustep, const int mylineoffset,
        const int totallines, const int rowlines, const int skiplines,
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_mc_ag(
        const int op, const int flagoffset, const int firstrank,
        const int myrank, const int gpustep, const int mylineoffset,
        const int totallines, void **commbuff, const int handleridx,
        uint4 *mc_ptr) {
...
__global__ void __launch_bounds__(MAX_THREADS)
  ...
  for (int chunk_i = 0; chunk_i < numchunks; chunk_i++) {
    ...
    if (threadIdx.x == 0) {
      const int adder = blockIdx.x == 0 ? NVTE_MAX_SMS - gridDim.x + 1 : 1;
      int old_val =
          atomicAdd(myptr + (NVTE_MAX_NVLINK * 2), /*numchunks * */ adder);
...
#pragma unroll
    ...
    for (int line = threadIdx.x + blockDim.x * blockIdx.x; line < totallines;
      ...
      const int index1_out = chunk_i * mylineoffset * 2 +
                             ((2 * line) / rowlines) * skiplines_out +
                             ((2 * line) % rowlines);
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_rs_oop_stride(
        const int op, const int flagoffset, const int firstrank,
        const int myrank, const int gpustep, const int mylineoffset,
        const int totallines, const int rowlines, const int skiplines,
        void **commbuff, const int handleridx, void *outbuf) {
...
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_rs_oop_stride_atomic(
        ...
        const int myrank, const int gpustep, const int mylineoffset,
        const int totallines, const int rowlines, const int skiplines,
        const int numchunks, void **commbuff, const int handleridx,
  ...
  if (counters) {
    if (threadIdx.x == 0) {
      ...
      int old_val2;
      atomicInc(((unsigned int *)counters) + numchunks, gridDim.x - 1);
      while (0 != (old_val2 = atomicCAS(((unsigned int *)counters) + numchunks,
                                        0, 0))) {
      }
  ...
  for (int chunk_i = 0; chunk_i < numchunks; chunk_i++) {
    if (counters) {
      if (threadIdx.x == 0) {
        // spin-lock on counter from producer
        int old_val;
        while (0 != (old_val = atomicCAS(((unsigned int *)counters) + chunk_i,
                                         0, 0))) {
        }

        // make sure all threadblocks have read/waited on counters.
        int old_val2;
        atomicInc(((unsigned int *)counters) + numchunks + chunk_i,
                  gridDim.x - 1);
        while (0 !=
               (old_val2 = atomicCAS(
                    ((unsigned int *)counters) + numchunks + chunk_i, 0, 0))) {
        }
...
#pragma unroll
    ...
    for (int line = threadIdx.x + blockDim.x * blockIdx.x; line < totallines;
         line += blockDim.x * gridDim.x) {
      int4 val[RANKS];
      int index_in = chunk_i * mylineoffset +
                     myrank * (totallines * skiplines / rowlines) +
                     (line / rowlines) * skiplines + (line % rowlines);
...
#pragma unroll
      ...
      int index_out = chunk_i * mylineoffset + (line / rowlines) * skiplines +
                      (line % rowlines);
      (reinterpret_cast<int4 *>(outbuf))[index_out] = sum;
    }
    if (threadIdx.x == 0 && lastSM)
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_ag(
        const int op, const int flagoffset, const int firstrank,
        const int myrank, const int gpustep, const int mylineoffset,
        const int totallines, void **commbuff, const int handleridx) {
  __shared__ int4 *userptr[RANKS];
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_blocked(
        const int op, const int flagoffset, const int firstrank,
        const int myrank, const int lineoffset, const int numlines,
        void **commbuff, const int handleridx, const int peerblocklines,
        int *hostflags, int *gpuflag, const int numblocks) {
  ...
  if (threadIdx.x < 32) {
    ...
    for (int nblock = 0; nblock < endblock; nblock++) {
      ...
      if (threadIdx.x == 0) {
        __threadfence();
        if (blockIdx.x)
          gpuflag[op * NVTE_MAX_SMS * 2 + blockIdx.x] =
              nblock + basecounter + 1;
      } else if (blockIdx.x == 0) {
        int expecting = (basecounter + nblock + 1);
        if (threadIdx.x < gridDim.x)
          while (
              ((volatile int *)gpuflag)[op * NVTE_MAX_SMS * 2 + threadIdx.x] <
              expecting) {
          }
      }
      if (!blockIdx.x) {
...
#pragma unroll
    for (int i = 0; i < RANKS; i++)
      userptr[i] = reinterpret_cast<int4 *>(
          commbuff[((i + myrank + warp) & (RANKS - 1)) + handleridx +
                   firstrank]);
    userptrmyrank =
        reinterpret_cast<int4 *>(commbuff[myrank + handleridx + firstrank]);
    __syncthreads();
    ...
    while (blocklineoffset < numlines) {
      const int remainder =
          min(numlines - blocklineoffset, peerblocklines * RANKS);
      const int blocklines = remainder / RANKS;
      const int blockstart = lineoffset + blocklineoffset + blocklines * myrank;

      for (int line = threadIdx.x - 32 + REDUCETHREADS * blockIdx.x;
           line < blocklines; line += REDUCETHREADS * gridDim.x) {
        int4 val[RANKS];
...
#define UNROLL 6
      ...
      if (threadIdx.x < maxthreadIdx) {
        ...
        const int aligned_elem =
            ((end_elem - start_elem) / (myblockDim * gridDim.x * UNROLL)) *
        ...
        for (int line = start_elem; line < end_aligned;
             line += myblockDim * gridDim.x * UNROLL) {
          int4 val[UNROLL];
#pragma unroll
...
#pragma unroll
        ...
        for (int line = end_aligned; line < end_elem;
             line += myblockDim * gridDim.x)
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_blocked2(
        const int op, const int maxcredit, const int headstart,
        const int myibrank, const int ibranks, const int commbufoffset,
        const int flagoffset, const int firstrank, const int myrank,
        const int gpustep, const int lineoffset, const int numlines,
        void **commbuff, const int handleridx, const int peerblocklines,
        int *hostflags, int *gpuflag, const int numblocks) {
  const int basecounter = gpuflag[NVTE_GF_STATE + op];
  if (threadIdx.x < 32) {
    ...
    if (threadIdx.x < RANKS) {
      if (!blockIdx.x) {
        flagptr = reinterpret_cast<int *>(
            commbuff[gpustep * threadIdx.x + firstrank]);
    ...
    for (int nblock = 0; nblock < numblocks + headstart; nblock++) {
      ...
      if (nblock >= headstart) {
        for (int ibflag = threadIdx.x; ibflag < ibranks; ibflag += 32)
          if (ibflag != myibrank)
            while (localflag[NVTE_REG0_IBRS + ibflag] <
                   basecounter + nblock - headstart + 1) {
            }
        asm volatile("bar.sync 13, %0;" ::"r"(blockDim.x));
        // REDUCE happens here
        SMBAR(op * 2 * NVTE_MAX_SMS + NVTE_MAX_SMS, nblock - headstart);
        if (!blockIdx.x && !threadIdx.x)
          hostflags[NVTE_HF_NVREDUCEDONE + (op & 1)] =
              nblock + basecounter + 1 - headstart;
      }
    }
    ...
    if (blockIdx.x == 0) {
      for (int nblock = 0; nblock < numblocks; nblock++) {
        ...
        asm volatile("bar.sync 15, %0;" ::"r"(32));
        if (threadIdx.x < RANKS)
          flagptr[flagoffset + gpustep * myrank + NVTE_MAX_NVLINK + firstrank] =
              expected;
      }
    }
...
#pragma unroll
    for (int i = 0; i < RANKS; i++)
      userptr[i] = reinterpret_cast<int4 *>(
          commbuff[((i + myrank + warp) & (RANKS - 1)) * gpustep + handleridx +
                   firstrank]);
    userptrmyrank = reinterpret_cast<int4 *>(
        commbuff[gpustep * myrank + handleridx + firstrank]);
    int4 *internalbuf = reinterpret_cast<int4 *>(
        commbuff[myrank * gpustep + firstrank] + commbufoffset * sizeof(int));
    __syncthreads();
    ...
    for (int nblock = 0; nblock < numblocks + headstart; nblock++) {
      ...
      if (blocklineoffset < numlines) {
        ...
        const int blockstart =
            lineoffset + blocklineoffset + blocklines * myrank;
        if (RANKS > 1) {
          for (int line = threadIdx.x - 32 + REDUCETHREADS * blockIdx.x;
               line < blocklines; line += REDUCETHREADS * gridDim.x) {
            int4 val[RANKS];
...
#define UNROLLRS 2
        const int remainder =
            min(numlines - rblocklineoffset, peerblocklines * RANKS);
        const int blocklines = remainder / RANKS;
        rblocklineoffset += peerblocklines * RANKS;
        const int ibblocklines = blocklines / ibranks;
        int4 *tempbufptr =
            &internalbuf[((nblock - headstart) % maxcredit) * peerblocklines];
        const int tempstart = lineoffset +
                              (nblock - headstart) * peerblocklines * RANKS +
                              myrank * blocklines + ibblocklines * myibrank;

        asm volatile("bar.sync 13, %0;" ::"r"(REDUCETHREADS + 32));

        for (int line = threadIdx.x - 32 + REDUCETHREADS * blockIdx.x;
             line < ibblocklines; line += REDUCETHREADS * gridDim.x) {
          int4 val[UNROLLRS];
...
#pragma unroll
          ...
          for (int i = 0; i < ibranks - UNROLLRS; i++) {
            val[i % UNROLLRS] = i == myibrank
                                    ? userptrmyrank[tempstart + line]
                                    : tempbufptr[i * ibblocklines + line];
...
#pragma unroll
    ...
    if (RANKS != 1) {
      ...
      volatile int *flag = (volatile int *)&(
          (reinterpret_cast<int *>(commbuff[gpustep * myrank + firstrank]))
              [flagoffset + gpustep * mydest + NVTE_MAX_NVLINK + firstrank]);
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_blocked2_rs(
        const int op, const int maxcredit, const int headstart,
        const int myibrank, const int ibranks, const int commbufoffset,
        const int flagoffset, const int firstrank, const int myrank,
        const int gpustep, const int lineoffset, const int numlines,
...
template <int RANKS>
__global__ void __launch_bounds__(MAX_THREADS)
    userbuffers_fp16_sum_inplace_gpu_rr_blocked2_ag(
        const int op, const int maxcredit, const int headstart,
        const int myibrank, const int ibranks, const int commbufoffset,
        const int flagoffset, const int firstrank, const int myrank,
        const int gpustep, const int lineoffset, const int numlines,
...
__global__ void userbuffers_fp16_sum_inplace_gpu_null(const int op,
                                                      int *hostflags,
                                                      int *gpuflag,
                                                      int numblocks) {
...
#define callranks_block(x)                                                     \
  if (comm->ar_nvsize == x)                                                    \
    userbuffers_fp16_sum_inplace_gpu_rr_blocked<x>                             \
        <<<sms, warps * 32, 0, stream>>>(                                      \
            userbuffers_allreduceop_sharp, NVTE_REG0_OFFSET(comm),             \
            comm->ar_firstgpu, comm->ar_nvrank, offset / 8, elements / 8,      \
            reinterpret_cast<void **>(comm->gpu_ptrs), handler * comm->nvsize, \
            blocksize / sizeof(int4) / comm->ar_nvsize,                        \
...
#define callranks2_block(x)                                                    \
  if (ar_nvsize == x) {                                                        \
    ...
    if (headstart == 0)                                                        \
      headstart = 1;                                                           \
    userbuffers_fp16_sum_inplace_gpu_rr_blocked2<x>                            \
        <<<sms, warps * 32, 0, stream>>>(                                      \
            op, maxcredit, headstart, my_node, num_nodes,                      \
            NVTE_REG0_OFFSET(comm) + NVTE_REG0_FLAGS +                         \
                (op == userbuffers_allreduceop_nonsharp ? NVTE_REG0_COMMBUFFER \
                                                        : 0),                  \
            NVTE_REG0_OFFSET(comm) + NVTE_REG0_OPFLAGS * op, ar_firstgpu,      \
            ar_nvrank, ar_step, offset / 8, elements / 8,                      \
            reinterpret_cast<void **>(comm->gpu_ptrs), handler * comm->nvsize, \
            blocksize / sizeof(int4) / ar_nvsize,                              \
...
#define callranks2_block_rs(x)                                                 \
  if (ar_nvsize == x) {                                                        \
    ...
    if (headstart == 0)                                                        \
      headstart = 1;                                                           \
    userbuffers_fp16_sum_inplace_gpu_rr_blocked2_rs<x>                         \
        <<<sms, warps * 32, 0, stream>>>(                                      \
            op, maxcredit, headstart, my_node, num_nodes,                      \
            NVTE_REG0_OFFSET(comm) + NVTE_REG0_FLAGS +                         \
                (op == userbuffers_allreduceop_nonsharp ? NVTE_REG0_COMMBUFFER \
...
#define callranks2_block_ag(x)                                                 \
  if (ar_nvsize == x) {                                                        \
    ...
    if (headstart == 0)                                                        \
      headstart = 1;                                                           \
    userbuffers_fp16_sum_inplace_gpu_rr_blocked2_ag<x>                         \
        <<<sms, warps * 32, 0, stream>>>(                                      \
            op, maxcredit, headstart, my_node, num_nodes,                      \
            NVTE_REG0_OFFSET(comm) + NVTE_REG0_FLAGS +                         \
                (op == userbuffers_allreduceop_nonsharp ? NVTE_REG0_COMMBUFFER \
  ...
  if (ar_nvsize == x) {                                                        \
    ...
    CUDACHECK(cudaLaunchKernelExC(                                             \
        ...
        reinterpret_cast<void *>(                                              \
            comm->use_rr_kernel ? userbuffers_fp16_sum_inplace_gpu_rr<x>       \
...
#define SETUP_LAUNCH_CONFIG(sms, threads, stream)                              \
  ...
  attribute_ub[1].val.clusterDim.x =                                           \
      sms % comm->cga_size == 0 ? comm->cga_size : 1;                          \
...
int allreduce_userbuff_inplace_gpu(const int handler, const int offset,
                                   const int elements, const int blocksize,
...
int allreduce2_userbuff_inplace_gpu(const int maxcredit, const int handler,
                                    const int offset, const int elements,
                                    const int blocksize, communicator *comm,
  ...
  const int num_nodes = op == userbuffers_allreduceop_nonsharp
                            ...
                            : comm->num2_nodes;
  const int my_node =
      op == userbuffers_allreduceop_nonsharp ? comm->my_node : comm->my2_node;
  ...
  if (warps < ar_nvsize)
    warps = ar_nvsize;
  if (num_nodes > 1) {
    callranks2_block(1) callranks2_block(2) callranks2_block(4)
        callranks2_block(8)
  } else {
...
#define callranks_ag(x)                                                        \
  if (ar_nvsize == x) {                                                        \
    ...
    CUDACHECK(cudaLaunchKernelExC(                                             \
        &cfg,                                                                  \
        reinterpret_cast<void *>(                                              \
            comm->use_rr_kernel ? userbuffers_fp16_sum_inplace_gpu_rr_ag<x>    \
  ...
  if (ar_nvsize == x) {                                                             \
    int arg1 = op - NVTE_MAX_OPS,                                                   \
        arg2 = NVTE_REG0_OFFSET(comm) -                                             \
               ...
               NVTE_MAX_OPS,                                                        \
        arg3 = ar_firstgpu, arg4 = ar_nvrank, arg5 = ar_step,                       \
        arg7 = elements / 8 / x, arg6 = offset / 8 + arg4 * arg7;                   \
    void **arg8 = reinterpret_cast<void **>(comm->gpu_ptrs);                        \
    int arg9 = handler * comm->nvsize;                                              \
    uint4 *arg10 = reinterpret_cast<uint4 *>(comm->mc_ptr[handler]);                \
    void *kernelArgs[] = {reinterpret_cast<void *>(&arg1),                          \
                          ...
                          reinterpret_cast<void *>(&arg7),                          \
                          reinterpret_cast<void *>(&arg8),                          \
                          reinterpret_cast<void *>(&arg9),                          \
                          reinterpret_cast<void *>(&arg10)};                        \
    CUDACHECK(cudaLaunchKernelExC(                                                  \
        &cfg, reinterpret_cast<void *>(userbuffers_fp16_sum_inplace_gpu_mc_ag<x>),  \
        kernelArgs));                                                               \
  }
...
#define callranks_rsMC(x)                                                      \
  if (ar_nvsize == x) {                                                        \
    ...
    CUDACHECK(cudaLaunchKernelExC(                                             \
        &cfg,                                                                  \
        reinterpret_cast<void *>(userbuffers_fp16_sum_inplace_gpu_mc_rs<x>),   \
        kernelArgs));                                                          \
  }
...
#define callranks_rs_oop(x)                                                    \
  if (ar_nvsize == x) {                                                        \
    int arg1 = op - NVTE_MAX_OPS,                                              \
        ...
        arg3 = ar_firstgpu, arg4 = ar_nvrank, arg5 = ar_step,                  \
        arg7 = elements / 8 / x, arg6 = offset / 8 + arg4 * arg7,              \
        arg8 = rowelements / 8, arg9 = strideelements / 8;                     \
    ...
    CUDACHECK(cudaLaunchKernelExC(                                             \
        &cfg,                                                                  \
        reinterpret_cast<void *>(                                              \
            userbuffers_fp16_sum_inplace_gpu_rr_rs_oop<x>),                    \
        kernelArgs));                                                          \
  }
  ...
  if (ar_nvsize == x) {                                                                         \
    int arg1 = op - NVTE_MAX_OPS,                                                               \
        arg2 = NVTE_REG0_OFFSET(comm) -                                                         \
               ...
               NVTE_MAX_OPS,                                                                    \
        arg3 = ar_firstgpu, arg4 = ar_nvrank, arg5 = ar_step,                                   \
        arg7 = elements / 16 / x, arg6 = offset / 16 + arg4 * arg7,                             \
        arg8 = rowelements / 8, arg9 = strideelements / 8;                                      \
    void **arg10 = reinterpret_cast<void **>(comm->gpu_ptrs);                                   \
    int arg11 = handler * comm->nvsize;                                                         \
    void *arg12 = output;                                                                       \
...
#define callranks_rs_oop_stride_atomic_fp8(x)                                                    \
  if (ar_nvsize == x) {                                                                          \
    int arg1 = op - NVTE_MAX_OPS,                                                                \
        ...
        arg3 = ar_firstgpu, arg4 = ar_nvrank, arg5 = ar_step,                                    \
        arg7 = elements / 16 / x, arg6 = offset / 16, arg8 = rowelements / 8,                    \
        arg9 = strideelements / 8, arg10 = numchunks;                                            \
    void **arg11 = reinterpret_cast<void **>(comm->gpu_ptrs);                                    \
    int arg12 = handler * comm->nvsize;                                                          \
    void *arg13 = output;                                                                        \
    void *arg14 = counters;                                                                      \
    ...
    CUDACHECK(cudaLaunchKernelExC(                                                               \
        &cfg,                                                                                    \
        reinterpret_cast<void *>(userbuffers_fp16_sum_inplace_gpu_rr_rs_oop_stride_atomic_fp8<   \
            x, fp8type>),                                                                        \
        kernelArgs));                                                                            \
  }
#endif
...
int reducescatter2_userbuff_inplace_gpu(const int maxcredit, const int handler,
                                        const int offset, const int elements,
                                        const int blocksize, communicator *comm,
                                        cudaStream_t stream, int op) {
  // schedule GPU kernel only
  ...
  if (num_nodes > 1) {
    callranks2_block_rs(1) callranks2_block_rs(2) callranks2_block_rs(4)
        callranks2_block_rs(8)
  } else {
    SETUP_LAUNCH_CONFIG(sms, warps * 32, stream);
...
void reducescatter2_userbuff_strided(void *output, const int handler,
                                     const int offset, const int rowelements,
                                     const int colelements,
                                     const int strideelements,
                                     communicator *comm, cudaStream_t stream) {
  ...
  SETUP_LAUNCH_CONFIG(sms, warps * 32, stream);
  callranks_rs_oop_stride(2) callranks_rs_oop_stride(4)
...
void reducescatter2_userbuff_strided_atomic(
    ...
    const int colelements, const int strideelements, const int numchunks,
...
void reducescatter2_userbuff_strided_universal_fp8(
  ...
  SETUP_LAUNCH_CONFIG(sms, warps * 32, stream);
  callranks_rs_oop_atomic_fp8(2) callranks_rs_oop_atomic_fp8(4)
...
void reducescatter2_userbuff_strided_atomic_fp8(
  ...
  reducescatter2_userbuff_strided_universal_fp8<fp8type>(
      ...
      strideelements_out, strideelements_in, 1, numchunks, counters /*nullptr*/,
      comm, stream);
}
template <typename fp8type>
...
int allgather2_userbuff_inplace_gpu(const int maxcredit, const int handler,
                                    const int offset, const int elements,
                                    const int blocksize, communicator *comm,
                                    cudaStream_t stream, int op) {
  // schedule GPU kernel only
  ...
  if (num_nodes > 1) {
    callranks2_block_ag(1) callranks2_block_ag(2) callranks2_block_ag(4)
        callranks2_block_ag(8)
  } else {
    SETUP_LAUNCH_CONFIG(sms, warps * 32, stream);
...
void allgather2_userbuff_inplace(const int handler, const int offset,
...
void allgather2_userbuff_inplace_sliced(const int handler, const int offset,
                                        const int elements, communicator *comm,
                                        const int slice_id, const int nslices,
                                        cudaStream_t stream) {
  const int op = userbuffers_allreduceop_nonsharp2;
  const int ar_nvrank = op == userbuffers_allreduceop_nonsharp
                            ? comm->ar_nvrank
  ...
  allgather2_userbuff_inplace(handler,
                              offset +
                                  ar_nvrank * peerelements * (nslices - 1) +
                                  slice_id * peerelements,
                              elements, comm, stream);
  comm->use_rr_kernel = saverrkernel;
}

void reducescatter2_userbuff_inplace(const int handler, const int offset,
                                     const int elements, communicator *comm,
                                     cudaStream_t stream) {
  const int op = userbuffers_allreduceop_nonsharp2;
  const int blocksize = elements * 2;
...
void reducescatter2_userbuff_stridedoutput(
    void *output, const int handler, const int offset, const int rowelements,
    const int colelements, const int strideelements, communicator *comm,
    cudaStream_t stream) {
  const int elements = rowelements * colelements;
  const int op = userbuffers_allreduceop_nonsharp2;
  const int blocksize = elements * 2;
...
void reducescatter2_userbuff(void *output, const int handler, const int offset,
                             const int elements, communicator *comm,
                             cudaStream_t stream) {
  reducescatter2_userbuff_stridedoutput(output, handler, offset, elements, 1, 0,
                                        comm, stream);
}
...
void reducescatter2_userbuff_fp8(void *output, float *scale, const int handler,
                                 const int offset, const int elements,
                                 communicator *comm, cudaStream_t stream) {
  reducescatter2_userbuff_stridedoutput_fp8<fp8type>(
      output, scale, handler, offset, elements, 1, 0, comm, stream);
}

template void reducescatter2_userbuff_fp8<__nv_fp8_e5m2>(
    void *output, float *scale, const int handler, const int offset,
    const int elements, communicator *comm, cudaStream_t stream = 0);
template void reducescatter2_userbuff_fp8<__nv_fp8_e4m3>(
    void *output, float *scale, const int handler, const int offset,
    const int elements, communicator *comm, cudaStream_t stream = 0);
#if 0
...
template void reducescatter2_userbuff_strided_multiatomic_fp8<__nv_fp8_e4m3>(
    ...
    communicator *comm, cudaStream_t stream = 0);
__global__ void __launch_bounds__(MAX_THREADS)
    kuserbuffers_pullsendrecv(int myrank, int peer, int *recv_id,
                              int *send_flagptr, int *recv_flagptr,
...
__global__ void kuserbuffers_pullsend(int myrank, int peer, int *send_id,
                                      int *flagptr) {
...
__global__ void __launch_bounds__(MAX_THREADS)
    kuserbuffers_pullrecv(int myrank, int peer, int *recv_id, int *flagptr,
                          int4 *srcptr, int4 *dstptr, const int lines) {
#define UNROLLCOPY 8
  const int start_elem = threadIdx.x + blockDim.x * blockIdx.x;
  const int end_elem = lines;
...
__global__ void __launch_bounds__(MAX_THREADS)
    kuserbuffers_pushsend(int *send_id, int *flagptr, int4 *srcptr,
                          int4 *dstptr, const int lines) {
  if (lines) {
    const int start_elem = threadIdx.x + blockDim.x * blockIdx.x;
    const int end_elem = lines;
    const int aligned_elem = ((end_elem - start_elem) &
                              (~(blockDim.x * gridDim.x * UNROLLCOPY - 1)));
    const int end_aligned = start_elem + aligned_elem;
    if (end_elem > start_elem) {
      for (int line = start_elem; line < end_aligned;
           line += blockDim.x * gridDim.x * UNROLLCOPY) {
...
__global__ void kuserbuffers_pushrecv(int myrank, int peer, int *recv_id,
                                      int *flagptr, int adder) {
...
__global__ void __launch_bounds__(MAX_THREADS)
    kuserbuffers_pushsendrecv(int *send_id, int *send_flagptr, int4 *srcptr,
                              int4 *dstptr, const int lines, int myrank,
                              int peer, int *recv_id, int *recv_flagptr,
                              int adder) {
  if (lines) {
    const int start_elem = threadIdx.x + blockDim.x * blockIdx.x;
    const int end_elem = lines;
...
__global__ void __launch_bounds__(MAX_THREADS)
    kuserbuffers_pushsendrecv_atomic(int *send_id, int *send_flagptr,
                                     int4 *srcptr, int4 *dstptr,
                                     const int lines, int myrank, int peer,
...
__global__ void __launch_bounds__(MAX_THREADS)
    kuserbuffers_pushsendrecv_multiatomic(int *send_id, int *send_flagptr,
                                          int4 *srcptr, int4 *dstptr,
                                          const int lines, int myrank, int peer,
                                          int *recv_id, int *recv_flagptr,
                                          int adder, void *counters,
                                          int nchunks, int send_stride,
                                          int recv_stride, bool shuffle) {
  for (int chunk_i = 0; chunk_i < nchunks - 1; chunk_i++) {
    int send_chunk_id =
        shuffle ? chunk_i : (nchunks + myrank - chunk_i) % nchunks;
    int recv_chunk_id =
        shuffle ? chunk_i + 1 : (nchunks + myrank - chunk_i - 1) % nchunks;
    int send_offset = (send_chunk_id * send_stride) / 16;
    int recv_offset =
        ((shuffle ? recv_chunk_id : send_chunk_id) * recv_stride) / 16;
...
#pragma unroll
    ...
    if (threadIdx.x == 0) {
      ...
      while (0 != (old_val2 = atomicCAS(
                       ((unsigned int *)counters) + nchunks + chunk_i, 0, 0))) {
      }
    }
    __syncthreads();
  }
...
void userbuffers_send(const int srchandler, const size_t srcoffset,
                      const int dsthandler, const size_t dstoffset,
                      const size_t bytes, communicator *comm, const int peer,
                      cudaStream_t stream) {
  int peerlocal = peer % comm->nvsize;
  void *flagptr = (comm->peer_ptr[0][peerlocal]) +
                  ((NVTE_REG0_OFFSET(comm) + NVTE_REG0_RECV +
                    comm->myrank * NVTE_MAX_REGIONS + dsthandler) *
                   sizeof(int));
  ...
  if (!intranode && (comm->launch_mode & NVTE_LAUNCH_GPU)) {
    kuserbuffers_proxysend<<<1, 1, 0, stream>>>(
        &(comm->flags[NVTE_GF_STATE + userbuffers_sendop]),
  ...
  if (!(comm->launch_mode & NVTE_LAUNCH_GPU))
    return;
  if (comm->push == 0) {
    kuserbuffers_pullsend<<<1, 1, 0, stream>>>(
        comm->myrank, peer, &(comm->send_id[peer]),
    ...
    if (comm->use_ce)
      CUDACHECK(cudaMemcpyAsync(dstptr, srcptr, bytes, cudaMemcpyDeviceToDevice,
                                stream));
    SETUP_LAUNCH_CONFIG(signalonly ? 1 : comm->sms, signalonly ? 1 : 1024,
                        stream);
    int *arg1 = &comm->send_id[peer], *arg2 = reinterpret_cast<int *>(flagptr);
    int4 *arg3 = reinterpret_cast<int4 *>(srcptr),
         *arg4 = reinterpret_cast<int4 *>(dstptr);
    ...
    void *kernelArgs[] = {
        ...
        reinterpret_cast<void *>(&arg5)};
    CUDACHECK(cudaLaunchKernelExC(
        &cfg, reinterpret_cast<void *>(kuserbuffers_pushsend), kernelArgs));
  }
}

void userbuffers_sendrecv(const int srchandler, const int dsthandler,
                          const size_t send_offset, const size_t recv_offset,
                          const size_t bytes, communicator *comm,
                          const int send_peer, const int recv_peer,
                          cudaStream_t stream) {
  bool signalonly = (bytes / 16 == 0) || (comm->use_ce != 0);
  int send_peerlocal = send_peer % comm->nvsize;
  int recv_peerlocal = recv_peer % comm->nvsize;
  void *flagptr_send = (comm->peer_ptr[0][send_peerlocal]) +
                       ((NVTE_REG0_OFFSET(comm) + NVTE_REG0_RECV +
                         comm->myrank * NVTE_MAX_REGIONS + dsthandler) *
                        sizeof(int));
  void *flagptr_recv =
      (comm->mem_ptr[0]) + ((NVTE_REG0_OFFSET(comm) + NVTE_REG0_RECV +
                             recv_peer * NVTE_MAX_REGIONS + dsthandler) *
                            sizeof(int));

  void *send_srcptr = (comm->mem_ptr[srchandler]) + send_offset;
  void *send_dstptr =
      (comm->peer_ptr[dsthandler][send_peerlocal]) + send_offset;
  if (comm->use_ce)
    CUDACHECK(cudaMemcpyAsync(send_dstptr, send_srcptr, bytes,
  ...
  void *kernelArgs[] = {
      ...
      reinterpret_cast<void *>(&arg9), reinterpret_cast<void *>(&arg10)};
  CUDACHECK(cudaLaunchKernelExC(
      &cfg, reinterpret_cast<void *>(kuserbuffers_pushsendrecv), kernelArgs));
  //}
}

void userbuffers_sendrecv_atomic(const int srchandler, const int dsthandler,
                                 const size_t send_offset,
                                 const size_t recv_offset, const size_t bytes,
                                 communicator *comm, const int send_peer,
                                 const int recv_peer, void *counters,
                                 cudaStream_t stream) {
  ...
  void *kernelArgs[] = {
      ...
      reinterpret_cast<void *>(&arg11)};
  CUDACHECK(cudaLaunchKernelExC(
      &cfg, reinterpret_cast<void *>(kuserbuffers_pushsendrecv_atomic),
      kernelArgs));
}

void userbuffers_sendrecv_multiatomic(
    const int srchandler, const int dsthandler, const size_t send_stride,
    const size_t recv_stride, const size_t bytes, communicator *comm,
    const int send_peer, const int recv_peer, const int nchunks, void *counters,
  ...
  int4 *arg4 =
      reinterpret_cast<int4 *>((comm->peer_ptr[dsthandler][send_peerlocal]));
  int arg5 = bytes / 16;
  int arg6 = comm->myrank;
  int arg7 = recv_peer;
  int *arg8 = &comm->recv_id[recv_peer * NVTE_MAX_REGIONS + dsthandler];
  ...
  void *kernelArgs[] = {
      ...
      reinterpret_cast<void *>(&arg15)};
  CUDACHECK(cudaLaunchKernelExC(
      &cfg, reinterpret_cast<void *>(kuserbuffers_pushsendrecv_multiatomic),
      kernelArgs));
}

__global__ void __launch_bounds__(MAX_THREADS)
    kuserbuffers_alltoall(void **baseflagptrs, int flagoffset, int4 *basesrcptr,
                          void **dstptrs, size_t dstoffset, const int lines,
                          const int myrank) {
  ...
  if (lines) {
    ...
    const int end_elem = lines;
    const int aligned_elem =
        ((end_elem - start_elem) & (~(blockDim.x * UNROLLCOPY - 1)));
    const int end_aligned = start_elem + aligned_elem;
    if (end_elem > start_elem) {
      for (int line = start_elem; line < end_aligned;
           line += blockDim.x * UNROLLCOPY) {
        int4 val[UNROLLCOPY];
#pragma unroll
        for (int i = 0; i < UNROLLCOPY; i++)
...
void userbuffers_alltoall_send(const int srchandler, const size_t srcoffset,
                               const int dsthandler, const size_t dstoffset,
                               const size_t bytes, communicator *comm,
                               cudaStream_t stream) {
  if (comm->launch_mode & NVTE_LAUNCH_CPU) {
    comm->fifo[comm->head].optype = userbuffers_alltoall;
    comm->fifo[comm->head].basecounter =
        comm->basecounter[userbuffers_alltoall];
    comm->fifo[comm->head].handler = srchandler;
    comm->fifo[comm->head].offset = srcoffset;
    comm->fifo[comm->head].handler2 = dsthandler;
    comm->fifo[comm->head].offset2 = dstoffset;
...
void userbuffers_recv(const int srchandler, const size_t srcoffset,
                      const int dsthandler, const size_t dstoffset,
                      const size_t bytes, communicator *comm, const int peer,
                      cudaStream_t stream) {
  int peerlocal = peer % comm->nvsize;
  void *flagptr =
      (comm->mem_ptr[0]) + ((NVTE_REG0_OFFSET(comm) + NVTE_REG0_RECV +
                             peer * NVTE_MAX_REGIONS + dsthandler) *
                            sizeof(int));
  bool signalonly = (bytes / 16 == 0) || (comm->use_ce != 0);
  bool intranode = INTRANODE(peer);
  if (!(comm->launch_mode & NVTE_LAUNCH_GPU))
  ...
  if (comm->push == 0 && intranode) {
    ...
    kuserbuffers_pullrecv<<<signalonly ? 1 : comm->sms, signalonly ? 1 : 1024,
                            0, stream>>>(
        comm->myrank, peer,
...
void userbuffers_alltoall_recv(communicator *comm, cudaStream_t stream) {
  void *flagptr =
      (comm->mem_ptr[0]) +
      ((NVTE_REG0_OFFSET(comm) + NVTE_REG0_OPFLAGS * userbuffers_alltoall) *
       sizeof(int));

  if (!(comm->launch_mode & NVTE_LAUNCH_GPU))
    return;
  kuserbuffers_pushrecv<<<1, 1, 0, stream>>>(
      comm->myrank, -1, reinterpret_cast<int *>(flagptr + 4),
...
static __global__ void consumer_kernel(void *atomic_ptr, int chunk_i) {
  ...
  if (blockIdx.x == 0 && threadIdx.x == 0) {
    int old_val;
    while (0 !=
           (old_val = atomicCAS((unsigned int *)atomic_ptr + chunk_i, 0, 0))) {
    }
