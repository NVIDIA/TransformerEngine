...
/*! \file nvfp4_1x64.h
 *  \brief Small helpers for NVFP4 per-1x64-K S_enc (env + config + preconditions), shared
 *         between NVFP4Quantizer and split_quantize to avoid duplicating policy.
 */
...
/// Whether rowwise 1x64 local encode is requested (TE-wide env, same for single-tensor and split).
...
inline void require_ok_for_non_split(bool with_rht, bool columnwise, bool sr) {
  ...
  NVTE_CHECK(
      !columnwise,
      "NVTE_NVFP4_ROWWISE_1X64_LOCAL_ENCODE=1 supports rowwise-only NVFP4 output.");
...
inline void require_ok_for_split(bool want_rowwise, bool have_columnwise, bool sr) {
  ...
  NVTE_CHECK(
      want_rowwise,
      "NVTE_NVFP4_ROWWISE_1X64_LOCAL_ENCODE in split_quantize requires rowwise output.");
  NVTE_CHECK(!have_columnwise,
             "NVTE_NVFP4_ROWWISE_1X64_LOCAL_ENCODE in split_quantize does not support columnwise "
             "output.");
  NVTE_CHECK(
