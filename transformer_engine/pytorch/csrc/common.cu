...
void dispatch_layernorm(
  ...
  nvte_layernorm_fwd(input_cu.data(), gamma_cu.data(), beta_cu.data(), epsilon, z_cu.data(),
...
void dispatch_cast_transpose_fusion(
  ...
  auto output_cast_cu = makeTransformerEngineTensor(output_cast, output_cast_shape,
                                                    output_cast_type, amax, scale, scale_inv);
  auto output_transpose_cu = makeTransformerEngineTensor(
      output_transpose, output_transpose_shape, output_transpose_type, amax, scale, scale_inv);
...
void dispatch_gelu(
  ...
  auto input_cu = makeTransformerEngineTensor(input, input_shape, input_type);
  auto output_cu =
      makeTransformerEngineTensor(output, output_shape, output_type, amax, scale, scale_inv);
...
void dispatch_bgrad_cast_transpose_fusion(
  ...
  auto cast_output_cu = makeTransformerEngineTensor(cast_output, cast_output_shape,
                                                    cast_output_type, amax, scale, scale_inv);
  auto transposed_output_cu = makeTransformerEngineTensor(
      transposed_output, transposed_output_shape, transposed_output_type, amax, scale, scale_inv);
...
void dispatch_multi_cast_transpose(
  ...
  std::vector<NVTETensor> input_list, cast_output_list, transposed_output_list;
  ...
  for (size_t i = 0; i < input_dptr_list.size(); ++i) {
    input_list.emplace_back(make_tensor(input_dptr_list[i], input_shape_list[i], input_type_list[i],
                                        nullptr, nullptr, nullptr));
    cast_output_list.emplace_back(make_tensor(cast_output_dptr_list[i], cast_output_shape_list[i],
                                              cast_output_type_list[i], amax_dptr_list[i],
                                              scale_dptr_list[i], scale_inv_dptr_list[i]));
    transposed_output_list.emplace_back(
  ...
  // Launch TE kernel
  nvte_multi_cast_transpose(input_list.size(), input_list.data(), cast_output_list.data(),
                            transposed_output_list.data(), at::cuda::getCurrentCUDAStream());
}
