 ...
 *
 * The Python class object is imported once and cached in a function-
 * local ``static`` (initialization is thread-safe under C++11), so the
 * runtime cost per call is one ``IntEnum.__call__`` (a dict lookup in
 * ``_value2member_map_``).
 */
