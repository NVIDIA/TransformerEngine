...
template <int n> struct TensorListMetadata {
  ...
  // only for fp8
  void *fp8_meta_addresses[3][depth_to_max_tensors[n - 1]]; // scale, amax,
                                                            // scale_inv
};
...
void multi_tensor_apply(
  ...
  bool use_fp8 = false;
  if (tensor_lists.size() == 8 and depth == 5) {
    // assume the order of the tensor lists is [g, p, m, v, fp8_raw_data, scale,
    // amax, scale_inv]
    use_fp8 = true;
  } else {
  ...
  if (use_fp8) {
    TORCH_CHECK(tensor_lists[depth].size() == len0 &&
                    tensor_lists[depth + 1].size() == len0 &&
                    tensor_lists[depth + 2].size() == len0,
                "Size mismatch among tensor lists");
  }
  ...
  for (int t = 0; t < ntensors; t++) {
    ...
    if (use_fp8) {
      for (int i = 0; i < 3; i++)
        tl.fp8_meta_addresses[i][loc_tensor_info] =
            tensor_lists[depth + i][t].data_ptr();
    }
    loc_tensor_info++;
    ...
    for (auto chunk = 0; chunk < chunks_this_tensor; chunk++) {
      ...
      if (tensors_full || blocks_full || last_chunk) {
        ...
        if (chunk == chunks_this_tensor - 1) {
          ...
          if (use_fp8) {
            for (int i = 0; i < 3; i++) {
              tl.fp8_meta_addresses[i][0] =
                  tl.fp8_meta_addresses[i][loc_tensor_info - 1];
            }
          }
          loc_tensor_info = 1;
