...
struct InterCTASync {
  ...
  inline __device__ InterCTASync(Params &params, uint32_t bidm, uint32_t bidn)  // NOLINT(*)
      : phase_counter_(0),
        b0_(params.barrier + bidm)  // The barrier for this group of CTAs. Assumed 0 init.
        ,
        b1_(params.barrier + bidm +
            params.ctas_per_col) {}  // The barrier for this group of CTAs. Assumed 0 init.
...
struct Reducer : public Reducer<T, 1, WARPS_M, WARPS_N> {
  ...
  template <typename Params>
  inline __device__ Reducer(Params &params, uint32_t bidm, uint32_t bidn,  // NOLINT(*)
...
template <typename T>
inline __device__ void warp_chan_upd_dynamic(T &m_a, T &m2_a, T &n_a,  // NOLINT(*)
                                             int num_active) {
...
struct Stats {
  ...
  template <typename Params>
  inline __device__ Stats(Params &params, uint32_t bidm, uint32_t bidn,  // NOLINT(*)
                          uint32_t warp_m, uint32_t warp_n, uint32_t lane,
                          void *smem)
      : inter_cta_(params, bidm, bidn),
