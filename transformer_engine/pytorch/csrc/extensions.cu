...
#define REGISTER_POINTWISE_OP(NAME, OUT_N)                                                         \
    at::Tensor NAME(at::Tensor input,                                                              \
                    at::Tensor scale,                                                              \
                    at::Tensor amax,                                                               \
                    at::Tensor scale_inv,                                                          \
                    transformer_engine::DType otype) {                                             \
        ...
        auto output_cu = makeTransformerEngineTensor(output.data_ptr(), {M, OUT_N}, otype,         \
                                                     amax.data_ptr(), scale.data_ptr(),            \
                                                     scale_inv.data_ptr());                        \
                                                                                                   \
        nvte_##NAME(input_cu.data(), output_cu.data(), at::cuda::getCurrentCUDAStream());          \
                                                                                                   \
        return output;                                                                             \
    }                                                                                              \
                                                                                                   \
    at::Tensor d##NAME(at::Tensor grad,                                                            \
                       at::Tensor input,                                                           \
                       transformer_engine::DType otype) {                                          \
        using namespace transformer_engine;                                                        \
                                                                                                   \
        ...
        auto input_cu =  makeTransformerEngineTensor(input.data_ptr(), {M, N}, itype);             \
        auto grad_cu =   makeTransformerEngineTensor(grad.data_ptr(), {M, OUT_N}, gtype);          \
        auto output_cu = makeTransformerEngineTensor(output.data_ptr(), {M, N}, otype);            \
                                                                                                   \
        nvte_d##NAME(grad_cu.data(), input_cu.data(), output_cu.data(),                            \
                     at::cuda::getCurrentCUDAStream());                                            \
                                                                                                   \
        return output;                                                                             \
    }
...
REGISTER_POINTWISE_OP(gelu, N)
REGISTER_POINTWISE_OP(relu, N)
REGISTER_POINTWISE_OP(geglu, N / 2)
REGISTER_POINTWISE_OP(reglu, N / 2)
REGISTER_POINTWISE_OP(swiglu, N / 2)
...
at::Tensor cast_to_fp8(const at::Tensor &input,
    ...
    cast_to_fp8_noalloc(input, scale, output, amax, scale_inv, otype);
