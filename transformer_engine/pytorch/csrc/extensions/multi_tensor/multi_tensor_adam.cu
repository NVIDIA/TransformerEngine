...
template <typename T, typename U, typename FULL_T, typename index_t>
struct AdamFunctor {
  __device__ __forceinline__ void operator()(index_t chunk_size, volatile int *noop_gmem,
    ...
    U *p = reinterpret_cast<U *>(tl.addresses[1][tensor_loc]);
...
template <typename T, typename U, typename FULL_T>
...
void multi_tensor_adam_cuda(int chunk_size, at::Tensor noop_flag,
  ...
  if (requires_64bit_indexing) {
    ...
    DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
        tensor_lists[0][0].scalar_type(), 0, "adam_g",
        DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
            tensor_lists[1][0].scalar_type(), 1, "adam_p",
    ...
    DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
        ...
        DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
            ...
            multi_tensor_apply<5>((int64_t)BLOCK_SIZE, (int64_t)chunk_size, noop_flag, tensor_lists,
                                  AdamMasterFunctor<scalar_t_0, scalar_t_1, float, int64_t>(),
                                  beta1, beta2, bias_correction1, bias_correction2, epsilon, lr,
                                  (adamMode_t)mode, weight_decay);))
  } else {
    // Assume single type across p,g,m1,m2 now
    DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
        ...
        DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
            ...
            multi_tensor_apply<5>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                                  AdamMasterFunctor<scalar_t_0, scalar_t_1, float, int32_t>(),
                                  beta1, beta2, bias_correction1, bias_correction2, epsilon, lr,
                                  (adamMode_t)mode, weight_decay);))
  }
  AT_CUDA_CHECK(cudaGetLastError());
...
void multi_tensor_adam_capturable_cuda(int chunk_size, at::Tensor noop_flag,
  ...
  DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
      ...
      DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
          tensor_lists[1][0].scalar_type(), 1, "adam_p",
          multi_tensor_apply<4>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                                AdamCapturableFunctor<scalar_t_0, scalar_t_1, float>(),
                                beta1, beta2, step.data_ptr<int>(), bias_correction, epsilon,
...
void multi_tensor_adam_capturable_master_cuda(int chunk_size, at::Tensor noop_flag,
  ...
  DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
      ...
      DISPATCH_DOUBLE_FLOAT_HALF_AND_BFLOAT(
          tensor_lists[1][0].scalar_type(), 1, "adam_p",
          multi_tensor_apply<5>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists,
                                AdamCapturableMasterFunctor<scalar_t_0, scalar_t_1, float>(),
                                beta1, beta2, step.data_ptr<int>(), bias_correction, epsilon,
                                lr.data_ptr<float>(), (adamMode_t)mode, weight_decay,
                                inv_scale.data_ptr<float>());))
