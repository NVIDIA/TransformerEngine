...
void multi_tensor_compute_scale_cuda(int chunk_size, at::Tensor noop_flag,
  ...
  multi_tensor_apply<2>(BLOCK_SIZE, chunk_size, noop_flag, tensor_lists, ComputeScaleFunctor(),
                        max_fp8, force_pow_2_scales, epsilon);
  AT_CUDA_CHECK(cudaGetLastError());
}

struct ComputeScaleInvFunctor {
  __device__ __forceinline__ void operator()(int chunk_size, volatile int *noop_gmem,
                                             TensorListMetadata<2> &tl,  // NOLINT(*)
                                             float max_fp8, bool force_pow_2_scales,
                                             float epsilon) {
    ...
    float *scale_inv = reinterpret_cast<float *>(tl.addresses[1][tensor_loc]);
