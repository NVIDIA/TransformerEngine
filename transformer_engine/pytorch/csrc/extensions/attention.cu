...
void copy_to_kv_cache_non_paged_launcher(
  ...
  copy_to_kv_cache_non_paged_kernel<<<16, 256, 0, at::cuda::getCurrentCUDAStream()>>>(
      ...
      qkv_format, h_kv, d_k, d_v, b, max_ctx_len, max_seq_len, max_ctx_tokens, max_tokens);
  }
  if (new_v.data_ptr() != nullptr && d_k == d_v) {
  copy_to_kv_cache_non_paged_kernel_same_d<<<16, 256, 0, at::cuda::getCurrentCUDAStream()>>>(
      ...
      step_lens.data_ptr<int>(),
      seq_lens.data_ptr<int>(),
      qkv_format, h_kv, d_k, b, max_ctx_len, max_seq_len, max_ctx_tokens, max_tokens);
  }
...
void copy_to_kv_cache_non_paged(
  ...
  if (k_cache.scalar_type() == at::ScalarType::Half) {
    using dtype = at::Half;
    copy_to_kv_cache_non_paged_launcher<dtype>(new_k, new_v, k_cache, v_cache, batch_indices, step_lens, seq_lens, qkv_format, h_kv, d_k, d_v, b, max_ctx_len, max_seq_len, max_ctx_tokens, max_tokens);
