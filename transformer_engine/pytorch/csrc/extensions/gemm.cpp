...
TensorWrapper buildInputTensorWrapper(const Tensor& rowwise_data, DType te_dtype,
  ...
  // Flatten >2D shapes to 2D for GEMM.
  // Only for MXFP8/DELAYED scaling where TensorWrapper::shape() returns
  // data shape as-is (no transpose).
  // Rowwise data is [M_dims..., K] → [M, K] (keep last dim).
  // Columnwise data is [K, M_dims...] → [K, M] (keep first dim).
  ...
  auto flatten2D_columnwise = [](std::vector<size_t>& shape, NVTEScalingMode sm) {
    if (shape.size() <= 2) return;
    if (sm != NVTE_MXFP8_1D_SCALING && sm != NVTE_DELAYED_TENSOR_SCALING) return;
    size_t K = shape.front();
    size_t M = 1;
    for (size_t i = 1; i < shape.size(); ++i) M *= shape[i];
    shape = {K, M};
  };
  ...
  if (amax.has_value() && amax->numel() > 0) {
    ...
    // Columnwise-only NVFP4: set rowwise amax from columnwise amax.
    // nvte_nvfp4_compute_per_tensor_scale reads columnwise_amax directly
    // for NT layout, but other paths may check amax.dptr.
  ...
  if (colwise_data.has_value() && colwise_data->numel() > 0) {
    ...
    // Do NOT flatten columnwise data. For DELAYED/Block scaling the columnwise
    // buffer is the physical transpose: shape [K, M_dims...].
    // TensorWrapper::shape() moves the first dim to the end, giving the logical
    // [M_dims..., K] shape from which flat_first_dim()=M and flat_last_dim()=K
    // are computed correctly, even for 3D+ tensors.
    // Applying flatten2D here would incorrectly treat the last dim as K.
