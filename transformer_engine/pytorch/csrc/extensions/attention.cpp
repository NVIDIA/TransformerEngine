...
Tensor fa_prepare_fwd(Tensor qkvi) {
  ...
  // qkvi is a non-contiguous 4D view (s, b, n, d) of interleaved (s, b, n, 3, d) storage.
  // size(3) = d (head_dim), NOT d*3.  The factor of 3 is in the strides, not the sizes.
  // Output: (3, b, s, n, d).
  auto s = qkvi.size(0), b = qkvi.size(1), n = qkvi.size(2), d = qkvi.size(3);
  auto qkv = allocateStableTensor({3, b, s, n, d}, dtype, qkvi.get_device_index());

  auto te_dtype = GetTransformerEngineDType(dtype);
  // Pass the tensor shape as [s, b, n, d] -- the kernel knows the data is 3-way interleaved
  // and uses the factor of 3 internally via blockIdx.y and stride arithmetic.
  auto te_qkvi = makeTransformerEngineTensor(qkvi.data_ptr(),
                                             {static_cast<size_t>(s), static_cast<size_t>(b),
...
Tensor thd_read_half_tensor(Tensor tensor, Tensor cu_seqlens, int64_t half_idx) {
  ...
  auto half = allocateStableTensor(shape, tensor.scalar_type(), tensor.get_device_index());
...
void thd_second_half_lse_correction(Tensor lse, Tensor lse_per_step, Tensor cu_seqlens,
                                    bool lse_packed) {
  auto te_lse = makeTransformerEngineTensor(lse);
  auto te_lse_ps = makeTransformerEngineTensor(lse_per_step);
  auto te_cu = makeTransformerEngineTensor(cu_seqlens);
  nvte_cp_thd_second_half_lse_correction(te_lse.data(), te_lse_ps.data(), te_cu.data(), lse_packed,
                                         getCurrentCUDAStreamRaw(lse.get_device_index()));
}

Tensor thd_read_second_half_lse(Tensor lse, Tensor cu_seqlens, bool lse_packed,
                                int64_t second_half_lse_seqlen) {
  std::vector<int64_t> shape;
  if (lse_packed) {
  ...
  auto half_lse = allocateStableTensorZeros(shape, ScalarType::Float, lse.get_device_index());

  auto te_lse = makeTransformerEngineTensor(lse);
  auto te_cu = makeTransformerEngineTensor(cu_seqlens);
  auto te_half = makeTransformerEngineTensor(half_lse);
  nvte_cp_thd_read_second_half_lse(te_lse.data(), te_cu.data(), te_half.data(), lse_packed,
                                   static_cast<int>(second_half_lse_seqlen),
                                   getCurrentCUDAStreamRaw(lse.get_device_index()));
  return half_lse;
}

void thd_out_correction(Tensor out, Tensor out_per_step, Tensor lse, Tensor lse_per_step,
                        Tensor cu_seqlens, bool only_second_half, bool lse_packed) {
  ...
  auto te_cu = makeTransformerEngineTensor(cu_seqlens);
  nvte_cp_thd_out_correction(te_out.data(), te_ops.data(), te_lse.data(), te_lps.data(),
                             te_cu.data(), only_second_half, lse_packed,
                             getCurrentCUDAStreamRaw(out.get_device_index()));
}

void thd_grad_correction(Tensor grad, Tensor grad_per_step, Tensor cu_seqlens,
  ...
  auto te_cu = makeTransformerEngineTensor(cu_seqlens);
  nvte_cp_thd_grad_correction(te_grad.data(), te_gps.data(), te_cu.data(), first_half.data(),
                              second_half.data(), getCurrentCUDAStreamRaw(grad.get_device_index()));
}

Tensor thd_get_partitioned_indices(Tensor cu_seqlens, int64_t total_tokens, int64_t world_size,
                                   int64_t rank) {
  auto output = allocateStableTensor({total_tokens / world_size}, ScalarType::Int,
                                     cu_seqlens.get_device_index());
  auto te_cu = makeTransformerEngineTensor(cu_seqlens);
  auto te_out = makeTransformerEngineTensor(output);
  nvte_cp_thd_get_partitioned_indices(te_cu.data(), te_out.data(), static_cast<int>(total_tokens),
...
Tensor convert_thd_to_bshd(Tensor tensor, Tensor cu_seqlens, int64_t b, int64_t max_seq_len) {
  auto h = tensor.size(1), d = tensor.size(2);
  auto new_tensor = allocateStableTensorZeros({b, max_seq_len, h, d}, tensor.scalar_type(),
                                              tensor.get_device_index());

  auto te_t = makeTransformerEngineTensor(tensor);
  auto te_cu = makeTransformerEngineTensor(cu_seqlens);
  auto te_new = makeTransformerEngineTensor(new_tensor);
  nvte_convert_thd_to_bshd(te_t.data(), te_cu.data(), te_new.data(), static_cast<int>(b),
...
Tensor convert_bshd_to_thd(Tensor tensor, Tensor cu_seqlens, int64_t t) {
  ...
  auto new_tensor =
      allocateStableTensorZeros({t, h, d}, tensor.scalar_type(), tensor.get_device_index());

  auto te_t = makeTransformerEngineTensor(tensor);
  auto te_cu = makeTransformerEngineTensor(cu_seqlens);
  auto te_new = makeTransformerEngineTensor(new_tensor);
  nvte_convert_bshd_to_thd(te_t.data(), te_cu.data(), te_new.data(), static_cast<int>(t),
                           getCurrentCUDAStreamRaw(tensor.get_device_index()));
  return new_tensor;
}
...
void copy_to_kv_cache(Tensor new_k, Tensor new_v, Tensor k_cache, Tensor v_cache, Tensor page_table,
                      Tensor cu_new_lens, Tensor cu_cached_lens, int64_t qkv_format, int64_t b,
                      int64_t max_ctx_len, int64_t max_seq_len, int64_t max_pages_per_seq,
                      bool is_non_paged) {
  ...
  nvte_copy_to_kv_cache(te_nk.data(), te_nv.data(), te_kc.data(), te_vc.data(), te_pt.data(),
                        te_cnl.data(), te_ccl.data(), static_cast<NVTE_QKV_Format>(qkv_format),
                        static_cast<int>(b), static_cast<int>(max_ctx_len),
                        static_cast<int>(max_seq_len), static_cast<int>(max_pages_per_seq),
                        is_non_paged, getCurrentCUDAStreamRaw(new_k.get_device_index()));
}
...
std::tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, int64_t>
fused_attn_fwd_noalloc(
    int64_t max_seqlen_q, int64_t max_seqlen_kv, bool is_training, double attn_scale,
    double p_dropout, bool set_zero, int64_t qkv_layout, int64_t bias_type, int64_t attn_mask_type,
    int64_t softmax_type, std::vector<int64_t> window_size, bool bottom_right_diagonal,
    Tensor cu_seqlens_q, Tensor cu_seqlens_kv,
    // Q/K/V with optional quantization
    Tensor Q_data, int64_t Q_dtype, std::optional<Tensor> Q_scale_inv, int64_t Q_scaling_mode,
    Tensor K_data, int64_t K_dtype, std::optional<Tensor> K_scale_inv, int64_t K_scaling_mode,
    Tensor V_data, int64_t V_dtype, std::optional<Tensor> V_scale_inv, int64_t V_scaling_mode,
    // S (softmax) — usually empty placeholder
    Tensor S_data, int64_t S_dtype, std::optional<Tensor> S_amax, std::optional<Tensor> S_scale,
    std::optional<Tensor> S_scale_inv, int64_t S_scaling_mode,
    // O (output) — pre-allocated
    Tensor O_data, int64_t O_dtype, std::optional<Tensor> O_amax, std::optional<Tensor> O_scale,
    std::optional<Tensor> O_scale_inv, int64_t O_scaling_mode,
    // Optional tensors
    std::optional<Tensor> cu_seqlens_q_padded, std::optional<Tensor> cu_seqlens_kv_padded,
    ...
    Tensor rng_state, bool return_max_logit, bool cuda_graph) {
  ...
  auto te_Q = makeQuantizedTensorWrapper(Q_data, static_cast<DType>(Q_dtype), Q_shape, std::nullopt,
                                         std::nullopt, Q_scale_inv,
                                         static_cast<NVTEScalingMode>(Q_scaling_mode));
  auto te_K = makeQuantizedTensorWrapper(K_data, static_cast<DType>(K_dtype), K_shape, std::nullopt,
                                         std::nullopt, K_scale_inv,
                                         static_cast<NVTEScalingMode>(K_scaling_mode));
  auto te_V = makeQuantizedTensorWrapper(V_data, static_cast<DType>(V_dtype), V_shape, std::nullopt,
                                         std::nullopt, V_scale_inv,
  ...
  auto te_O =
      makeQuantizedTensorWrapper(O_data, static_cast<DType>(O_dtype), O_shape, O_amax, O_scale,
                                 O_scale_inv, static_cast<NVTEScalingMode>(O_scaling_mode));

  // Build S TensorWrapper (placeholder — NVTE determines actual shape)
  auto S_shape_vec = getStableTensorShape(S_data);
  auto te_S =
      makeQuantizedTensorWrapper(S_data, static_cast<DType>(S_dtype), S_shape_vec, S_amax, S_scale,
                                 S_scale_inv, static_cast<NVTEScalingMode>(S_scaling_mode));
  ...
  // Phase 1: shape query
  nvte_fused_attn_fwd(te_Q.data(), te_K.data(), te_V.data(), te_Bias.data(),
                      te_SoftmaxOffset.data(), te_S.data(), te_O.data(), &nvte_aux, te_cu_q.data(),
                      te_cu_kv.data(), te_cu_q_pad.data(), te_cu_kv_pad.data(), te_pt_k.data(),
                      te_pt_v.data(), te_rng.data(), static_cast<size_t>(max_seqlen_q),
                      static_cast<size_t>(max_seqlen_kv), is_training, return_max_logit, cuda_graph,
                      static_cast<float>(attn_scale), static_cast<float>(p_dropout), nvte_layout,
                      nvte_bias, nvte_mask, nvte_softmax, window_size[0], window_size[1],
                      bottom_right_diagonal, workspace.data(), stream);
  ...
  if (ws_shape.ndim > 0 && workspace.numel() > 0) {
    ws_data =
        allocateStableTensor(std::vector<int64_t>(ws_shape.data, ws_shape.data + ws_shape.ndim),
                             workspace.dtype(), device_idx);
    workspace = makeTransformerEngineTensor(
        ws_data.data_ptr(), std::vector<size_t>(ws_shape.data, ws_shape.data + ws_shape.ndim),
        workspace.dtype());
  }
  ...
  //
  // Similarly, the Bias slot must use the caller-supplied Bias tensor so that the
  // backward pass can read the original bias shape for dBias allocation. The old pybind
  // code saves the original Bias tensor at extensions/attention.cpp:283.
  // The SoftmaxOffset slot is handled the same way.
  bool rng_state_placed = false;
  bool bias_placed = false;
  bool softmax_offset_placed = false;
  ...
  for (size_t i = 0; i < nvte_aux.size; ++i) {
    ...
    bool is_rng_slot = (!rng_state_placed && aux_dtype == DType::kInt64 && aux_shape.ndim == 1 &&
                        aux_shape.data[0] == 2);
    // Detect the Bias slot: 4D tensor that comes AFTER the rng_state slot.
    // bias_type must be not NO_BIAS(0) and not ALIBI(3), and the caller must have
    // provided a Bias tensor. We require rng_state to be placed first because
    // the aux order is: S, [Max], rng_state, [Bias], [SoftmaxOffset].
    // Note: NVTE reports the Bias aux slot with QKV dtype (BF16/FP16), not kFloat32,
    // so we do NOT filter on aux_dtype here.
    bool is_bias_slot =
        (rng_state_placed && !bias_placed && Bias.has_value() && nvte_bias != NVTE_NO_BIAS &&
         nvte_bias != NVTE_ALIBI && aux_shape.ndim == 4);
    // Detect the SoftmaxOffset slot: 4D float32 [1, h, 1, 1] tensor after rng_state.
    bool is_softmax_offset_slot =
        (rng_state_placed && !softmax_offset_placed && SoftmaxOffset.has_value() &&
         nvte_softmax != NVTE_VANILLA_SOFTMAX && aux_dtype == DType::kFloat32 &&
         aux_shape.ndim == 4 && aux_shape.data[0] == 1 && aux_shape.data[2] == 1 &&
         aux_shape.data[3] == 1);

    if (is_rng_slot) {
      aux_tensor = rng_state;
      rng_state_placed = true;
    } else if (is_bias_slot) {
      aux_tensor = Bias.value();
      bias_placed = true;
    } else if (is_softmax_offset_slot) {
      aux_tensor = SoftmaxOffset.value();
      softmax_offset_placed = true;
    } else {
      aux_tensor = allocateStableTensor(shape_vec, aux_dtype, device_idx);
    }
    aux_tensors.push_back(aux_tensor);

    // Use the aux_tensor's actual shape for the tensor param (important for Bias
    // which may have batch dim > 1 while NVTE reports [1, h, s, s]).
    auto actual_shape = getStableTensorShape(aux_tensor);
    NVTEBasicTensor temp = {aux_tensor.data_ptr(), nvte_tensor_type(nvte_aux.tensors[i]),
                            nvte_make_shape(actual_shape.data(), actual_shape.size())};
  ...
  return std::make_tuple(aux_tensors[0], aux_tensors[1], aux_tensors[2], aux_tensors[3],
...
std::tuple<Tensor, Tensor> fused_attn_bwd_noalloc(
    int64_t max_seqlen_q, int64_t max_seqlen_kv, double attn_scale, double p_dropout, bool set_zero,
    int64_t qkv_layout, int64_t bias_type, int64_t attn_mask_type, int64_t softmax_type,
    std::vector<int64_t> window_size, bool bottom_right_diagonal, bool deterministic,
    Tensor cu_seqlens_q, Tensor cu_seqlens_kv,
    // Q/K/V/O/dO with optional quantization
    Tensor Q_data, int64_t Q_dtype, std::optional<Tensor> Q_scale_inv, int64_t Q_scaling_mode,
    Tensor K_data, int64_t K_dtype, std::optional<Tensor> K_scale_inv, int64_t K_scaling_mode,
    Tensor V_data, int64_t V_dtype, std::optional<Tensor> V_scale_inv, int64_t V_scaling_mode,
    Tensor O_data, int64_t O_dtype, std::optional<Tensor> O_scale_inv, int64_t O_scaling_mode,
    Tensor dO_data, int64_t dO_dtype, std::optional<Tensor> dO_scale_inv, int64_t dO_scaling_mode,
    // S and dP (softmax tensors)
    Tensor S_data, int64_t S_dtype, std::optional<Tensor> S_amax, std::optional<Tensor> S_scale,
    std::optional<Tensor> S_scale_inv, int64_t S_scaling_mode, Tensor dP_data, int64_t dP_dtype,
    std::optional<Tensor> dP_amax, std::optional<Tensor> dP_scale,
    std::optional<Tensor> dP_scale_inv, int64_t dP_scaling_mode,
    // dQ/dK/dV pre-allocated by Python
    Tensor dQ_data, int64_t dQ_dtype, std::optional<Tensor> dQ_amax, std::optional<Tensor> dQ_scale,
    std::optional<Tensor> dQ_scale_inv, int64_t dQ_scaling_mode, Tensor dK_data, int64_t dK_dtype,
    std::optional<Tensor> dK_amax, std::optional<Tensor> dK_scale,
    std::optional<Tensor> dK_scale_inv, int64_t dK_scaling_mode, Tensor dV_data, int64_t dV_dtype,
    ...
    int64_t num_aux_tensors, std::optional<Tensor> aux0, std::optional<Tensor> aux1,
    std::optional<Tensor> aux2, std::optional<Tensor> aux3, std::optional<Tensor> aux4,
    std::optional<Tensor> aux5, std::optional<Tensor> aux6, std::optional<Tensor> aux7,
  ...
  auto te_Q = makeQuantizedTensorWrapper(Q_data, static_cast<DType>(Q_dtype),
                                         getStableTensorShape(Q_data), std::nullopt, std::nullopt,
                                         Q_scale_inv, static_cast<NVTEScalingMode>(Q_scaling_mode));
  auto te_K = makeQuantizedTensorWrapper(K_data, static_cast<DType>(K_dtype),
                                         getStableTensorShape(K_data), std::nullopt, std::nullopt,
                                         K_scale_inv, static_cast<NVTEScalingMode>(K_scaling_mode));
  auto te_V = makeQuantizedTensorWrapper(V_data, static_cast<DType>(V_dtype),
                                         getStableTensorShape(V_data), std::nullopt, std::nullopt,
                                         V_scale_inv, static_cast<NVTEScalingMode>(V_scaling_mode));
  auto te_O = makeQuantizedTensorWrapper(O_data, static_cast<DType>(O_dtype),
                                         getStableTensorShape(O_data), std::nullopt, std::nullopt,
                                         O_scale_inv, static_cast<NVTEScalingMode>(O_scaling_mode));
  auto te_dO = makeQuantizedTensorWrapper(
      dO_data, static_cast<DType>(dO_dtype), getStableTensorShape(dO_data), std::nullopt,
      std::nullopt, dO_scale_inv, static_cast<NVTEScalingMode>(dO_scaling_mode));

  auto te_S = makeQuantizedTensorWrapper(S_data, static_cast<DType>(S_dtype),
                                         getStableTensorShape(S_data), S_amax, S_scale, S_scale_inv,
                                         static_cast<NVTEScalingMode>(S_scaling_mode));
  auto te_dP = makeQuantizedTensorWrapper(
      dP_data, static_cast<DType>(dP_dtype), getStableTensorShape(dP_data), dP_amax, dP_scale,
      dP_scale_inv, static_cast<NVTEScalingMode>(dP_scaling_mode));

  auto te_dQ = makeQuantizedTensorWrapper(
      dQ_data, static_cast<DType>(dQ_dtype), getStableTensorShape(dQ_data), dQ_amax, dQ_scale,
      dQ_scale_inv, static_cast<NVTEScalingMode>(dQ_scaling_mode));
  auto te_dK = makeQuantizedTensorWrapper(
      dK_data, static_cast<DType>(dK_dtype), getStableTensorShape(dK_data), dK_amax, dK_scale,
      dK_scale_inv, static_cast<NVTEScalingMode>(dK_scaling_mode));
  auto te_dV = makeQuantizedTensorWrapper(
      dV_data, static_cast<DType>(dV_dtype), getStableTensorShape(dV_data), dV_amax, dV_scale,
      dV_scale_inv, static_cast<NVTEScalingMode>(dV_scaling_mode));

  TensorWrapper te_dBias, te_dSoftmaxOffset;
  if (dBias.has_value()) te_dBias = makeTransformerEngineTensor(dBias.value());
  if (dSoftmaxOffset.has_value())
    te_dSoftmaxOffset = makeTransformerEngineTensor(dSoftmaxOffset.value());
  ...
  std::optional<Tensor> aux_slots[] = {aux0, aux1, aux2, aux3, aux4, aux5, aux6, aux7, aux8, aux9};
  NVTETensorPack nvte_aux;
  nvte_tensor_pack_create(&nvte_aux);
  nvte_aux.size = static_cast<size_t>(num_aux_tensors);
  for (size_t i = 0; i < nvte_aux.size; ++i) {
    NVTE_CHECK(aux_slots[i].has_value(), "aux tensor ", i,
               " is None but num_aux_tensors=", num_aux_tensors);
    auto& t = aux_slots[i].value();
    auto shape_vec = getStableTensorShape(t);
    auto dtype = GetTransformerEngineDType(t.scalar_type());
    NVTEBasicTensor temp = {t.data_ptr(), static_cast<NVTEDType>(dtype),
  ...
  nvte_fused_attn_bwd(te_Q.data(), te_K.data(), te_V.data(), te_O.data(), te_dO.data(), te_S.data(),
                      te_dP.data(), &nvte_aux, te_dQ.data(), te_dK.data(), te_dV.data(),
                      te_dBias.data(), te_dSoftmaxOffset.data(), te_cu_q.data(), te_cu_kv.data(),
                      te_cu_q_pad.data(), te_cu_kv_pad.data(), static_cast<size_t>(max_seqlen_q),
                      static_cast<size_t>(max_seqlen_kv), static_cast<float>(attn_scale),
                      static_cast<float>(p_dropout), nvte_layout, nvte_bias, nvte_mask,
                      nvte_softmax, window_size[0], window_size[1], bottom_right_diagonal,
...
std::tuple<Tensor, Tensor> fused_attn_bwd_packed(
    ...
    bool cuda_graph,
    // Sequence lengths (4)
    Tensor cu_seqlens_q, Tensor cu_seqlens_kv, std::optional<Tensor> cu_seqlens_q_padded,
    std::optional<Tensor> cu_seqlens_kv_padded,
    // Input tensors: data + scale_inv (10)
    Tensor Q_data, std::optional<Tensor> Q_scale_inv, Tensor K_data,
    std::optional<Tensor> K_scale_inv, Tensor V_data, std::optional<Tensor> V_scale_inv,
    Tensor O_data, std::optional<Tensor> O_scale_inv, Tensor dO_data,
    std::optional<Tensor> dO_scale_inv,
    // Softmax buffers (8)
    Tensor S_data, std::optional<Tensor> S_amax, std::optional<Tensor> S_scale,
    std::optional<Tensor> S_scale_inv, Tensor dP_data, std::optional<Tensor> dP_amax,
    std::optional<Tensor> dP_scale, std::optional<Tensor> dP_scale_inv,
    // Output grad tensors (12)
    Tensor dQ_data, std::optional<Tensor> dQ_amax, std::optional<Tensor> dQ_scale,
    std::optional<Tensor> dQ_scale_inv, Tensor dK_data, std::optional<Tensor> dK_amax,
    std::optional<Tensor> dK_scale, std::optional<Tensor> dK_scale_inv, Tensor dV_data,
    std::optional<Tensor> dV_amax, std::optional<Tensor> dV_scale,
    std::optional<Tensor> dV_scale_inv,
  ...
  return fused_attn_bwd_noalloc(
      max_seqlen_q, max_seqlen_kv, attn_scale, p_dropout, set_zero, qkv_layout, bias_type,
      attn_mask_type, softmax_type, window_size, bottom_right_diagonal, deterministic, cu_seqlens_q,
      cu_seqlens_kv, Q_data, Q_dtype, Q_scale_inv, Q_sm, K_data, K_dtype, K_scale_inv, K_sm, V_data,
      V_dtype, V_scale_inv, V_sm, O_data, O_dtype, O_scale_inv, O_sm, dO_data, dO_dtype,
      dO_scale_inv, dO_sm, S_data, S_dtype, S_amax, S_scale, S_scale_inv, S_sm, dP_data, dP_dtype,
      dP_amax, dP_scale, dP_scale_inv, dP_sm, dQ_data, dQ_dtype, dQ_amax, dQ_scale, dQ_scale_inv,
      dQ_sm, dK_data, dK_dtype, dK_amax, dK_scale, dK_scale_inv, dK_sm, dV_data, dV_dtype, dV_amax,
      dV_scale, dV_scale_inv, dV_sm, dBias, dSoftmaxOffset, num_aux_tensors, aux0, aux1, aux2, aux3,
      aux4, aux5, aux6, aux7, aux8, aux9, cu_seqlens_q_padded, cu_seqlens_kv_padded, cuda_graph);
}
...
STABLE_TORCH_LIBRARY_FRAGMENT(transformer_engine_stable, m) {
  ...
  m.def(
      "fused_attn_fwd_noalloc(int max_seqlen_q, int max_seqlen_kv, bool is_training, float "
      "attn_scale, float p_dropout, bool set_zero, int qkv_layout, int bias_type, int "
      "attn_mask_type, int softmax_type, int[] window_size, bool bottom_right_diagonal, Tensor "
      "cu_seqlens_q, Tensor cu_seqlens_kv, Tensor Q_data, int Q_dtype, Tensor? Q_scale_inv, int "
      "Q_scaling_mode, Tensor K_data, int K_dtype, Tensor? K_scale_inv, int K_scaling_mode, Tensor "
      "V_data, int V_dtype, Tensor? V_scale_inv, int V_scaling_mode, Tensor S_data, int S_dtype, "
      "Tensor? S_amax, Tensor? S_scale, Tensor? S_scale_inv, int S_scaling_mode, Tensor O_data, "
      "int O_dtype, Tensor? O_amax, Tensor? O_scale, Tensor? O_scale_inv, int O_scaling_mode, "
      "Tensor? cu_seqlens_q_padded, Tensor? cu_seqlens_kv_padded, Tensor? page_table_k, Tensor? "
      "page_table_v, Tensor? Bias, Tensor? SoftmaxOffset, Tensor rng_state, bool return_max_logit, "
      "bool cuda_graph) -> (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, "
      "Tensor, Tensor, int)");
  m.def(
      "fused_attn_bwd_packed("
      ...
      "Tensor cu_seqlens_q, Tensor cu_seqlens_kv, Tensor? cu_seqlens_q_padded, Tensor? "
      ...
      "Tensor S_data, Tensor? S_amax, Tensor? S_scale, Tensor? S_scale_inv, "
      "Tensor dP_data, Tensor? dP_amax, Tensor? dP_scale, Tensor? dP_scale_inv, "
  ...
  m.def(
      "thd_second_half_lse_correction(Tensor lse, Tensor lse_per_step, Tensor cu_seqlens, bool "
      "lse_packed) -> ()");
  m.def(
      "thd_read_second_half_lse(Tensor lse, Tensor cu_seqlens, bool lse_packed, int "
      "second_half_lse_seqlen) -> Tensor");
  m.def(
      "thd_out_correction(Tensor out, Tensor out_per_step, Tensor lse, Tensor lse_per_step, Tensor "
      "cu_seqlens, bool only_second_half, bool lse_packed) -> ()");
  m.def(
      "thd_grad_correction(Tensor grad, Tensor grad_per_step, Tensor cu_seqlens, str first_half, "
      "str second_half) -> ()");
  m.def(
      "thd_get_partitioned_indices(Tensor cu_seqlens, int total_tokens, int world_size, int rank) "
      "-> Tensor");
  ...
  m.def(
      "copy_to_kv_cache(Tensor new_k, Tensor new_v, Tensor k_cache, Tensor v_cache, Tensor "
      "page_table, Tensor cu_new_lens, Tensor cu_cached_lens, int qkv_format, int b, int "
      "max_ctx_len, int max_seq_len, int max_pages_per_seq, bool is_non_paged) -> ()");
}
