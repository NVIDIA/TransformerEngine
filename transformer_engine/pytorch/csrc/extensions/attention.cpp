...
std::vector<py::object> fused_attn_bwd(
  ...
  std::tie(te_dQ, py_dQ, dq_amax_buf) =
      quantizer_helper(dqkv_quantizer, q_shape, fake_dtype_te, true, dQ);
  std::tie(te_dK, py_dK, dk_amax_buf) =
      quantizer_helper(dqkv_quantizer, k_shape, fake_dtype_te, true, dK);
  std::tie(te_dV, py_dV, dv_amax_buf) =
      quantizer_helper(dqkv_quantizer, v_shape, fake_dtype_te, true, dV);
