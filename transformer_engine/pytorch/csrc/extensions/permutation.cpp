...
std::tuple<Tensor, Tensor> moe_unpermute_bwd(Tensor input_bwd, Tensor input_fwd, int64_t dtype,
  ...
  auto te_dtype = static_cast<DType>(dtype);
  auto bwd_shape = getStableTensorShape(input_bwd);
  NVTE_CHECK(bwd_shape.size() == 2, "Input must be 2D.");
  const size_t num_cols = bwd_shape[1];

  auto prob_shape = getStableTensorShape(prob);
  const size_t topK = (prob.numel() > 0) ? prob_shape[1] : 1;
  const size_t num_tokens =
      (prob.numel() > 0) ? prob_shape[0] : getStableTensorShape(row_id_map)[0];

  auto fwd_shape = getStableTensorShape(input_fwd);

  auto device_idx = input_bwd.get_device_index();
  auto act_grad =
      allocateStableTensor({static_cast<int64_t>(fwd_shape[0]), static_cast<int64_t>(num_cols)},
                           GetStableScalarType(te_dtype), device_idx);
  auto prob_grad =
      allocateStableTensorZeros({static_cast<int64_t>(num_tokens), static_cast<int64_t>(topK)},
                                ScalarType::Float, device_idx);

  auto input_bwd_cu = makeTransformerEngineTensor(
      input_bwd.data_ptr(), std::vector<size_t>{bwd_shape[0], num_cols}, te_dtype);
  auto act_grad_cu = makeTransformerEngineTensor(
      act_grad.data_ptr(), std::vector<size_t>{static_cast<size_t>(fwd_shape[0]), num_cols},
      te_dtype);
  auto row_id_map_cu = makeTransformerEngineTensor(row_id_map);
  auto prob_cu = makeTransformerEngineTensor(prob);
  auto prob_grad_cu = makeTransformerEngineTensor(prob_grad);
  auto input_fwd_cu = makeTransformerEngineTensor(
      input_fwd.data_ptr(), std::vector<size_t>{static_cast<size_t>(fwd_shape[0]), num_cols},
      te_dtype);
  TensorWrapper empty;
