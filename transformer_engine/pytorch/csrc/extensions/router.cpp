...
std::tuple<at::Tensor, at::Tensor, at::Tensor> fused_topk_softmax_sigmod_fwd(
    ...
    c10::optional<int> num_groups,
    c10::optional<int> group_topk,
    ...
    TORCH_CHECK(num_tokens > 0 && num_experts > 0, "num_tokens and num_experts must be greater than 0");
    // Pre-softmax only happens at the softmax case
    if(use_pre_softmax) {
        TORCH_CHECK(score_function == "softmax", "score_function must be softmax when use_pre_softmax is true");
    }
    // Expert bias only happens at the sigmoid case
    if(expert_bias.has_value()) {
        TORCH_CHECK(score_function == "sigmoid", "score_function must be sigmoid when expert_bias is not None");
    }
    // Check if the score function is valid
    TORCH_CHECK(score_function == "softmax" || score_function == "sigmoid", "score_function must be softmax or sigmoid for router fusion");
    ...
    at::Tensor probs = at::empty({num_tokens, num_experts}, at::dtype(logits.scalar_type()).device(at::kCUDA));
    at::Tensor routing_map = at::empty({num_tokens, num_experts}, at::dtype(at::kBool).device(at::kCUDA));
    // Intermediate output is used to store the output of the softmax/sigmoid function
    at::Tensor intermediate_output = at::empty({num_tokens, num_experts}, at::dtype(logits.scalar_type()).device(at::kCUDA));
    ...
    nvte_fused_topk_softmax_sigmoid_forward(
        ...
        score_function_map[score_function],
        expert_bias_cu.data(),
        probs_cu.data(),
        routing_map_cu.data(),
...
at::Tensor fused_topk_softmax_sigmod_bwd(
    int num_tokens,
    int num_experts,
    ...
    at::Tensor grad_logits = at::empty({num_tokens, num_experts}, at::dtype(intermediate_output.scalar_type()).device(at::kCUDA));
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> fused_scores_for_aux_loss_fwd(
    ...
    // Construct the output tensor
    at::Tensor scores = at::empty({num_tokens, num_experts}, at::dtype(logits.scalar_type()).device(at::kCUDA));
    at::Tensor routing_map = at::empty({num_tokens, num_experts}, at::dtype(at::kBool).device(at::kCUDA));
    at::Tensor intermediate_output = at::empty({num_tokens, num_experts}, at::dtype(logits.scalar_type()).device(at::kCUDA));
...
at::Tensor fused_scores_for_aux_loss_bwd(
    int num_tokens,
    int num_experts,
    at::Tensor intermediate_output,
    ...
    nvte_fused_scores_for_aux_loss_backward(
        intermediate_output_cu.data(),
        grad_scores_cu.data(),
        num_tokens,
        num_experts,
...
std::tuple<at::Tensor, at::Tensor> fused_aux_loss_fwd(
    at::Tensor probs,
    ...
    nvte_fused_aux_loss_forward(
        probs_cu.data(),
        tokens_per_expert_cu.data(),
        num_tokens,
        num_experts,
        topk,
        coeff,
...
at::Tensor fused_aux_loss_bwd(
    at::Tensor Const_buf,
    at::Tensor tokens_per_expert,
    int num_tokens,
    int num_experts,
    at::Tensor grad_aux_loss
){
    // Create the output tensor
    at::Tensor grad_probs = at::empty({num_tokens, num_experts}, at::dtype(grad_aux_loss.scalar_type()).device(at::kCUDA));
    ...
    nvte_fused_aux_loss_backward(
        Const_buf_cu.data(),
        tokens_per_expert_cu.data(),
        num_tokens,
        num_experts,
        grad_aux_loss_cu.data(),
        grad_probs_cu.data(),
        at::cuda::getCurrentCUDAStream()
    );
