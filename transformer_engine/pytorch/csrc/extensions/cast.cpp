  ...
  switch (grouped_quantization_mode) {
    case GroupedQuantizationMode::NVFP4_GROUPED_QUANTIZE: {
      ...
      group_quantize_nvfp4_impl(grouped_input_tensor, grouped_output_tensor_cpp,
                                ...
                                !nvfp4_quantizer_cpp->disable_2d_scaling);
      break;
    }
...
std::tuple<std::vector<py::object>, std::vector<TensorWrapper>, bool> bulk_allocate_nvfp4_tensors(
  ...
  const bool disable_2d_scaling = quantizer_cpp_list[0]->disable_2d_scaling;
  const auto columnwise_usage = quantizer_cpp_list[0]->columnwise_usage;
  if (row_scaled_nvfp4) {
  ...
  for (size_t i = 1; i < num_tensors; ++i) {
    ...
    NVTE_CHECK(quantizer_cpp_list[i]->disable_2d_scaling == disable_2d_scaling,
               "NVFP4 bulk allocation requires all quantizers in the group to share "
               "the same disable_2d_scaling value.");
  }
  ...
  for (size_t i = 0; i < num_tensors; ++i) {
    ...
    py::object amax_rowwise =
        (rowwise_usage && !disable_2d_scaling) ? py::cast(amax_rowwise_list[i]) : py::none();
    py::object amax_columnwise =
        (columnwise_usage && !disable_2d_scaling) ? py::cast(amax_columnwise_list[i]) : py::none();
...
void split_quantize_nvfp4_impl_with_rht_helper(const TensorWrapper &input,
  ...
  // Compute amaxes
  if (quantizer.disable_2d_scaling) {
