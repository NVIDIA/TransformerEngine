...
py::object create_empty_quantized_tensor(py::handle quantizer, const std::vector<size_t> &shape,
  ...
  py::object group_quantize(const at::Tensor &tensor, py::handle quantizer,
  ...
  std::tuple<std::vector<py::object>, std::vector<TensorWrapper>> bulk_allocate_mxfp8_tensors(
    ...
    for (size_t i = 0; i < num_tensors; ++i) {
      ...
      tensor_py_list.emplace_back(
          MXFP8TensorClass(rowwise_data, rowwise_scale, columnwise_data, columnwise_scale,
                           fp8_dtype, quantizer_py_list[i], with_gemm_swizzled_scales));
  ...
  std::tuple<std::vector<py::object>, std::vector<TensorWrapper>, bool> bulk_allocate_nvfp4_tensors(
    ...
    const bool with_gemm_swizzled_scales =
        false;  /// TODO (tmoon) Enable based on optimize_for_gemm;
    constexpr size_t scale_elem_size = 1;
    ...
    for (size_t i = 0; i < num_tensors; ++i) {
      ...
      py::object amax_columnwise =
          columnwise_usage ? py::cast(amax_columnwise_list[i]) : py::none();
  ...
  void split_quantize_nvfp4_impl(const TensorWrapper &input,
    ...
    // Check tensor lists
    const size_t num_tensors = split_sections.size();
    NVTE_CHECK(input_list.size() == num_tensors, "Expected ", num_tensors,
               " input tensors, but got ", input_list.size(), ".");
    NVTE_CHECK(output_list.size() == num_tensors, "Expected ", num_tensors,
               " output tensors, but got ", output_list.size(), ".");
