...
def efficient_entropy_kernel_general_mainloop(
    ...
    stride_max_n,
    max_indices_ptr,
    stride_max_indices_m,
    stride_max_indices_n,
    accu_ptr,
    stride_accu_m,
    stride_accu_n,
    entropy_b_ptr,
    ...
    maximum_indices_ptrs = (
        max_indices_ptr + offs_max_n * stride_max_indices_n + offs_max_m * stride_max_indices_m
    )
    tl.store(
        maximum_indices_ptrs,
        _max_indices,
    ...
    # store logprobs
    mask = (labels >= pid_n * vocab_per_split) & (
        labels < min((pid_n + 1) * vocab_per_split, vocab_size)
    )
...
def efficient_entropy_triton_kernel_epilogue(
    ...
    global_max_ptr,
    stride_global_max,
    global_max_indices_ptr,
    stride_global_max_indices,
    accu_ptr,
    stride_accu_m,
    stride_accu_n,
    global_accu_ptr,
    ...
    for pid_n in range(0, tl.cdiv(num_splits, BLOCK_SIZE_N)):
        ...
        max_indices_ptrs = (
            max_indices_ptr
            + offs_m[:, None] * stride_max_indices_m
            + offs_n[None, :] * stride_max_indices_n
        )
    ...
    max_indices_ptrs = (
        max_indices_ptr + offs_m * stride_max_indices_m + offs_n * stride_max_indices_n
    )
...
def efficient_entropy_foward(
    ...
    assert (
        _accu.is_contiguous()
        and _entropy_b.is_contiguous()
        and _max.is_contiguous()
        and _max_indices.is_contiguous()
    )
    ...
    efficient_entropy_kernel_general_mainloop[mainloop_grid](
        ...
        _max.stride(1),
        _max_indices,
        _max_indices.stride(0),
        _max_indices.stride(1),
        _accu,
        _accu.stride(0),
        _accu.stride(1),
        _entropy_b,
...
def efficient_entropy_backward_kernel_general_preprocess(
    ...
    d_max_ptr,
    stride_d_max,
    d_max_additional_ptr,
    stride_d_max_additional,
    d_acc_exp_logits_ptr,
    stride_d_acc_exp_logits,
    BLOCK_SIZE_M: tl.constexpr,
    BLOCK_SIZE_N: tl.constexpr,
    BLOCK_SIZE_K: tl.constexpr,
):
    ...
    if reduction == 0:  # none
        ...
        d_logprobs = tl.broadcast_to(
            tl.fdiv(tl.load(d_logprobs_ptr), num_tokens.to(tl.float32)), (BLOCK_SIZE_M,)
        )
...
@triton.jit
def efficient_entropy_backward_kernel_general_preprocess_update(
    num_tokens: int,
    accu_ptr,
    stride_accu,
    d_entropy_ptr,
...
@triton.autotune(
    configs=[
        triton.Config(
            {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 32}, num_stages=3, num_warps=4
        )
    ],
    key=["num_tokens", "hidden_size", "vocab_size"],
)
...
def efficient_entropy_backward_kernel_general_mainloop_MN(
    ...
    d_weight_ptrs = (
        d_weight_ptr + offs_k[:, None] * stride_d_weight_k + offs_bn[None, :] * stride_d_weight_n
    )
...
def efficient_entropy_backward(
    ...
    assert maximum.is_contiguous() and maximum_indices.is_contiguous() and acc.is_contiguous()
    assert (
        maximum.device == hidden.device
        and maximum_indices.device == hidden.device
        and acc.device == hidden.device
    )
    ...
    efficient_entropy_backward_kernel_general_preprocess[preprocess_grid](
        ...
        _d_max.stride(0),
        _d_max_additional,
        _d_max_additional.stride(0),
        _d_acc_exp_logits,
        _d_acc_exp_logits.stride(0),
    )
