...
class mHCElementwiseOp(torch.autograd.Function):
    ...
    def forward(ctx, H, alpha, beta, r, n):
        ...
        out = torch.zeros((M, 32), device=H.device, dtype=H.dtype) # Pad the output to 32 in the last dimension
    ...
    def backward(ctx, grad_out):
        ...
        grad_h = torch.zeros((M, 32), device=grad_out.device, dtype=grad_out.dtype) # Pad the grad_h to 32 in the last dimension
...
class mHCSinkhornOp(torch.autograd.Function):
    ...
    def forward(ctx, H_res, n=4, iters=20):
        ...
        _mhc_sinkhorn_knopp_fwd_fused[grid](
            ...
            stride_out_m=n*n, stride_out_n=1,
            M=B*T, n=n,
            iters=iters,
        )
    ...
    def backward(ctx, grad_out):
        ...
        _mhc_sinkhorn_knopp_bwd_fused[grid](
            ...
            stride_grad_out_m=n*n, stride_grad_out_n=1,
            stride_out_m=n*n, stride_out_n=1,
