...
def cast_master_weights_to_fp8(model_weights, master_weights, start_offsets, group):
    ...
    for model_weight, master_weight, start_offset in zip(
        ...
        if isinstance(quantizer, Float8Quantizer):
            ...
            blockwise_scaling_params.append((model_weight, master_weight, start_offset))
    ...
    if len(blockwise_scaling_params) > 0:
        _cast_master_weights_to_fp8_blockwise_scaling(blockwise_scaling_params, group)
...
def _cast_master_weights_to_fp8_blockwise_scaling(params, group):
    ...
    # ---------------------------------------------------------------------------------------------
    amaxes, scales, scale_invs = [], [], []
    for i, (model_weight, master_weight, start_offset) in enumerate(params):
