    ...
    def _create_columnwise(self):
        ...
        if self._is_2D_scaled:
            ...
            if self._columnwise_scale_inv is None:
                ...
                columnwise_scale_inv_shape = self._quantizer.get_scale_shape(
                    rowwise_data.shape, True
                )
            ...
            assert self._get_quantizer().rowwise_fmt == tex.RowwiseFmt.COMPACT_DATA_AND_SCALES, (
                "Float8BlockwiseQTensor only supports creating columnwise data from 1D scaled"
                " tensors with compact data and scales."
            )
            ...
            self._columnwise_data = torch.empty(
                (rowwise_shape[-1], rowwise_shape[0]),
                dtype=self._rowwise_data.dtype,
                device=self._rowwise_data.device,
            )
            self._columnwise_scale_inv = torch.empty(
                self._quantizer.get_scale_shape(rowwise_shape, True),
                dtype=self._rowwise_scale_inv.dtype,
                device=self._rowwise_scale_inv.device,
            )
