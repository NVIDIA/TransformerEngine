...
def fp8_primary_weights_cast_helper(model_params, master_params, start_offsets, group):
    ...
    if isinstance(recipe, DelayedScaling):
        ...
        fp8_primary_weights_cast_helper_delayed_scaling(
            model_params, master_params, start_offsets, group
        )
    elif isinstance(recipe, Float8CurrentScaling):
        fp8_primary_weights_cast_helper_float8_current_scaling(
            model_params, master_params, start_offsets, group
        )
    elif isinstance(recipe, MXFP8BlockScaling):
        raise NotImplementedError(
            "fp8_primary_weights_cast_helper for MXFP8BlockScaling is not supported yet"
        )
    else:
        raise ValueError(f"Unsupported FP8 recipe: {type(recipe)}")


def fp8_primary_weights_cast_helper_delayed_scaling(
    model_params, master_params, start_offsets, group
):
    for model_param, master_param, start_offset in zip(model_params, master_params, start_offsets):
        assert (master_param is not None and start_offset is not None) or (
            master_param is None and start_offset is None
        )
        if master_param is None:
            continue

        shard_model_param = model_param._data.view(-1)[
            start_offset : start_offset + master_param.numel()
        ]
...
def fp8_primary_weights_cast_helper_float8_current_scaling(
    model_params, master_params, start_offsets, group
):
    for model_param, master_param, start_offset in zip(model_params, master_params, start_offsets):
        assert (master_param is not None and start_offset is not None) or (
