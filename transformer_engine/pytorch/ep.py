...
class _EpPrepareAndDispatch(torch.autograd.Function):
    ...
    def forward(  # type: ignore[override]
        ...
        """Fused prepare + dispatch fwd; the dispatch writes ``tokens_per_expert``. When scales are
        set (MXFP8 for now), ``tokens`` is the quantized tensor kept as the autograd operand so grad
        reaches the pre-quant input, and the fused op routes scales; recv data + scales share one
        buffer carved here (or supplied by the caller)."""
        is_scaled = tokens_scale_inv is not None
        tokens_data = tokens._rowwise_data if isinstance(tokens, QuantizedTensor) else tokens
        hidden = tokens_data.shape[-1]
        device = tokens_data.device
        ...
        # recv data + scales are wrapped into a per-expert GroupedTensor (built from the counts the
        # fused op just wrote) so downstream grouped GEMM and autograd see a quantized grouped tensor.
        if is_scaled:
            recv_out = _make_grouped_mxfp8(
                recv_tokens.view(tokens._rowwise_data.dtype),
                recv_scale_inv,
            ...
            recv_out = recv_tokens.detach()
        return recv_out, recv_topk_weights.detach(), tokens_per_expert
...
def ep_dispatch(
    ...
    *,
    ...
    Non-eager bf16 and non-zero-copy MXFP8 use the fused prepare+dispatch: the dispatch seeds
    routing and writes tokens_per_expert in one call, with recv sized to ``recv_capacity_per_rank``.
    Eager mode (host-synced recv-token total) and zero-copy MXFP8 use a separate prepare + dispatch.
    """
    ...
    # Eager mode (host-synced recv-token total for sizing) uses a separate prepare + dispatch.
    # Zero-copy MXFP8 also does: the fused count-mode dispatch does not write the per-expert counts
    # under zero-copy (the permute that produces them is elided), and the MXFP8 recv GroupedTensor
    # needs those counts, so the standalone prepare must write them first.
    if buffer.eager or (tokens_scale_inv is not None and buffer.zero_copy):
    ...
    # Non-eager (incl. drop_on_overflow): fused prepare+dispatch, recv sized to
    # recv_capacity_per_rank; the fused dispatch drops over-budget tokens when drop_on_overflow
    # is enabled, otherwise traps on overflow. bf16 sizes recv here; MXFP8 carves the recv
    # data+scales buffer inside the op (num_recv_tokens sizes it), so size only its weights here.
    rows = buffer.recv_capacity_per_rank
    if tokens_scale_inv is None:
        recv_tokens, recv_topk_weights = _size_recv_buffers(
            buffer, rows, recv_tokens, recv_topk_weights
        )
    elif recv_topk_weights is None:
        recv_topk_weights = _alloc_io((rows,), torch.float32, buffer.device, buffer.zero_copy)
