...
class SymmTensor(torch.Tensor):
    ...
    @staticmethod
    def __new__(cls, pool: torch.Tensor, offset: int, shape: torch.Size, dtype: torch.dtype, allocator: 'SymmAllocator'):
        ...
        assert pool.numel() >= offset + nbytes, f"Pool too small: {pool.numel()} bytes, need {offset + nbytes}"
...
class SymmAllocator:
    def __init__(self, size_bytes: int, device: torch.device, dist_group: torch.distributed.group):
        ...
        if os.environ.get("NVTE_USE_UB_FOR_UBNEXT"):
            ...
            self.internal_pool.view(torch.int64)[:self.world_size].copy_(torch.tensor(self.hdl0.buffer_ptrs).view(torch.int64))
    ...
    def allocate(self, nbytes: int) -> Tuple[Optional[int], Optional[torch.Tensor]]:
        ...
        raise MemoryError(f"Preallocated pool exhausted: requested {nbytes} bytes, "
    ...
    def create_tensor(self, shape: Tuple[int, ...], dtype: torch.dtype = torch.float32) -> Optional[torch.Tensor]:
    ...
    def allreduce_lamport(self, tensor_in: torch.Tensor) -> torch.Tensor:
        ...
        """
        assert tensor_in.device == self.device, "Tensor device mismatch with allocator device"
        if self.mc0_ptr is None or self.mc0_ptr == 0: return self.allreduce_uc(tensor_in)
        ...
        if self.nextpoisoned is None or self.nextpoisoned.shape!=tensor_in.shape:
            ...
            tensor_out = self.create_tensor(tensor_in.shape, tensor_in.dtype)
            poisonedout = False
        if tensor_out is None: return self.allreduce(tensor_in)
...
def ubsymm_request_allocator(dist_group: torch.distributed.group, shape:Optional[Tuple[int, ...]] = None, dtype: torch.dtype = torch.bfloat16) -> None:
...
def ubsymm_get_sym_tensor(shape: Tuple[int, ...], dtype: torch.dtype, dist_group: torch.distributed.group) -> torch.Tensor:
    ...
    if dist_group not in _allocator_map: return None # No allocator requested earlier, do fallback to nccl
    (max_size, allocator) = _allocator_map[dist_group]
    if allocator is None:
        new_max_size = int(os.environ.get('NVTE_UB_SYMM_POOL_SIZE', ((6*max_size+1048575)/1024/1024) ) )
