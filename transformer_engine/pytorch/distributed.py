...
def graph_safe_rng_available() -> bool:
    ...
    return (
        hasattr(te_platform().CUDAGraph, "register_generator_state")
...
def _get_cuda_rng_state(
    ...
    default_generator = te_platform().default_generators[idx]
...
class _CheckpointFunction(torch.autograd.Function):
    ...
    two main changes:
        1) te_platform().set_rng_state is replaced with `_set_cuda_rng_state`
...
def _get_symm_mem_pool(device: torch.device, backend: str = "NCCL"):
    ...
    if _SYMM_MEM_POOL is None:
        ...
        if hasattr(symm_mem, "get_mem_pool"):
            ...
            _SYMM_MEM_POOL = te_platform().MemPool(symm_mem.get_mempool_allocator(device))
        else:
            raise RuntimeError(
                "No symmetric-memory MemPool API available (need torch symm-mem get_mem_pool, or "
                "te_platform().MemPool + get_mempool_allocator)."
            )
...
def symm_mem_alloc(
    ...
    if use_pool:
        ...
        with te_platform().use_mem_pool(pool):
