...
python3 -m pytest --import-mode=importlib -v --junitxml=$XML_LOG_DIR/pytest_test_common_attention_helpers.xml $TE_PATH/tests/test_common_attention_helpers.py || test_fail "test_common_attention_helpers.py"
python3 -m pytest -c $TE_PATH/tests/jax/pytest.ini -v --junitxml=$XML_LOG_DIR/pytest_jax_not_distributed.xml $TE_PATH/tests/jax --ignore=$TE_PATH/tests/jax/test_multi_process_ep.py --ignore=$TE_PATH/tests/jax/test_fp8_fused_attn.py -k 'not distributed' || test_fail "tests/jax/*not_distributed_*"
python3 -m pytest -c $TE_PATH/tests/jax/pytest.ini -v --junitxml=$XML_LOG_DIR/pytest_jax_fp8_fused_attn.xml $TE_PATH/tests/jax/test_fp8_fused_attn.py || test_fail "tests/jax/test_fp8_fused_attn.py"
