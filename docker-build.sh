...
if [[ "$BUILD_BASE" -eq 1 ]]; then
  ...
  # CI exposes per-architecture overrides; select the one for this build.
  if [[ -z "${CUDNN_BACKEND_URL}" ]]; then
    case "${ARCH}" in
      amd64) CUDNN_BACKEND_URL="${CUDNN_BACKEND_URL_AMD64}" ;;
      arm64) CUDNN_BACKEND_URL="${CUDNN_BACKEND_URL_ARM64}" ;;
    esac
  fi
  ...
  ARTIFACTORY_SECRETS_ARGS=""
  if [[ -n "${ARTIFACTORY_USER}" && -n "${ARTIFACTORY_TOKEN}" ]]; then
    ARTIFACTORY_SECRETS_ARGS="--secret id=ARTIFACTORY_USER,env=ARTIFACTORY_USER --secret id=ARTIFACTORY_TOKEN,env=ARTIFACTORY_TOKEN"
  fi
  ...
  docker buildx build $PULL_FLAG $FROM_FLAG \
      ...
      $FRAMEWORK_ARG \
      ${CCACHE_BUILD_ARG} \
      --build-arg CUDNN_FRONTEND="${CUDNN_FRONTEND}" \
      --build-arg WITH_CUBLASMP="${WITH_CUBLASMP}" \
      --build-arg WITH_NCCL_EP="${WITH_NCCL_EP}" \
      ${CUDNN_BACKEND_URL_ARG} \
      ${INSTALL_FA3_ARG} \
      ${ARTIFACTORY_SECRETS_ARGS} \
