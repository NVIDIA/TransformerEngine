...
.. code-block:: python
   ...
   class Quantizer:
       # Core interface
       def __call__(self, tensor) -> QuantizedTensor: ...
       def quantize(self, tensor, *, out=None, dtype=None) -> QuantizedTensor: ...
...
.. list-table::
   ...
   * - ``Float8BlockQuantizer``
     ...
     - 1D or 2D block scaling with a fixed block length of 128 (dimensionality is configurable via ``block_scaling_dim``)
   * - ``NVFP4Quantizer``
