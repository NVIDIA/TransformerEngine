...
def speedometer(
    model_apply_fn: Callable, # should be model.apply 
    variables: Any,
    input: jnp.ndarray,
...
def create_train_step_fn(
    ...
    def loss_fn(variables : Any, inp: jnp.ndarray, grad_target: jnp.ndarray, dropout_key, key):
        ...
        with te.fp8_autocast(**fp8_autocast_kwargs):
            ...
            # Note: model_init_fn is only called here for debugging/validation purposes
            # The actual variables should already be initialized with FP8 metadata
            if model_init_fn is not None and fp8_autocast_kwargs.get('enabled', False):
                # This verifies the model can be initialized within the FP8 context
                # but we don't use the result - we use the pre-initialized variables
                _ = model_init_fn(key, inp, deterministic=False)
            
            # Merge forward_kwargs with rngs
    ...
    # We take gradients with respect to both parameters (0) and other variables (1)
    # because TE/Flax models often have mutable states like FP8 scaling factors 
    # stored in 'other_vars' (e.g., 'params_frozen').
...
def create_train_step_fn_vjp(
    ...
    def train_step_fn(params: Any, other_vars: Any, inp: jnp.ndarray, grad_target: jnp.ndarray):
        ...
        # Return loss value (for compatibility) and gradients
...
def share_parameters_with_basic_te_model(basic_model_params, te_params_template):
   ...
   # 1. Layer Norms
    ...
    # 2. QKV and Projection (assuming layer names are consistent)
    # The structure might differ slightly (e.g., TE might use a single 'Dense' or 'DenseGeneral')
...
def share_parameters_with_transformerlayer_te_model(basic_model_params, te_params_template):
    ...
    TE TransformerLayer parameter shapes: {
        'attention': {
            'out': {
...
# 1. Layer Norms
    ...
    # 2. Attention QKV and Output Projection
    ...
    # 4. Relative Positional Bias (if any) -- this is usually model specific
    # Example:
