...
recv_capacity = ep_size * num_tokens * top_k  # dropless bound with alignment=0
...
# Expert blocks are packed consecutively in [recv_capacity, hidden_size].
recv_tokens, recv_w, tokens_per_expert = ep_dispatch(buffer, tokens, topk_idx, topk_w)

# With alignment=0, unused capacity is a tail after all expert blocks.
valid = torch.arange(recv_capacity, device=tokens.device) < tokens_per_expert.sum()
recv_tokens = recv_tokens.masked_fill(~valid[:, None], 0)
recv_w = recv_w.masked_fill(~valid, 0)
expert_out = experts(recv_tokens, tokens_per_expert)

# Clear unused outputs before weighting, including possible NaNs in the tail.
expert_out = expert_out.masked_fill(~valid[:, None], 0)
