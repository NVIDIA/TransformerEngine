...
| `CUDNN_FRONTEND` | `nvidia-cudnn-frontend[cutedsl]==1.25.0` | An argument to pip3 install command, to be used to install nvidia-cudnn-frontend package. |
| `CUDNN_BACKEND_URL_AMD64` |                   | Optional cuDNN backend archive URL for AMD64 base images. URM URLs and authenticated `artifactory.nvidia.com` URLs are supported. |
| `CUDNN_BACKEND_URL_ARM64` |                   | Optional cuDNN backend archive URL for ARM64/SBSA base images. URM URLs and authenticated `artifactory.nvidia.com` URLs are supported. |
| `SEND_SLACK_MESSAGE`  | `0`                   | Send Slack message on CI completion                       |

For example, an AMD64 cuDNN backend override can point to either Artifactory
(`https://artifactory.nvidia.com/artifactory/hw-cudnn-generic-local/CUDNN/v9.24/v9.24.0.31/13.3/cudnn-linux-x86_64-9.24.0.31.tar.gz`)
or URM
(`https://urm.nvidia.com/artifactory/hw-cudnn-generic-local/CUDNN/v9.21/v9.21.0.19/13.2/cudnn-linux-x86_64-9.21.0.19.tar.gz`).
For ARM64/SBSA, use the matching `cudnn-linux-sbsa-...tar.gz` archive.
